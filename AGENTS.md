...
- `Projetos/draxos-mobile/` is the P2 implementation workspace for DraxosMobile - PVE Arena-first async autobattler, base manager, later PVP, social; Godot 4.6.2 + Supabase; Android + PC + PC browser. Current operational stage: `BOSQUE_FEEL_SPAWN_AUTHORITY_V1_PUBLISHED_INTERNAL_ALPHA`; the latest remote Internal Alpha package is Bosque Feel & Spawn Authority v1 (`internal-alpha/v0-bosque-feel-spawn-authority-v1-20260608-70b79c3`, preview evidence `https://16ac3cb7.draxos-mobile-internal-alpha.pages.dev`, official URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`, APK/manifest `0.0.11-alpha.0`/version code `11`), preserving Bosque operations-v2 durable persistence while fixing player feel around server authority: server-time node cooldowns, visual respawn grace, sticky collection, no one-frame node rollback and non-freezing deposit/craft/menu actions; Bosque Persistence Rebase v1 remains the previous persistence/operations package, followed by Bosque Session Lifecycle & Durable Structures Hotfix v1, Bosque World Hub Domain Separation v1, Bosque Fogueira Potion Crafting v1, Bosque Durable Bau Mochila v1, Arena PVE Menu Flow Simplification v1, Bosque Offline-First Checkpoint v1, Bosque Sync Responsiveness v1, Arena/Bosque Visible V2, Arena/Bosque Regression Hotfix, Arena PVE Season 1 Loop v1 and the earlier Internal Alpha lineage.
...
- Projetos com `ARQUIVO_DESIGN` permitem apenas leitura e referencia de design Ã¢â‚¬â€ nao criar codigo, cenas ou assets.
...
- `Projetos/draxos-roguelike-cardgame/` Ã¢â‚¬â€ P0, Steam, roguelike cardgame
- `Projetos/draxos-mobile/` - P2, mobile + PC + browser, Godot 4.6.2 + Supabase (`BOSQUE_FEEL_SPAWN_AUTHORITY_V1_PUBLISHED_INTERNAL_ALPHA`; proximo passo e playtest humano do pacote publicado, validando feel de coleta, ausencia de flicker/rollback visual, cooldowns por node apos relog, Fogueira persistente somente apos ACK, aviso ao sair com operacoes pendentes e regressao de Arena/Bosque)
