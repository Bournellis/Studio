...
- Remote backup: `origin` = `https://github.com/Bournellis/Studio.git` (private GitHub repository). Agents perform LOCAL git by default (`commit`, `merge`, `branch`, `worktree`, `status`, `diff`, `log`). Remote git stays Fabio/GitHub Desktop by default; when Fabio explicitly authorizes an agent push, the only approved agent path is non-interactive git: set `GCM_INTERACTIVE=Never` and `GIT_TERMINAL_PROMPT=0`, then run `git push origin <branch>`. Never run naked `git push`, never start `gh auth login`, never create/store PATs, and stop immediately if credentials are missing instead of opening browser login. Fallback remains `PUSH PENDENTE: Fabio - GitHub Desktop - Push origin`. Policy: `08_Coordenacao_Agentes/Decisoes/2026-06-14_estudio_git_push_nao_interativo_agentes.md`.
