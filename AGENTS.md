...
- `Projetos/draxos-mobile/` is the P2 implementation workspace for DraxosMobile - PVE Arena-first async autobattler, base manager, later PVP, social; Godot 4.6.2 + Supabase; Android + PC + PC browser. Current operational stage: `ARENA_PVE_BONUS_VISUAL_V1_PUBLISHED_INTERNAL_ALPHA`; the latest remote Internal Alpha package is Arena PVE Bonus Visual v1 (`internal-alpha/v0-arena-pve-bonus-visual-v1-20260608-e281d63`, preview evidence `https://6c8bf8e1.draxos-mobile-internal-alpha.pages.dev`, official URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`, APK/manifest `0.0.14-alpha.0`/version code `14`, minimum supported code `13`), fixing Arena PVE reward buffs by exporting initial `hp`, `max_hp`, `max_mana` and `stat_modifiers` in the battle log and making the replay apply `battle_start` before the first action; Bosque Node Cooldown ACK v1 remains the previous Bosque package, followed by Bosque Resume Exit Lifecycle v1, Bosque Feel & Spawn Authority v1, Bosque Persistence Rebase v1, Bosque Session Lifecycle & Durable Structures Hotfix v1, Bosque World Hub Domain Separation v1, Bosque Fogueira Potion Crafting v1, Bosque Durable Bau Mochila v1, Arena PVE Menu Flow Simplification v1, Bosque Offline-First Checkpoint v1, Bosque Sync Responsiveness v1, Arena/Bosque Visible V2, Arena/Bosque Regression Hotfix, Arena PVE Season 1 Loop v1 and the earlier Internal Alpha lineage.
...
- `Projetos/draxos-mobile/` - P2, mobile + PC + browser, Godot 4.6.2 + Supabase (`ARENA_PVE_BONUS_VISUAL_V1_PUBLISHED_INTERNAL_ALPHA`; proximo passo e playtest humano do pacote publicado, validando buffs de HP/Mana entre lutas na Arena PVE, replay mostrando HP inicial buffado e regressao rapida de Arena/Bosque)
