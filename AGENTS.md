...
- `Projetos/draxos-mobile/` is the P2 implementation workspace for DraxosMobile - PVE Arena-first async autobattler, base manager, later PVP, social; Godot 4.6.2 + Supabase; Android + PC + PC browser. Current operational stage: `BOSQUE_PERSISTENCE_REBASE_V1_PUBLISHED_INTERNAL_ALPHA`; the latest remote Internal Alpha package is Bosque Persistence Rebase v1 (`internal-alpha/v0-bosque-persistence-rebase-v1-20260608-bc23f74`, preview evidence `https://0c0a8dcf.draxos-mobile-internal-alpha.pages.dev`, official URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`, APK/manifest `0.0.10-alpha.0`/version code `10`), making Bosque durable progress server-authoritative through ACK-backed operations, persisting `openworld_forest_progress_v2`, tracking per-node cooldowns via `node_state.next_spawn_at`, and unlocking `fogueira_estavel_1` only after server-confirmed `structures`; Bosque Session Lifecycle & Durable Structures Hotfix v1 remains the previous session lifecycle package, followed by Bosque World Hub Domain Separation v1, Bosque Fogueira Potion Crafting v1, Bosque Durable Bau Mochila v1, Arena PVE Menu Flow Simplification v1, Bosque Offline-First Checkpoint v1, Bosque Sync Responsiveness v1, Arena/Bosque Visible V2, Arena/Bosque Regression Hotfix, Arena PVE Season 1 Loop v1 and the earlier Internal Alpha lineage.
...
- `Projetos/draxos-mobile/` - P2, mobile + PC + browser, Godot 4.6.2 + Supabase (`BOSQUE_PERSISTENCE_REBASE_V1_PUBLISHED_INTERNAL_ALPHA`; proximo passo e playtest humano do pacote publicado, validando ACK/retry, cooldowns por node apos relog, Fogueira persistente somente apos ACK, aviso ao sair com operacoes pendentes e regressao de Arena/Bosque)
