...
- `Projetos/draxos-mobile/` is the P2 implementation workspace for DraxosMobile - PVE Arena-first async autobattler, base manager, later PVP, social; Godot 4.6.2 + Supabase; Android + PC + PC browser. Current operational stage: `LAB_WEB_EXPORT_GUARD_PUBLISHED_INTERNAL_ALPHA`; the latest remote Internal Alpha package is the Lab Web Export Guard hotfix (`internal-alpha/v0-lab-web-export-guard-20260531-9a415c3`) on top of Track 19 Arena Consistency Pass, and Track 13 validation/release safety plus Track 14 `TRACK_14_AGENT_OPS_FOUNDATION_ACTIVE` remain active baselines.
...
- `Projetos/draxos-mobile/` - P2, mobile + PC + browser, Godot 4.6.2 + Supabase (`LAB_WEB_EXPORT_GUARD_PUBLISHED_INTERNAL_ALPHA`; Track 19 Arena Consistency Pass publicado com hotfix de Labs Web; proximo passo e playtest/tuning da Arena PVE inicial)
