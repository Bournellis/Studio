...
- `Projetos/draxos-mobile/` is the P2 implementation workspace for DraxosMobile - PVE Arena-first async autobattler, base manager, later PVP, social; Godot 4.6.2 + Supabase; Android + PC + PC browser. Current operational stage: `BOSQUE_NODE_COOLDOWN_ACK_V1_PUBLISHED_INTERNAL_ALPHA`; the latest remote Internal Alpha package is Bosque Node Cooldown ACK v1 (`internal-alpha/v0-bosque-node-cooldown-ack-v1-20260608-626b4ad`, preview evidence `https://5cce952e.draxos-mobile-internal-alpha.pages.dev`, official URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`, APK/manifest `0.0.13-alpha.0`/version code `13`), preserving Bosque operations-v2 durable persistence, local feel and resume/exit lifecycle while fixing node cooldown ACK behavior: local cooldown starts at collection time, idle same-session resync applies durable server progress, and terminal cooldown rejections clear invalid collect ops instead of retrying forever; Bosque Resume Exit Lifecycle v1 remains the previous resume/exit package, followed by Bosque Feel & Spawn Authority v1, Bosque Persistence Rebase v1, Bosque Session Lifecycle & Durable Structures Hotfix v1, Bosque World Hub Domain Separation v1, Bosque Fogueira Potion Crafting v1, Bosque Durable Bau Mochila v1, Arena PVE Menu Flow Simplification v1, Bosque Offline-First Checkpoint v1, Bosque Sync Responsiveness v1, Arena/Bosque Visible V2, Arena/Bosque Regression Hotfix, Arena PVE Season 1 Loop v1 and the earlier Internal Alpha lineage.
...
- `Projetos/draxos-mobile/` - P2, mobile + PC + browser, Godot 4.6.2 + Supabase (`BOSQUE_NODE_COOLDOWN_ACK_V1_PUBLISHED_INTERNAL_ALPHA`; proximo passo e playtest humano do pacote publicado, validando respawn/cooldown apos sair/voltar/relog, segunda coleta sem "aguardando servidor" permanente, Fogueira persistente somente apos ACK, aviso ao sair com operacoes pendentes e regressao de Arena/Bosque)
