...
- `Projetos/draxos-mobile/` is the P2 implementation workspace for DraxosMobile - PVE Arena-first async autobattler, base manager, later PVP, social; Godot 4.6.2 + Supabase; Android + PC + PC browser. Current operational stage: `BOSQUE_BOOTSTRAP_AUTHORITY_V1_PUBLISHED_INTERNAL_ALPHA`; the latest remote Internal Alpha package is Bosque Bootstrap Authority v1 (`internal-alpha/v0-bosque-bootstrap-authority-v1-20260609-ba99e70`, preview evidence `https://0123894f.draxos-mobile-internal-alpha.pages.dev`, official URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`, APK/manifest `0.0.15-alpha.0`/version code `15`, minimum supported code `13`), preventing the integrated Bosque from rendering a transient full-spawn world before canonical remote/cache bootstrap completes. Fabio reported the initial human playtest OK on 2026-06-09; documentation drift cleanup is complete and the current open decision is `DMOB-D076`, choosing the next post-Bootstrap product package. Future bugs return to the normal bugfix flow if they appear. Arena PVE Bonus Visual v1 remains the previous Arena package, and Bosque Node Cooldown ACK v1 remains the previous Bosque persistence/spawn package, followed by Bosque Resume Exit Lifecycle v1, Bosque Feel & Spawn Authority v1, Bosque Persistence Rebase v1, Bosque Session Lifecycle & Durable Structures Hotfix v1, Bosque World Hub Domain Separation v1, Bosque Fogueira Potion Crafting v1, Bosque Durable Bau Mochila v1, Arena PVE Menu Flow Simplification v1, Bosque Offline-First Checkpoint v1, Bosque Sync Responsiveness v1, Arena/Bosque Visible V2, Arena/Bosque Regression Hotfix, Arena PVE Season 1 Loop v1 and the earlier Internal Alpha lineage.
...
- `Projetos/draxos-mobile/` - P2, mobile + PC + browser, Godot 4.6.2 + Supabase (`BOSQUE_BOOTSTRAP_AUTHORITY_V1_PUBLISHED_INTERNAL_ALPHA`; playtest humano inicial do Bosque Bootstrap Authority v1 reportado OK por Fabio em 2026-06-09; drift documental corrigido; proximo foco e decidir `DMOB-D076`)
