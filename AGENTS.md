...
- `Projetos/draxos-mobile/` is the P2 implementation workspace for DraxosMobile - PVE Arena-first async autobattler, base manager, later PVP, social; Godot 4.6.2 + Supabase; Android + PC + PC browser. Current operational stage: `PVE_ARENA_INITIAL_DIRECTION_APPROVED`; Foundation Final Polish remains the latest published Internal Alpha foundation, Track 13 validation/release safety and Track 14 `TRACK_14_AGENT_OPS_FOUNDATION_ACTIVE` remain active baselines, and the next product package is Arena PVE initial (`docs/pve-arena-initial-direction.md`).
...
- `Projetos/draxos-mobile/` - P2, mobile + PC + browser, Godot 4.6.2 + Supabase (`PVE_ARENA_INITIAL_DIRECTION_APPROVED`; Foundation Final Polish e a base publicada; proximo pacote e Arena PVE inicial)
