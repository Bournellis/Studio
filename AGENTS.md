...
- `Projetos/draxos-mobile/` is the P2 implementation workspace for DraxosMobile - PVE Arena-first async autobattler, base manager, later PVP, social; Godot 4.6.2 + Supabase; Android + PC + PC browser. Current operational stage: `ARENA_CONSISTENCY_PASS_PUBLISHED_INTERNAL_ALPHA`; Track 19 Arena Consistency Pass is the latest remote Internal Alpha package (`internal-alpha/v0-arena-consistency-pass-20260531-0865e43`), and Track 13 validation/release safety plus Track 14 `TRACK_14_AGENT_OPS_FOUNDATION_ACTIVE` remain active baselines.
...
- `Projetos/draxos-mobile/` - P2, mobile + PC + browser, Godot 4.6.2 + Supabase (`ARENA_CONSISTENCY_PASS_PUBLISHED_INTERNAL_ALPHA`; Track 19 Arena Consistency Pass publicado; proximo passo e playtest/tuning da Arena PVE inicial)
