...
- `Projetos/draxos-mobile/` is the P2 implementation workspace for DraxosMobile - PVE Arena-first async autobattler, base manager, later PVP, social; Godot 4.6.2 + Supabase; Android + PC + PC browser. Current operational stage: `REMOTE_LAB_RUNNER_PUBLISHED_INTERNAL_ALPHA`; the latest remote Internal Alpha package is Remote Lab Runner (`internal-alpha/v0-remote-lab-runner-20260531-e659d7e5`) on top of Track 19 Arena Consistency Pass, and Track 13 validation/release safety plus Track 14 `TRACK_14_AGENT_OPS_FOUNDATION_ACTIVE` remain active baselines.
...
- `Projetos/draxos-mobile/` - P2, mobile + PC + browser, Godot 4.6.2 + Supabase (`REMOTE_LAB_RUNNER_PUBLISHED_INTERNAL_ALPHA`; Track 19 Arena Consistency Pass publicado com Labs Web remotos; proximo passo e playtest/tuning da Arena PVE inicial)
