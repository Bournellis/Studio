...
- Active stage: `Post Track 06B - Track 06C/06D Ready`
- Active stage status: `MERGED_LOCAL - Track 06B approved by Claude/Fabio and merged in main as 0935529d; 06C/06D blocked only on Kenney Fonts download by Fabio`
- Status marker: `JOGO_DA_COPA_TRACK_06B_ESC_MENU_V1_MERGED`
- Approved plan: Track 06B closed with complete ESC menu and persistent runtime settings. Next: Fabio downloads Kenney Fonts, then Track 06C Menu Broadcast and Track 06D HUD Broadcast may start in parallel. Remote publication remains governed by `../../08_Coordenacao_Agentes/Decisoes/2026-06-12_jogodacopa_publicacao-web-cloudflare.md` and is deferred to 06E.
...
Track 06B ESC Menu Completo V1 is approved and merged locally in `main` as merge commit `0935529d`. The public Web baseline remains `v1.0.3+ef9c5baa` at `https://copa-arena-futebol.pages.dev/`; no 06A/06B publication before 06E. Next step: Fabio downloads Kenney Fonts to unlock Track 06C/06D.
...
Latest Track 06B branch result: PASS, 95 tests, 1512 asserts, including Web gzip transfer gate `30.34 MiB / 50.00 MiB`, source integrity check for 41 `.gd/.gdshader` files outside `addons/` and UTF-8 BOM rejection.
