...
`JogoDaCopa` is the football/TPS project split from the former `Projetos/FpsShooter` workspace. It owns the independent football minigame direction. The first playable public product surface is now `Super Campeao`, published on Cloudflare Pages as `v1.2.1+6ef3074c`.
...
- Main menu launches public `Super Campeao`; Cloudflare Pages stable URL serves `v1.2.1+6ef3074c`.
...
- Track 09A FootballRoot Extraction V1: branch local `codex/jogodacopa/footballroot-extraction-v1` extraiu ambiente noturno, capture director, placares do estadio e perf scenario para helpers dedicados; `FootballRoot` caiu de `2280` para `1862` linhas sem mudar gameplay; validate PASS `104/1825`; Web export PASS; Web gzip `30.58 MiB / 50.00 MiB`; Web boot smoke Chrome headless PASS.
...
`Super Campeao v1.2.1+6ef3074c` is public at `https://copa-arena-futebol.pages.dev/` with release root `web/v1-copa-arena-futebol-20260614-6ef3074c`. Track 08A remote gates passed; next public gate remains human retest by Fabio + external tester.

Track 09A is local-only and changes code architecture, not the public release. Next engineering gate is review/merge of `codex/jogodacopa/footballroot-extraction-v1`.
...
Latest Track 09A result: import headless PASS; Web export PASS; Web boot smoke Chrome headless PASS; `tools/validate.gd` PASS, 104 tests, 1825 asserts, Web gzip transfer `30.58 MiB / 50.00 MiB`; `FootballRoot` line count `2280 -> 1862`.
