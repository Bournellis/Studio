...
`v1.1.0+22850c06` is public at `https://copa-arena-futebol.pages.dev/` with release root `web/v1-copa-arena-futebol-20260613-22850c06`. Remote gates passed after the 06F hotfix: first minute has `0` hitches `>100ms`, page errors `0`, console errors `0`; stability 5min has heap retained `+1.14%`, nodes/caches stable and worst 5s window `105.2 FPS`; luma gate is `10.3 < 90`. Next gate is human retest by Fabio + external tester.
...
Latest Track 06F post-merge result: PASS, 101 tests, 1735 asserts, including source integrity check and UTF-8 BOM rejection.

Latest Web remote smoke: 06F first-minute `docs/playtest-reports/track-06f-data/06f-remote-first-minute-22850c06.json` PASS (`pageErrors=0`, `consoleErrorCount=0`, `firstMinuteHitches=0`); 06F stability `docs/playtest-reports/track-06f-data/06f-remote-stability-5min-22850c06.json` PASS (heap retido `+1.14%`, counters estaveis, pior janela 5s `105.2 FPS`); luma `docs/playtest-reports/track-06f-data/06f-remote-night-luma-gate-22850c06.json` PASS (`10.3 < 90`); menu footer `docs/playtest-reports/track-06f-data/06f-remote-menu-footer-22850c06.png` shows `v1.1.0+22850c06`.
