...
- Active stage: `Track 06G - Countdown Direto E Restart Confirmado V1`
- Active stage status: `PUBLICADO_RETEST_HUMANO_PENDENTE - 06G was merged and republished as v1.1.0+be453dc3; remote first-minute, stability 5min, menu URL and luma gates passed`
- Status marker: `JOGO_DA_COPA_V1_1_0_PUBLICADO_RETEST_HUMANO_PENDENTE`
- Approved plan: 06G was a surgical post-publication correction: direct kickoff countdown and restart only through ESC menu confirmation, with no gameplay tuning changes.
...
`v1.1.0+be453dc3` is public at `https://copa-arena-futebol.pages.dev/` with release root `web/v1-copa-arena-futebol-20260613-be453dc3`. Remote gates passed after 06G: first minute has `0` hitches `>100ms`, page errors `0`, console errors `0`; stability 5min has heap retained `+9.03%`, nodes/caches stable and worst 5s window `121.4 FPS`; menu user URL has runtime errors `0`; luma gate is `17.413 < 90`. Next gate is human retest by Fabio + external tester.
...
Latest Track 06G final result: PASS, 103 tests, 1842 asserts, including source integrity check and UTF-8 BOM rejection.

Latest Web remote smoke: 06G first-minute `docs/playtest-reports/track-06g-data/06g-remote-first-minute-be453dc3.json` PASS (`pageErrors=0`, `consoleErrorCount=0`, `firstMinuteHitches=0`); 06G stability `docs/playtest-reports/track-06g-data/06g-remote-stability-5min-be453dc3.json` PASS (heap retido `+9.03%`, counters estaveis, pior janela 5s `121.4 FPS`); luma `docs/playtest-reports/track-06g-data/06g-remote-night-luma-gate-be453dc3.json` PASS (`17.413 < 90`); menu user URL `docs/playtest-reports/track-06g-data/06g-remote-menu-user-url-be453dc3.json` PASS (`pageErrors=0`, `consoleErrorCount=0`, root `be453dc3`).
