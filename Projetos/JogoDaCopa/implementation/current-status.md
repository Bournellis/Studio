...
- Active stage: `Track 07 - Visual Polish & Web-Safe Broadcast Pass`
- Active stage status: `BLOQUEADO_REMOTE_GATE_ROLLBACK_EXECUTADO - Track 07B is merged locally as v1.2.0+6de8d6b7, but remote menu gate failed with AudioWorklet module AbortError; public URL was rolled back to v1.1.0+be453dc3`
- Status marker: `JOGO_DA_COPA_TRACK07B_REMOTE_MENU_WORKLET_BLOCKED_ROLLBACK_DONE`
...
`v1.1.0+be453dc3` is public at `https://copa-arena-futebol.pages.dev/` with release root `web/v1-copa-arena-futebol-20260613-be453dc3`. Track 07 `v1.2.0+138cf4f7` and Track 07B `v1.2.0+6de8d6b7` are merged locally, but publication was rolled back after remote gates failed. Track 07 failed retained JS/WASM heap (`+10.34%` vs `<10%`); Track 07B then failed menu runtime with `AbortError: Unable to load a worklet's module.`. Next gate is a new Web Audio-safe hotfix before any `v1.2.0` publication or human retest.
