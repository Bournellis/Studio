...
- Active stage status: `BLOQUEADO_REMOTE_GATE_ROLLBACK_EXECUTADO - Track 07 is merged locally as v1.2.0+138cf4f7, but remote stability 5min failed heap gate; public URL was rolled back to v1.1.0+be453dc3`
- Status marker: `JOGO_DA_COPA_TRACK07_REMOTE_STABILITY_BLOCKED_ROLLBACK_DONE`
...
`v1.1.0+be453dc3` is public at `https://copa-arena-futebol.pages.dev/` with release root `web/v1-copa-arena-futebol-20260613-be453dc3`. Track 07 `v1.2.0+138cf4f7` is merged locally, but publication was rolled back after the remote stability gate failed the retained JS/WASM heap threshold (`+10.34%` vs `<10%`). Menu URL and first-minute remote gates passed with runtime errors `0`; nodes/caches/video memory/FPS were stable. Next gate is a focused heap-retention hotfix before any new `v1.2.0` publication or human retest.
