...
Track 06C Menu Broadcast V1 is approved and merged locally in `main` as merge commit `c14bf5a5`. The public Web baseline remains `v1.0.3+ef9c5baa` at `https://copa-arena-futebol.pages.dev/`; no 06C publication before 06E. Track 06D remains separate for its own merge.
...
Latest Track 06C branch result: PASS, 98 tests, 1699 asserts, including Web gzip transfer gate `30.43 MiB / 50.00 MiB`, source integrity check and UTF-8 BOM rejection.
