...
- Active stage: `Track 06E - Remote Gate Failed / Rolled Back`
- Active stage status: `BLOCKED_REMOTE_GATE - 06E merged locally as ea15d5dd and validate passed; public v1.1.0 attempt failed 5min remote gate and stable URL was rolled back to v1.0.3+ef9c5baa`
- Status marker: `JOGO_DA_COPA_TRACK_06E_REMOTE_GATE_FAILED_ROLLED_BACK`
- Approved plan: 06E was approved pre-merge and remained release-only/no gameplay. Publication remains governed by `../../08_Coordenacao_Agentes/Decisoes/2026-06-12_jogodacopa_publicacao-web-cloudflare.md`; after the remote gate failure, no human retest of `v1.1.0` should run before investigation.
...
Track 06E Release v1.1.0 is merged locally in `main` as merge commit `ea15d5dd`, but the public Web release is blocked. The `v1.1.0+ea15d5dd` attempt passed first-minute remote (`0` hitches, `0` runtime errors) and failed the remote 5min stability gate with `pageErrors=2` plus heap growth `19.43%`; Cloudflare Pages stable URL was rolled back to `v1.0.3+ef9c5baa`.
...
Latest Track 06E post-merge result: PASS, 101 tests, 1735 asserts, including source integrity check and UTF-8 BOM rejection.

Latest remote Web smoke: 06E first-minute `docs/playtest-reports/track-06e-data/06e-remote-first-minute-ea15d5dd.json` PASS, but 06E stability `docs/playtest-reports/track-06e-data/06e-remote-stability-5min-ea15d5dd.json` FAIL. Rollback verification `docs/playtest-reports/track-06e-data/06e-rollback-release-root-ef9c5baa.json` confirms stable URL release root `web/v1-copa-arena-futebol-20260612-ef9c5baa`, page errors `0`, runtime console errors `0`.
