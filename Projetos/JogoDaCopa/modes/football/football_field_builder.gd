...
static func build(parent: Node3D, config: Dictionary) -> void:
	...
	var wall_height: float = float(config.get("wall_height", 2.4))
	...
	_add_box(parent, "FootballPitch", Vector3(0.0, -0.5, 0.0), Vector3(field_width, 1.0, field_length), Color(0.08, 0.34, 0.16, 1.0))
	...
	_add_visual_box(parent, "NorthGoalBox", Vector3(0.0, 0.04, -17.7), Vector3(10.8, 0.05, 0.16), Color(0.92, 0.96, 0.86, 1.0))
	_add_visual_box(parent, "SouthGoalBox", Vector3(0.0, 0.04, 17.7), Vector3(10.8, 0.05, 0.16), Color(0.92, 0.96, 0.86, 1.0))
	...
	_add_box(parent, "NorthGoalFloor", Vector3(0.0, -0.5, goal_line_north - 1.35), Vector3(goal_half_width * 2.4, 1.0, 2.9), Color(0.07, 0.28, 0.14, 1.0))
	_add_box(parent, "SouthGoalFloor", Vector3(0.0, -0.5, goal_line_south + 1.35), Vector3(goal_half_width * 2.4, 1.0, 2.9), Color(0.07, 0.28, 0.14, 1.0))
	_add_goal_side_walls(parent, "North", goal_line_north - 1.35, goal_side_wall_x, goal_side_wall_thickness, goal_closed_depth, wall_height)
	_add_goal_side_walls(parent, "South", goal_line_south + 1.35, goal_side_wall_x, goal_side_wall_thickness, goal_closed_depth, wall_height)

	_add_box(parent, "WestWall", Vector3(-field_half_width, wall_height * 0.5, 0.0), Vector3(wall_thickness, wall_height, field_length), Color(0.12, 0.16, 0.18, 1.0))
	_add_box(parent, "EastWall", Vector3(field_half_width, wall_height * 0.5, 0.0), Vector3(wall_thickness, wall_height, field_length), Color(0.12, 0.16, 0.18, 1.0))
	_add_box(parent, "NorthWallLeft", Vector3(-10.2, wall_height * 0.5, goal_line_north), Vector3(11.6, wall_height, wall_thickness), Color(0.12, 0.16, 0.18, 1.0))
	_add_box(parent, "NorthWallRight", Vector3(10.2, wall_height * 0.5, goal_line_north), Vector3(11.6, wall_height, wall_thickness), Color(0.12, 0.16, 0.18, 1.0))
	_add_box(parent, "SouthWallLeft", Vector3(-10.2, wall_height * 0.5, goal_line_south), Vector3(11.6, wall_height, wall_thickness), Color(0.12, 0.16, 0.18, 1.0))
	_add_box(parent, "SouthWallRight", Vector3(10.2, wall_height * 0.5, goal_line_south), Vector3(11.6, wall_height, wall_thickness), Color(0.12, 0.16, 0.18, 1.0))
	_add_box(parent, "NorthBackStop", Vector3(0.0, 1.1, goal_line_north - 2.2), Vector3(10.5, 2.2, 0.45), Color(0.18, 0.2, 0.24, 1.0))
	_add_box(parent, "SouthBackStop", Vector3(0.0, 1.1, goal_line_south + 2.2), Vector3(10.5, 2.2, 0.45), Color(0.18, 0.2, 0.24, 1.0))
	...
	_add_stadium_bands(parent, goal_line_north, goal_line_south)
...
static func _add_goal_side_walls(parent: Node3D, prefix: String, goal_center_z: float, goal_side_wall_x: float, side_wall_thickness: float, goal_closed_depth: float, wall_height: float) -> void:
	...
	_add_box(parent, "%sGoalSideWallL" % prefix, Vector3(-goal_side_wall_x, wall_height * 0.5, goal_center_z), side_size, side_wall_color)
	_add_box(parent, "%sGoalSideWallR" % prefix, Vector3(goal_side_wall_x, wall_height * 0.5, goal_center_z), side_size, side_wall_color)
...
static func _add_stadium_bands(parent: Node3D, goal_line_north: float, goal_line_south: float) -> void:
	...
	for index in range(8):
		...
		_add_visual_box(parent, "NorthCrowdBand%d" % index, Vector3(x, 2.8, goal_line_north - 2.8), Vector3(2.4, 1.2, 0.18), color)
		_add_visual_box(parent, "SouthCrowdBand%d" % index, Vector3(x, 2.8, goal_line_south + 2.8), Vector3(2.4, 1.2, 0.18), color)

static func _add_box(parent: Node3D, node_name: String, node_position: Vector3, node_size: Vector3, color: Color) -> StaticBody3D:
