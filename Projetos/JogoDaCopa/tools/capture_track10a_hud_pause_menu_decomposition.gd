...
const OUTPUT_DIR: String = "res://docs/screenshots/track-10a-hud-pause-menu-decomposition-v1"
...
const SECTIONS: Array[Dictionary] = [
	{"id": &"controls", "slug": "controls"},
	{"id": &"audio", "slug": "audio"},
	{"id": &"video", "slug": "video"},
	{"id": &"sensitivity", "slug": "sensitivity"},
]
...
func _run() -> void:
	var exit_code := await _capture_track10a()
	quit(exit_code)


func _capture_track10a() -> int:
	if DisplayServer.get_name().to_lower().contains("headless"):
		printerr("[track10a-capture] run without --headless to render screenshots")
		return 1
	var scene_result: Dictionary = BootstrapSceneGeneratorScript.new().generate_all()
	if not bool(scene_result.get("ok", false)):
		printerr("[track10a-capture] scene generation failed: %s" % str(scene_result.get("message", "")))
		return 1
...
func _capture_size(capture_size: Vector2i, output_dir_absolute: String) -> void:
	...
	if football_scene == null:
		push_error("[track10a-capture] missing football scene")
		return
	...
	for section: Dictionary in SECTIONS:
		...
		var image := _save_capture(output_dir_absolute, "track10a-pause-%s-%dx%d.png" % [slug, capture_size.x, capture_size.y])
	...
	await _drain_frames(4)
	var confirm_image := _save_capture(output_dir_absolute, "track10a-pause-restart-confirm-%dx%d.png" % [capture_size.x, capture_size.y])
	_assert_capture_luminance(confirm_image, "restart-confirm", capture_size)
	football.queue_free()
	await _drain_frames(4)
...
func _save_capture(output_dir_absolute: String, file_name: String) -> Image:
	...
	print("[track10a-capture] %s" % output_path)
	return image
...
func _assert_capture_luminance(image: Image, section_slug: String, capture_size: Vector2i) -> void:
	...
	print("[track10a-capture] %s %dx%d luma=%.4f" % [section_slug, capture_size.x, capture_size.y, luminance])
	if luminance < MIN_CAPTURE_LUMINANCE:
		push_error("[track10a-capture] %s %dx%d luminance %.4f below %.4f" % [section_slug, capture_size.x, capture_size.y, luminance, MIN_CAPTURE_LUMINANCE])
