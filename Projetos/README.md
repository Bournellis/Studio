...
- `draxos-mobile/`: mobile-first Draxos PVE Arena-first async autobattler with Refugio/Base, later PVP, social systems and server-authoritative progression. Platforms: Android app, PC executable and PC browser. Backend alpha: Supabase, with Backend Proprio + Postgres as the preferred long-term exit path. The latest remote Internal Alpha package is `Bosque World Hub Domain Separation v1` from 2026-06-06. Release root: `internal-alpha/v0-bosque-world-hub-domain-separation-v1-20260606-81ecf05`; official Portal URL: `https://draxos-mobile-internal-alpha.pages.dev/`; direct Web URL: `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`; deployment evidence: `https://d1872010.draxos-mobile-internal-alpha.pages.dev`. The package bumps APK/manifest to `0.0.8-alpha.0` / version code `8`, separates Bosque local materials (`resto_ritual`, `po_cinzento`) from global account resources (`ossos`, `po_osso`), persists `fogueira_estavel_1` as both `upgrades` and `structures`, keeps active Bosque play client-owned/offline-first and preserves station craft server-authoritative for global potions. Bosque Fogueira Potion Crafting v1 remains the previous station-craft package (`internal-alpha/v0-bosque-fogueira-potion-crafting-v1-20260606-cad6d2c`, evidence `https://08d00f24.draxos-mobile-internal-alpha.pages.dev`). Bosque Durable Bau Mochila v1 remains the previous durable Openworld package. Arena PVE Menu Flow Simplification v1 remains the previous Arena menu package. Bosque Offline-First Checkpoint v1 remains the previous Openworld policy package. Track 13 release safety and Track 14 agent ops remain preserved operational baselines. Remote artifact smoke, remote Web launch smoke and read-only release/CORS smoke passed; preview loaded the game with release root matched and no runtime errors. Stable Portal/Web are Cloudflare Access protected. Android APK uses `debug_fallback`, accepted for playtest while release signing is deferred to broader Android distribution.
  - Priority/status: `P2_IMPLEMENTACAO - BOSQUE_WORLD_HUB_DOMAIN_SEPARATION_V1_PUBLISHED_INTERNAL_ALPHA`
  ...
  - Current next step: playtest Bosque World Hub Domain Separation v1 from updated `main`, focusing local material labels, global `ossos`/`po_osso` separation, durable Fogueira after relog, station craft material sources and Arena potion regression. Preserve Arena PVE Menu Flow Simplification v1 regression coverage and do not reintroduce revisioned microaction sync as the normal Bosque loop without a new decision.
