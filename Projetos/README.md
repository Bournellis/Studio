...
- `draxos-mobile/`: mobile-first Draxos PVE Arena-first async autobattler with Refugio/Base, later PVP, social systems and server-authoritative progression. Platforms: Android app, PC executable and PC browser. Backend alpha: Supabase, with Backend Proprio + Postgres as the preferred long-term exit path. The latest remote Internal Alpha package is `Bosque Mecanico Basico v2` from 2026-06-04. Release root: `internal-alpha/v0-bosque-v2-guidance-20260604-7c2d981`; official Portal URL: `https://draxos-mobile-internal-alpha.pages.dev/`; direct Web URL: `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`; deployment evidence: `https://ae049df9.draxos-mobile-internal-alpha.pages.dev`. The package delivers optional server-persisted Bosque guidance, fixed resource slack, `Encerrar visita` without mandatory objective and procedural blocking `fogueira_estavel_1`. `supabase db push`, `supabase functions deploy modes`, Storage upload, Cloudflare Pages, release manifest and RemoteReadOnly passed. Remote Chrome/CDP smoke loaded the game in 3699 ms and captured `build/diagnostics/web-launch-remote-20260604-194152/web-launch-remote.png`; anonymous production fixed-domain reads can return Cloudflare Access, as expected. Android APK uses `debug_fallback`, accepted for playtest while release signing is deferred to broader Android distribution. First Access Runtime Fix `4608977`, Integrated Runtime Fix `ab5834c`, Integrated App/Arena/Bosque `99304ed`, Web Launch Resilience, Refugio Visual Cleanup, Openworld QoL regression fix, Foundation Hardening V2 and Hardening Platform V1 remain previous baselines. Compatibility guard marker: Foundation Hardening V2 is the latest remote Internal Alpha baseline for hardening/live-doc gates, release root `internal-alpha/v0-foundation-hardening-v2-hotfix2-20260601-58671a4`, preview `https://ca946749.draxos-mobile-internal-alpha.pages.dev`. Track 13 release safety and Track 14 agent ops remain preserved baselines. Track 21 Arena Loop Unlock/Friction remains the Autobattler/Arena PVE context; Web Battle Lab/Progression Lab remain served by Edge Function `lab-runner` behind the same Supabase email/password alpha account gate.
  ...
  - Current local implementation package: `Main Menu Refactor` on `codex/draxos-mobile/main-menu-refactor`, validated locally and not published; removes player-facing Mode Hub, collect-all, direct energy and dev Openworld shortcuts, keeps `Bosque` as direct entry, moves Preparacao into Arena PVE and keeps Energy purchase inside Loja.
