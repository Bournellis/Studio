...
- `draxos-mobile/`: jogo mobile multi-plataforma - mago Draxos (PVP assincrono, base manager, social). Plataformas: Android + PC executavel + PC browser. Backend: Supabase. Batalha 100% simulada no servidor. Track 00 completa, Track 01 completa e Track 02 com Progression Lab/Battle Lab v1 implementados; batalha visual procedural 2D e labs dev-only estao prontos para a primeira rodada de teste. Track 03 foi preparada em 2026-05-26 para Internal Alpha v0: email/senha, dois saves por conta (`normal` e `progression_lab`), Supabase remoto Free, Progression Lab isolado, Base/Social/Competicao/Loja jogaveis, leaderboard basica, redeems alpha, manifest de updates e playtest fechado Fabio + 1 amigo.
  - Priority/status: `P2_IMPLEMENTACAO - internal alpha v0 ready for implementation`
  ...
  - Current next step: executar o design lock da Track 03 (`DMOB-D048` a `DMOB-D055`) e iniciar Supabase remoto/email-senha/dois saves.
