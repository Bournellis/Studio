...
- `draxos-mobile/`: mobile-first Draxos PVE Arena-first async autobattler with Refugio/Base, later PVP, social systems and server-authoritative progression. Platforms: Android app, PC executable and PC browser. Backend alpha: Supabase, with Backend Proprio + Postgres as the preferred long-term exit path. The latest remote Internal Alpha package is the first access runtime fix from 2026-06-02. Release root: `internal-alpha/v0-first-access-runtime-20260602-4608977`; official Portal URL: `https://draxos-mobile-internal-alpha.pages.dev/`; direct Web URL: `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`; deployment evidence: `https://36db2742.draxos-mobile-internal-alpha.pages.dev`. The latest local implementation package is `Bosque Mecanico Basico v2` on branch `codex/draxos-mobile/bosque-v2-guidance`, not published, with optional server-persisted guidance, fixed resource slack, `Encerrar visita` without mandatory objective and procedural blocking `fogueira_estavel_1`. It preserves the integrated runtime fix package and adds immediate local shell rendering for no-cache first access, Arena first-access sync shell without dev fallback actions before the server response, repaired local Edge/DatabaseLocal proof and Mode live test coverage aligned to active Bosque v1. Storage upload, Cloudflare Pages, release manifest and RemoteReadOnly passed. Remote Chrome/CDP smoke loaded the game in 4946 ms and captured `build/diagnostics/web-launch-remote-20260603-000139/web-launch-remote.png`; anonymous production fixed-domain reads can return Cloudflare Access, as expected. Android APK uses `debug_fallback`, accepted for playtest while release signing is deferred to broader Android distribution. Integrated Runtime Fix `ab5834c`, Integrated App/Arena/Bosque `99304ed`, Web Launch Resilience, Refugio Visual Cleanup, Openworld QoL regression fix, Foundation Hardening V2 and Hardening Platform V1 remain previous baselines. Compatibility guard marker: Foundation Hardening V2 is the latest remote Internal Alpha baseline for hardening/live-doc gates, release root `internal-alpha/v0-foundation-hardening-v2-hotfix2-20260601-58671a4`, preview `https://ca946749.draxos-mobile-internal-alpha.pages.dev`; actual latest published package is the first access runtime fix above. Track 13 release safety and Track 14 agent ops remain preserved baselines. Track 21 Arena Loop Unlock/Friction remains the Autobattler/Arena PVE context; Web Battle Lab/Progression Lab remain served by Edge Function `lab-runner` behind the same Supabase email/password alpha account gate.
  - Priority/status: `P2_IMPLEMENTACAO - FIRST_ACCESS_RUNTIME_PUBLISHED_INTERNAL_ALPHA`
  ...
  - Current next step: human playtest of the local Bosque Mecanico Basico v2 package: free entry/exit, guidance, collect/deposit/craft, `Voltar`, `Encerrar visita`, guidance persistence and campfire visual/collision; remote package remains first access runtime until publication is explicitly approved.
