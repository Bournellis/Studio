...
- `draxos-mobile/`: mobile-first Draxos PVE Arena-first async autobattler with Refugio/Base, later PVP, social systems and server-authoritative progression. Platforms: Android app, PC executable and PC browser. Backend alpha: Supabase, with Backend Proprio + Postgres as the preferred long-term exit path. The latest remote Internal Alpha package is `Bosque Fogueira Potion Crafting v1` from 2026-06-06. Release root: `internal-alpha/v0-bosque-fogueira-potion-crafting-v1-20260606-cad6d2c`; official Portal URL: `https://draxos-mobile-internal-alpha.pages.dev/`; direct Web URL: `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`; deployment evidence: `https://08d00f24.draxos-mobile-internal-alpha.pages.dev`. The package bumps APK/manifest to `0.0.7-alpha.0` / version code `7`, turns durable `Fogueira Estavel I` into a server-authoritative station for global potions, consumes Bosque chest materials plus account `po_osso`, preserves durable `Bau`/`Mochila/Bolso`/structures and keeps active Bosque play client-owned/offline-first. Bosque Durable Bau Mochila v1 remains the previous durable Openworld package (`internal-alpha/v0-bosque-durable-bau-mochila-v1-20260606-6e7ca6b`, evidence `https://39198a35.draxos-mobile-internal-alpha.pages.dev`). Arena PVE Menu Flow Simplification v1 remains the previous Arena menu package (`internal-alpha/v0-arena-pve-menu-flow-simplification-v1-20260606-5d03a68`, evidence `https://fdf44707.draxos-mobile-internal-alpha.pages.dev`). Bosque Offline-First Checkpoint v1 remains the previous Openworld policy package (`internal-alpha/v0-bosque-offline-first-checkpoint-v1-20260606-f649d22`, evidence `https://fa84e109.draxos-mobile-internal-alpha.pages.dev`). Bosque Sync Responsiveness v1 remains the previous Bosque sync package (`internal-alpha/v0-bosque-sync-responsiveness-v1-20260605-a5f8c95`, evidence `https://60e2d4be.draxos-mobile-internal-alpha.pages.dev`). Arena/Bosque Visible V2 remains the previous visible package (`internal-alpha/v0-arena-bosque-visible-v2-20260605-01d80d5`, evidence `https://7b9c8f38.draxos-mobile-internal-alpha.pages.dev`). Arena/Bosque Regression Hotfix, Arena PVE Season 1 Loop v1, Arena Duel Flow Hotfix, Arena PVE First Real Run + Update Recovery, Bosque v3 UX/Feel, Technical Hardening and Openworld Main Menu Sync remain preserved previous packages. Export, public Storage upload, Cloudflare Pages production branch `main`, release manifest, Edge Functions deploy, remote artifacts smoke, remote Web launch smoke and read-only release/CORS smoke passed; preview loaded the game with release root matched and no runtime errors. Stable Portal/Web are Cloudflare Access protected. Android APK uses `debug_fallback`, accepted for playtest while release signing is deferred to broader Android distribution. Track 13 release safety and Track 14 agent ops remain preserved baselines.
  - Priority/status: `P2_IMPLEMENTACAO - BOSQUE_FOGUEIRA_POTION_CRAFTING_V1_PUBLISHED_INTERNAL_ALPHA`
