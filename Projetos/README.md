...
- `draxos-mobile/`: mobile-first Draxos PVE Arena-first async autobattler with Refugio/Base, later PVP, social systems and server-authoritative progression. Platforms: Android app, PC executable and PC browser. Backend alpha: Supabase, with Backend Proprio + Postgres as the preferred long-term exit path. The latest remote Internal Alpha package is the integrated runtime fix from 2026-06-02. Release root: `internal-alpha/v0-integrated-runtime-fix-20260602-ab5834c`; official Portal URL: `https://draxos-mobile-internal-alpha.pages.dev/`; direct Web URL: `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`; deployment evidence: `https://888320f4.draxos-mobile-internal-alpha.pages.dev`. It corrects the integrated App/Arena/Bosque package with Bosque revision/event serialization, server-confirmed Bosque mutations, common `/modes/session/event` envelopes, Arena replay metadata and live Bosque policy compatibility, with Supabase migration, Edge Functions, Storage upload, Cloudflare Pages and release manifest updated. Remote Chrome/CDP smoke loaded the game in 5838 ms and captured `build/diagnostics/web-launch-remote-20260602-223337/web-launch-remote.png`; release artifacts smoke, release manifest smoke and integrated internal alpha remote smoke with Arena/Bosque passed; anonymous production fixed-domain reads can return Cloudflare Access, as expected. Android APK uses `debug_fallback`, accepted for playtest while release signing is deferred to broader Android distribution. Integrated App/Arena/Bosque `99304ed`, Web Launch Resilience, Refugio Visual Cleanup, Openworld QoL regression fix, Foundation Hardening V2 and Hardening Platform V1 remain previous baselines. Compatibility guard marker: Foundation Hardening V2 is the latest remote Internal Alpha baseline for hardening/live-doc gates, release root `internal-alpha/v0-foundation-hardening-v2-hotfix2-20260601-58671a4`, preview `https://ca946749.draxos-mobile-internal-alpha.pages.dev`; actual latest published package is the integrated runtime fix above. Track 13 release safety and Track 14 agent ops remain preserved baselines. Track 21 Arena Loop Unlock/Friction remains the Autobattler/Arena PVE context; Web Battle Lab/Progression Lab remain served by Edge Function `lab-runner` behind the same Supabase email/password alpha account gate.
  - Priority/status: `P2_IMPLEMENTACAO - INTEGRATED_RUNTIME_FIX_PUBLISHED_INTERNAL_ALPHA`
  ...
  - Current next step: human playtest of the integrated runtime fix package: login/cache refresh, first Arena real loop, Arena replay/reward and online Bosque start/event/deposit/complete.
