...
- `FpsShooter/`: independent PC Windows editor-first Godot 4.6.2 first-person shooter tech probe with a light Draxos visual theme. The first contract is a traditional FPS baseline in a local 1x1 arena against a bot. Track 01D Knockback Movement Combat V1 is complete on top of the feel/feedback, bot and arena layout baselines: `Duel Pit V1` map, protected spawns, central blocker, low/high cover, side platforms, ramp primitives, route markings, simple hitscan rifle, HUD combat feedback, synthetic audio, bot states, strafe/reposition, line-of-sight gated normal shots, exposed camera/head checks over low cover, tall-cover denial, deterministic aim error and knockback with horizontal force, controlled lift, clamps and air/ground decay.
  - Priority/status: `P2_IMPLEMENTACAO - FPS_SHOOTER_TRACK_01D_KNOCKBACK_MOVEMENT_COMBAT_COMPLETE`
  ...
  - Current next step: run the 3-5 minute editor human smoke for hit/received knockback and ramp/platform movement in `Duel Pit V1`, then select the next Track 01 focus: future weapon/projectile variants or first hazard/verticality expansion.
