...
- `draxos-mobile/`: mobile-first Draxos PVE Arena-first async autobattler with Refugio/Base, later PVP, social systems and server-authoritative progression. Platforms: Android app, PC executable and PC browser. Backend alpha: Supabase, with Backend Proprio + Postgres as the preferred long-term exit path. Track 13 release safety and Track 14 agent ops remain preserved baselines. First Session Clarity v1 is approved, and Foundation Closeout is delivered over Foundation Expansion Readiness: account/save authority, immutable ruleset publication registry, persisted replay hashes, API v1 header, idempotency v1, internal auditable admin RPCs, client retry registry, ActionRouter/OperationState in the real shell path, minigame placeholder only and transactional RPC promotion including build behavior/potion and social friend/chat. Track 20 Season 1 Arena Calibration is the latest local package over the remote Remote Lab Runner publication: S1 arena matrix with 27 tiers, generated PVE catalog, Battle/Progression Labs reading real tier data, backend/client start by `arena_id + difficulty_id`, rewards by tier first-clear/repeat, local package in `build/internal-alpha/publish`, and Web Battle Lab/Progression Lab via Edge Function `lab-runner` protected by the same Supabase email/password alpha account gate.
  - Priority/status: `P2_IMPLEMENTACAO - S1_ARENA_CALIBRATION_LOCAL_PACKAGE_READY`
  - Canonical local branch for Arena PVE follow-up: `codex/draxos-mobile/s1-arena-calibration-integration`; remote published baseline remains `codex/draxos-mobile/remote-lab-runner`.
  ...
  - Current next step: human playtest tutorial, 3-duel multidifficulty flow, potion consumption, repeat rewards and Web Battle Lab/Progression Lab generation from the Track 20 local package; remote publication requires explicit approval.
