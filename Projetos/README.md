...
- `FpsShooter/`: independent PC Windows editor-first Godot 4.6.2 first-person shooter tech probe with a light Draxos visual theme. The first contract is a traditional FPS baseline in a local 1x1 arena against a bot. Track 02A Combat Loop Expansion V1 is complete on top of the feel/feedback, bot, arena layout and knockback baselines: `Duel Pit V1` map, protected spawns, central blocker, low/high cover, side platforms, ramp primitives, route markings, simple hitscan rifle, RMB Plasma Bolt, Health Shard, Overcharge, HUD combat/pickup feedback, synthetic audio, bot states, strafe/reposition, line-of-sight gated normal shots, exposed camera/head checks over low cover, tall-cover denial, deterministic aim error, health seeking, overcharge contest, plasma dodge and knockback with horizontal force, controlled lift, clamps and air/ground decay.
  - Priority/status: `P2_IMPLEMENTACAO - FPS_SHOOTER_TRACK_02A_COMBAT_LOOP_EXPANSION_COMPLETE`
  ...
  - Current next step: run the 5-minute editor human smoke for rifle vs Plasma Bolt, pickups, overcharge, bot pickup contest/dodge and fairness of the expanded loop, then decide between tuning weapons/resources or first verticality/hazard expansion.
