...
- `draxos-mobile/`: mobile-first Draxos PVE Arena-first async autobattler with Refugio/Base, later PVP, social systems and server-authoritative progression. Platforms: Android app, PC executable and PC browser. Backend alpha: Supabase, with Backend Proprio + Postgres as the preferred long-term exit path. The latest remote Internal Alpha package is the integrated App/Arena/Bosque publication from 2026-06-02. Release root: `internal-alpha/v0-integrated-app-arena-bosque-20260602-99304ed`; official Portal URL: `https://draxos-mobile-internal-alpha.pages.dev/`; direct Web URL: `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`; deployment evidence: `https://8f2829c0.draxos-mobile-internal-alpha.pages.dev`. It publishes App Responsiveness Architecture Pass, Arena Loop Simplification/Feedback and Openworld Bosque Hardening V1 together, with Supabase migration, Edge Functions, Storage upload and Cloudflare Pages updated; the release manifest was republished with `portal_url` at the official root (`released_at=2026-06-02T20:53:32Z`). Remote Chrome/CDP smoke loaded the game in 6737 ms and captured `build/diagnostics/web-launch-remote-20260602-202832/web-launch-remote.png`; release artifacts smoke, release manifest smoke and lightweight internal alpha remote smoke passed; anonymous production fixed-domain reads can return Cloudflare Access, as expected. `index.pck` (`4660188`) and `index.wasm` (`37695054`) match remote `Content-Length`. Android APK uses `debug_fallback`, accepted for playtest while release signing is deferred to broader Android distribution. Web Launch Resilience, Refugio Visual Cleanup, Openworld QoL regression fix, Foundation Hardening V2 and Hardening Platform V1 remain previous baselines. Compatibility guard marker: Foundation Hardening V2 is the latest remote Internal Alpha baseline for hardening/live-doc gates, release root `internal-alpha/v0-foundation-hardening-v2-hotfix2-20260601-58671a4`, preview `https://ca946749.draxos-mobile-internal-alpha.pages.dev`; actual latest published package is the integrated App/Arena/Bosque release above. Track 13 release safety and Track 14 agent ops remain preserved baselines. Track 21 Arena Loop Unlock/Friction remains the Autobattler/Arena PVE context; Web Battle Lab/Progression Lab remain served by Edge Function `lab-runner` behind the same Supabase email/password alpha account gate.
  - Priority/status: `P2_IMPLEMENTACAO - INTEGRATED_APP_ARENA_BOSQUE_PUBLISHED_INTERNAL_ALPHA`
  ...
  - Current next step: human playtest of the integrated package: login/cache refresh, first Arena real loop, next difficulty unlock and online Bosque start/event/deposit/complete.
