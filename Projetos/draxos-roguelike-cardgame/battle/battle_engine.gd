...
# Forked from Projetos/rpg-turnos as bootstrap scaffolding.
# Track 00 must simplify this for Draxos Roguelike Cardgame: no routes,
# terrain, elevation, neutral slots, or RPG Turnos-specific deck economy.
...
func start_battle(catalog, deck_ids: Array, config: Dictionary = {}) -> void:
	...
	encounter_id = str(encounter.get("id", "pouso_elemental"))
	encounter_name = str(encounter.get("display_name", "Pouso Elemental"))
