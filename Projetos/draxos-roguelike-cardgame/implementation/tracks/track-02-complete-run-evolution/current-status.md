...
Completed prompt: `REWARD-CARD-REDESIGN-BATCH-01-USING-V4 - run Card Impact V4 before/change/after/compare for a coherent reward-card redesign batch and accept only the intended card/effect movement`.

Next implementation prompt: `REWARD-CARD-REDESIGN-BATCH-02-UTILITY-USING-V4 - run a second V4 reward-card batch focused on utility/card-flow/AP effects so temporary ability power and other utility deltas are exercised by real card movement`. Track 02 remains ready for user playtest.
