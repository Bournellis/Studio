...
- Active operational stage: `Arena/Bosque Visible V2`
- Active stage status: `PUBLISHED_INTERNAL_ALPHA`
- Latest remote Internal Alpha package: `Arena/Bosque Visible V2`,
...
- Immediate product gate: Arena/Bosque Visible V2 is the latest remote Internal Alpha publication for human playtest. It preserves Arena/Bosque Regression Hotfix, then bumps the APK/manifest to `0.0.2-alpha.0` / version code `2` and restores Preparacao even when Arena selection is blocked by an active or stuck attempt. Arena/Bosque Regression Hotfix remains the previous visibility hotfix package for Preparacao before Arena start, during active attempts and in pending buff choice, plus Bosque deposit/craft feedback and pending-event flush before leaving integrated sessions. Arena PVE Season 1 Loop v1 remains the previous Season 1 package for grouped arenas/difficulties, progress, next-step guidance, reward preview and remote pending-buff recovery through `/arena/pve/state`. Arena Duel Flow Hotfix remains the previous duel-flow hotfix package for Preparacao/behavior in the active-duel menu and selected buff -> `Resolver duelo`. Foundation Hardening V2 remains the previous hardening/live-doc enforcement baseline. Track 21 remains preserved Autobattler/Arena PVE context for no combat cooldown, locked loadout on start, temporary stat buffs, HP reset per duel, live-stock potion consumption in Arena, summary-only claim, public buff select endpoint, data-driven Arena list, Season 1 tier matrix, tutorial XP -> level recalculation, direct start into active Arena and continue-in-Arena summary flow.
