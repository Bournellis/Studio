...
Track 19 Arena Consistency Pass is the latest remote Internal Alpha publication. New DraxosMobile agents should branch from `codex/draxos-mobile/arena-consistency-pass` for Arena PVE follow-up work, or from `codex/draxos-mobile/pve-arena-integration` only if explicitly returning to the Track 18 baseline. Latest published preview: `https://168dc669.draxos-mobile-internal-alpha.pages.dev/web/index.html`; release root: `internal-alpha/v0-arena-consistency-pass-20260531-0865e43`. Do not change gameplay tuning, Supabase APIs, economy, content, weapons, spells, potions, crafting, advanced behavior, final visuals or authoritative flows beyond the Arena PVE consistency package without an explicit package decision.
