...
- Active operational stage: `Track 21 - Arena Loop Unlock And Friction Pass`
...
- Immediate product gate: Track 20 Season 1 Arena Calibration is the latest remote Internal Alpha publication. Track 21 is the latest local package: Arena PVE first, PVP later, no combat cooldown, locked loadout on start, temporary stat buffs, HP reset per duel, live-stock potion consumption in Arena, summary-only claim, public buff select endpoint, data-driven Arena list, Season 1 tier matrix, tutorial XP -> level recalculation, direct start into active Arena and continue-in-Arena summary flow. Remote deployment still requires explicit approval and `-ConfirmRemoteMutation`.
...
Track 21 Arena Loop Unlock And Friction Pass is the latest implemented local package on `codex/draxos-mobile/track21-arena-loop-unlock-friction`. Track 20 Season 1 Arena Calibration remains the latest remote Internal Alpha publication until Track 21 is deployed. New DraxosMobile agents should branch from Track 21 for Arena PVE follow-up work, or from `codex/draxos-mobile/s1-arena-calibration-integration` only if explicitly returning to the pre-hotfix remote baseline. Latest remote preview remains `https://c20c0ff3.draxos-mobile-internal-alpha.pages.dev/web/index.html`; local Track 21 package root is `internal-alpha/v0-track21-arena-loop-20260531-local`. Do not change gameplay tuning, Supabase APIs, economy, content, weapons, spells, potions, crafting, advanced behavior, final visuals or authoritative flows beyond the Arena loop unlock/friction package without an explicit package decision.
