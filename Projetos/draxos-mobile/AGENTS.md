...
- Immediate product gate: Foundation Closeout and Lab Track 16 Alignment are delivered; Foundation Final Polish is implemented, Full-gated and published to Internal Alpha. Track 19 is the latest product baseline, and the latest remote Internal Alpha package is Remote Lab Runner on `codex/draxos-mobile/remote-lab-runner`: Arena PVE first, PVP later, no combat cooldown, locked loadout, temporary stat buffs, HP reset per duel, live-stock potion consumption in Arena, summary-only claim, public buff select endpoint, data-driven Arena list, Arena-focused labs and Web Battle Lab/Progression Lab via Edge Function `lab-runner` protected by the same Supabase email/password alpha account gate. Foundation Loop UX Pass 01 and First Session Clarity v1 are accepted baselines.
...
Remote Lab Runner is the latest remote Internal Alpha publication on top of Track 19 Arena Consistency Pass. New DraxosMobile agents should branch from `codex/draxos-mobile/remote-lab-runner` for Arena PVE follow-up work, or from `codex/draxos-mobile/arena-consistency-pass` only if explicitly returning to the Track 19 pre-hotfix baseline. Latest published preview: `https://9ae1e953.draxos-mobile-internal-alpha.pages.dev/web/index.html`; release root: `internal-alpha/v0-remote-lab-runner-20260531-e659d7e5`. Do not change gameplay tuning, Supabase APIs, economy, content, weapons, spells, potions, crafting, advanced behavior, final visuals or authoritative flows beyond the Arena PVE consistency/hotfix package without an explicit package decision.
