...
- Immediate product gate: Foundation Closeout and Lab Track 16 Alignment are delivered; Foundation Final Polish is implemented, Full-gated and published to Internal Alpha. Track 19 is the latest product baseline, and the latest remote Internal Alpha package is the Lab Web Export Guard hotfix on `codex/draxos-mobile/lab-web-export-guard`: Arena PVE first, PVP later, no combat cooldown, locked loadout, temporary stat buffs, HP reset per duel, live-stock potion consumption in Arena, summary-only claim, public buff select endpoint, data-driven Arena list, Arena-focused labs and Web-export guards that prevent Labs Dev from trying to execute `npx/deno` in the browser. Foundation Loop UX Pass 01 and First Session Clarity v1 are accepted baselines.
...
Lab Web Export Guard is the latest remote Internal Alpha publication on top of Track 19 Arena Consistency Pass. New DraxosMobile agents should branch from `codex/draxos-mobile/lab-web-export-guard` for Arena PVE follow-up work, or from `codex/draxos-mobile/arena-consistency-pass` only if explicitly returning to the Track 19 pre-hotfix baseline. Latest published preview: `https://fc60138d.draxos-mobile-internal-alpha.pages.dev/web/index.html`; release root: `internal-alpha/v0-lab-web-export-guard-20260531-9a415c3`. Do not change gameplay tuning, Supabase APIs, economy, content, weapons, spells, potions, crafting, advanced behavior, final visuals or authoritative flows beyond the Arena PVE consistency/hotfix package without an explicit package decision.
