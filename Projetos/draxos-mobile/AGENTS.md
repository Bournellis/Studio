...
Foundation Hardening V2 is the latest remote Internal Alpha publication on `codex/draxos-mobile/foundation-hardening-v2`. New DraxosMobile agents should branch from updated `master` after this package is merged, use a dedicated worktree and follow `docs/multi-agent-workflow.md`. Latest remote preview: `https://4315dd54.draxos-mobile-internal-alpha.pages.dev`; release root: `internal-alpha/v0-foundation-hardening-v2-hotfix1-20260601-f8ff795`. Track 21 remains the preserved Arena PVE/Autobattler context. Do not change gameplay tuning, Supabase APIs, economy, content, weapons, spells, potions, crafting, advanced behavior, final visuals or authoritative flows without an explicit package decision.
