...
- Active operational stage: `Bosque Overlay Interactive Controls Authority v1`
- Active stage status: `BOSQUE_OVERLAY_INTERACTIVE_CONTROLS_AUTHORITY_V1_PUBLISHED_INTERNAL_ALPHA`
- Latest remote Internal Alpha package: `Bosque Overlay Interactive Controls Authority v1`,
...
- Build version: `0.0.21-alpha.0` / version code `21`; minimum supported version code `13`.
...
Bosque Overlay Interactive Controls Authority v1 is the latest remote Internal Alpha publication. Initial human playtest was reported OK by Fabio on `2026-06-09`; the previous menu-action package still left Social, Shop and Arena controls partially inert inside the overlay, so this package adds overlay LineEdit focus/text input, overlay-owned confirmations, stable embedded Arena/Shop button metadata, late-refresh guards and remote interactive smoke coverage for Social typing, Shop cancel/confirm, Arena resume/abandon, `Fechar`, `Voltar` and Esc. New DraxosMobile agents should branch from updated `main`, use a dedicated worktree and follow `docs/multi-agent-workflow.md`. Latest remote preview evidence: `https://9461e4be.draxos-mobile-internal-alpha.pages.dev`; release root: `internal-alpha/v0-bosque-overlay-interactive-controls-authority-v1-20260609-d3be1fb`; official URL remains `https://draxos-mobile-internal-alpha.pages.dev/`; direct Web URL remains `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`. APK: `https://armxgipvnbbshzqawklw.supabase.co/functions/v1/release/download?artifact=android`. PC ZIP: `https://armxgipvnbbshzqawklw.supabase.co/functions/v1/release/download?artifact=pc_windows`.
