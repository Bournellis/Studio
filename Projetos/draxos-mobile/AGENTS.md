...
Track 21 Arena Loop Unlock And Friction Pass is the latest remote Internal Alpha publication on `codex/draxos-mobile/track21-arena-loop-unlock-friction`. New DraxosMobile agents should branch from Track 21 for Arena PVE follow-up work, or from `codex/draxos-mobile/s1-arena-calibration-integration` only if explicitly returning to the pre-hotfix Track 20 baseline. Latest remote preview: `https://2adcfa6b.draxos-mobile-internal-alpha.pages.dev`; release root: `internal-alpha/v0-track21-arena-loop-20260531-df9f12d`. Do not change gameplay tuning, Supabase APIs, economy, content, weapons, spells, potions, crafting, advanced behavior, final visuals or authoritative flows beyond the Arena loop unlock/friction package without an explicit package decision.
