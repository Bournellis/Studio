...
- primeiros modulos de dominio portavel: `_shared/base_domain.ts` concentra regras/projecao puras da Base, `_shared/battle_log_projection.ts` concentra projecao de `battle_log_v1`, historico e metadata de ruleset sem rerodar simulador, `_shared/progression_domain.ts` concentra payload de build, unlocks, validacao de equipamento, power runtime e helpers de battle usados por build/battle, e `_shared/economy_domain.ts` concentra rewards, produtos alpha, deltas source/sink e payloads de crafting/monetization;
