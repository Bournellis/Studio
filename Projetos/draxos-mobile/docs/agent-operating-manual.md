...
Active stage: `BOSQUE_OVERLAY_MENU_ACTION_AUTHORITY_V1_PUBLISHED_INTERNAL_ALPHA`.
Previous hardening baseline marker: `FOUNDATION_HARDENING_V2_PUBLISHED_INTERNAL_ALPHA`.

The project is a base implemented for refinement. Bosque Overlay Menu Action Authority v1 is the latest remote Internal Alpha publication: release root `internal-alpha/v0-bosque-overlay-menu-action-authority-v1-20260609-aa9402d`, official Portal URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`, preview evidence `https://5f04e6ae.draxos-mobile-internal-alpha.pages.dev`, APK/manifest `0.0.20-alpha.0` / version code `20`, minimum supported version code `13`. It publishes the persistent overlay shell with a single interaction authority: five Bosque landmarks open Arena PVE, Refugio/Base, Shop, Social and Profile above the live Bosque, input is paused behind the sheet, `Fechar`/`Voltar`/Esc close through the same path, read-only refresh does not block close, late refresh responses are ignored after close/route change, and closing returns focus to the same Bosque node without rebootstrap.

Initial human playtest of the previous Bosque Bootstrap Authority v1 package was reported OK by Fabio on `2026-06-09`: everything tested at that point appeared to work. The current operational next step is focused human playtest of Bosque Overlay Menu Action Authority v1 on Web/APK, especially landmark prompts/actions, menu routing and `Fechar`, `Voltar` and Esc returning to the Bosque. Future bugs return to the normal bugfix flow if they appear.
