...
- A live doc must distinguish operational publication from product direction: `BOSQUE_V3_UX_FEEL_PUBLISHED_INTERNAL_ALPHA` is the current published package, while Technical Hardening remains the previous technical package, Openworld Main Menu Sync remains the previous content package and Arena PVE remains the approved early-game direction after the published package is reviewed.
