...
- A live doc must distinguish operational publication from product direction: `ARENA_PVE_FIRST_REAL_RUN_PUBLISHED_INTERNAL_ALPHA` is the current published package, Bosque v3 UX/Feel remains the previous content/polish package, Technical Hardening remains the previous technical package, Openworld Main Menu Sync remains the previous Openworld content package and Arena PVE remains the approved early-game direction after the published package is reviewed.
