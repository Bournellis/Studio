...
- A live doc may treat `docs/pve-arena-v1.md` as the current contract/data package for Arena PVE implementation, while values marked `CALIBRAVEL_ALPHA` still require labs and human playthrough. Track 18 local integration is not a remote publication until `publish_internal_alpha.ps1` package/upload/deploy is explicitly approved and recorded.
