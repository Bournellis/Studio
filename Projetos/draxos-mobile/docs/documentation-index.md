...
- A live doc must distinguish operational publication from product direction: Bosque Feel & Spawn Authority v1 is the current remote Internal Alpha package, Bosque Persistence Rebase v1 remains the previous persistence/operations package, Bosque Session Lifecycle & Durable Structures Hotfix v1 remains the previous session lifecycle package, Bosque World Hub Domain Separation v1 remains the previous local/account domain package, Bosque Offline-First Checkpoint v1 remains the previous Openworld/Bosque policy package, Bosque Sync Responsiveness v1 remains the previous Bosque sync package, Arena PVE Menu Flow Simplification v1 remains the previous Arena menu package, Arena/Bosque Visible V2 remains the previous visible package, Arena/Bosque Regression Hotfix remains the previous visibility hotfix package, Arena PVE Season 1 Loop v1 remains the previous Season 1 package, Arena PVE remains the approved early-game direction, and Openworld/Bosque currently uses server-authoritative durable operations with client preview/retry, node cooldowns, server-owned durable progress and a client-owned feel layer for sticky collection, local node visibility and non-blocking menu actions.
