...
- A live doc must distinguish operational publication from product direction: `TECHNICAL_HARDENING_PUBLISHED_INTERNAL_ALPHA` is the current published package, while Openworld Main Menu Sync remains the previous content package and Arena PVE remains the approved early-game direction after the published package is reviewed.
