...
O estado operacional atual, pacote publicado, release root, evidencia, URLs,
versoes e proximo gate vivem em `../implementation/current-status.md`. A
linhagem de pacotes e endpoints estaveis vive em `release-history.md`. Esta
visao de produto nao lista estado operacional atual.
...
direcao: Arena PVE segue primeiro core; Bosque/Openworld e slice integrado de
Internal Alpha, nao autorizacao para expansao ampla de mundo, economia, conteudo
ou visual final.
...
O projeto ja tem uma base implementada com substancia suficiente para nao
parecer um app vazio. Foundation Closeout e Lab Track 16 Alignment anteciparam a
fundacao de producao futura: `account_profiles/game_saves`, registry imutavel de
ruleset, idempotencia com `request_hash`, admin minimo auditavel, shell/retry
client e labs alinhados ao estado de pocoes/comportamento/crafting. Foundation
Final Polish, Hardening Platform V1 e Foundation Hardening V2 ficam como
baselines tecnicas/historicas preservadas; a linhagem operacional posterior fica
em `release-history.md`.
