...
- review_when: `only for link or encoding repair`
- supersedes: `none`
- superseded_by: `../documentation-index.md`

Preserved as the full pre-cutover classification map; it is no longer a live router.
...
| `docs/release-history.md` | `HISTORICO` | Single package history, release roots, preview evidence, stable endpoints and download paths. |
...
- A live doc must distinguish operational publication from product direction without
  duplicating operational state. Current package name, release root, preview URL,
  stable URLs, version code, minimum supported version code and next step live in
  `implementation/current-status.md`; package lineage lives in
  `docs/release-history.md`. Arena PVE remains the approved early-game
  direction; Openworld/Bosque remains an integrated Internal Alpha slice, not
  approval for broad expansion.
