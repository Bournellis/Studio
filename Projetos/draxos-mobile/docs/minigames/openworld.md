...
Oficial, neste documento, significa `mode_registry.status=active` dentro do
canal `internal_alpha`. O pacote publicado atual, release root, preview tecnico,
URLs, versoes e proximo passo operacional vivem em
`../../implementation/current-status.md`; a linhagem de pacotes vive em
`../release-history.md`. Este documento descreve o contrato vivo do slice
Bosque/Openworld, nao a historia de publicacao.
...
## Overlay Layer And Readiness Authority

O pacote atual listado em `../../implementation/current-status.md` preserva o
modelo do overlay persistente e adiciona autoridade explicita de
...
A historia de pacotes anteriores, incluindo recuperacao de abandono da Arena,
botoes internos e hotfixes de navegacao, vive em `../release-history.md`.
