...
func _check_portrait_app_loop() -> void:
	...
	_expect(_label_tree_contains(boot, "Rotina do Refugio"), "portrait Refugio embeds routine with the main paths")
	_expect(_find_button_by_text(boot, "Coletar producao") != null, "portrait Refugio exposes collect action near main paths")
