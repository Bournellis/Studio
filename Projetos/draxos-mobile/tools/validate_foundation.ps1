...
function Assert-LiveDocsReleaseRootFreshness {
    $currentPackage = "Arena Runtime Config Sync Ready v3"
    $currentRoot = "internal-alpha/v0-arena-runtime-config-sync-ready-v3-20260616-bc04e88a"
    $currentPreview = "https://a50d282b.draxos-mobile-internal-alpha.pages.dev"
    $currentVersion = "0.0.27-alpha.0"
    ...
    foreach ($relative in $filesWithoutOperationalState) {
        foreach ($needle in @($currentPackage, $currentRoot, $currentPreview, $currentVersion, 'version code `27`', 'minimum supported version code `13`')) {
