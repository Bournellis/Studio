...
function Assert-LiveDocsReleaseRootFreshness {
    $currentRoot = "internal-alpha/v0-arena-pve-bonus-visual-v1-20260608-e281d63"
    $currentPreview = "https://6c8bf8e1.draxos-mobile-internal-alpha.pages.dev"
    ...
    foreach ($entry in @(
        ...
        @{ Base = $RepoPath; Path = "08_Coordenacao_Agentes\Prioridades_Estudio.md"; Needles = @("ARENA_PVE_BONUS_VISUAL_V1_PUBLISHED_INTERNAL_ALPHA", $currentRoot, $currentPreview, "Bosque Node Cooldown ACK v1 preservado como pacote Bosque anterior", "Track 13 release safety", "Track 14 agent ops") },
        @{ Base = $RepoPath; Path = "08_Coordenacao_Agentes\Estado_Atual.md"; Needles = @("ARENA_PVE_BONUS_VISUAL_V1_PUBLISHED_INTERNAL_ALPHA", $currentRoot, $currentPreview, "Bosque Node Cooldown ACK v1 segue preservado como pacote Bosque anterior", "Bosque Resume Exit Lifecycle v1", "Bosque Feel & Spawn Authority v1", "Bosque Persistence Rebase v1", $previousSyncRoot, $previousSyncPreview, $previousVisibleRoot, $previousVisiblePreview, "FOUNDATION_HARDENING_V2_PUBLISHED_INTERNAL_ALPHA", $hardeningRoot, $hardeningPreview) },
        @{ Base = $RepoPath; Path = "Projetos\README.md"; Needles = @("Current published package:", "Arena PVE Bonus Visual v1", $currentRoot, $currentPreview, "Previous Bosque package:", $previousBosqueRoot, $previousBosquePreview, "Previous resume/exit package:", $previousResumeRoot, $previousResumePreview, "Previous feel/spawn package:", $previousFeelRoot, $previousFeelPreview, "Previous persistence/operations package:", $previousPersistenceRoot, $previousPersistencePreview) }
    )) {
