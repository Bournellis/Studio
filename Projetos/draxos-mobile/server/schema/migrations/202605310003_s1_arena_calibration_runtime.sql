-- Track 20 - Season 1 Arena Calibration Runtime
-- Promotes calibrated Arena PVE tiers to runtime while preserving Track 19
-- potion consumption, read-only claim, idempotency and no-ranking guarantees.
...
create table if not exists public.arena_first_clears (
	...
	first_attempt_id uuid not null references public.arena_attempts(id) on delete cascade,
	cleared_at timestamptz not null default now(),
	primary key (game_save_id, arena_id, difficulty_id)
);

create index if not exists arena_first_clears_player_idx
	on public.arena_first_clears (player_id, cleared_at desc);

alter table public.arena_first_clears enable row level security;

drop policy if exists "arena_first_clears_select_own" on public.arena_first_clears;
create policy "arena_first_clears_select_own"
	on public.arena_first_clears for select
	using (
		game_save_id in (
			select gs.id
