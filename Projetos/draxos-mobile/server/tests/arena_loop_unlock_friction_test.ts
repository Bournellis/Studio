...
const SERVER_MIGRATION = "server/schema/migrations/202605310004_arena_loop_unlock_friction.sql";
const SUPABASE_MIGRATION = "supabase/migrations/202605310004_arena_loop_unlock_friction.sql";

Deno.test("arena loop unlock migration is mirrored and updates level with arena XP", async () => {
  const serverMigration = await Deno.readTextFile(projectFile(SERVER_MIGRATION));
  const supabaseMigration = await Deno.readTextFile(projectFile(SUPABASE_MIGRATION));
...
Deno.test("tutorial first clear XP reaches first real Arena unlock level", async () => {
  ...
  const reward = objectItems(rewardsPayload).find((item) =>
    stringField(item, "id") === "reward_s1_1d_d00"
  );
  if (reward === undefined) {
    throw new Error("reward_s1_1d_d00 should exist");
  }

  const resources = objectField(reward, "resources");
  const firstClearXp = Math.round(
    numberField(resources, "xp") * numberField(reward, "first_clear_multiplier"),
  );
  assert(firstClearXp >= xpForLevel(3), "tutorial first clear should reach level 3");
  assert(firstClearXp < xpForLevel(4), "tutorial first clear should not skip level 4");
  ...
  assertEq(unlocks[0].unlocked, true, "first real Arena should unlock after tutorial");
});

Deno.test("client Arena loop removes loadout click and continues inside Arena", async () => {
  const lifecycle = await Deno.readTextFile(projectFile("modes/boot/flows/arena_lifecycle_flow.gd"));
  const presenter = await Deno.readTextFile(projectFile("modes/boot/surfaces/arena_surface_presenter.gd"));
  ...
  assertIncludes(lifecycle, "await _refresh_arena_selection(host, \"Arena atualizada. Proximo desafio pronto.\")");
...
async function readJson(relativePath: string): Promise<Record<string, unknown>> {
  const payload = JSON.parse(await Deno.readTextFile(projectFile(relativePath)));
...
function objectItems(payload: Record<string, unknown>): Record<string, unknown>[] {
...
function assertObject(value: unknown, message = "value should be an object"): Record<string, unknown> {
...
function assertEq(actual: unknown, expected: unknown, message = "values should match"): void {
  if (actual !== expected) {
