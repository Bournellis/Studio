...
func test_battle_stage_2d_exposes_compact_replay_readout() -> void:
	...
	var tooltips := Dictionary(snapshot.get("tooltips", {}))
	assert_string_contains(readout, "Lance 3/8")
	assert_string_contains(readout, "3.5s")
	assert_string_contains(readout, "Draxos 90%")
	assert_string_contains(readout, "Bot 72%")
	assert_string_contains(readout, "Pressao")
	assert_string_contains(readout, "efeitos 1-1")
	assert_string_contains(readout, "esperas 1-0")
	assert_string_contains(readout, "aliados 3-0")
...
func test_battle_stage_2d_effect_feedback_uses_full_names() -> void:
	...
	var consumable_text := stage.debug_event_feedback_text({"type": "consumable_use", "item_id": "pocao_vida", "effect_id": "heal_over_time", "duration": 5, "tick_percent": 4})
	var heal_text := stage.debug_event_feedback_text({"type": "heal", "item_id": "pocao_vida", "amount": 8})
...
func test_battle_stage_2d_empty_state_is_stable() -> void:
	...
	assert_string_contains(str(snapshot.get("readout", "")), "aguardando luta")
