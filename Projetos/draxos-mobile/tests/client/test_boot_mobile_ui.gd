...
func test_arena_selection_renders_remote_arenas_as_data_driven_actions() -> void:
	...
	assert_not_null(_find_button_by_text(boot._content_body, "Iniciar tutorial recomendado"))
	...
	assert_not_null(_find_node_by_name(boot._content_body, "ArenaPathGuidePanel"))
	...
	assert_true(_label_tree_contains(boot._content_body, "Comece pelo tutorial de 1 duelo"))
	assert_true(_label_tree_contains(boot._content_body, "resolver duelo -> escolher 1 buff temporario"))
...
func test_arena_selection_recommends_next_uncompleted_arena_after_tutorial() -> void:
	...
	assert_not_null(_find_button_by_text(boot._content_body, "Iniciar primeira arena real"))
	...
	assert_true(_child_index_by_name(boot._content_body, "ArenaPathGuidePanel") < _child_index_by_name(boot._content_body, "ArenaRecommendedCard"))
	...
	assert_true(_label_tree_contains(boot._content_body, "Tutorial concluido. Proximo alvo: Cinzas"))
...
func test_arena_active_exposes_behavior_adjustment_without_unlocking_loadout() -> void:
	...
	assert_true(_label_tree_contains(boot._content_body, "Agora: resolver duelo 1/3."))
	...
	assert_true(_label_tree_contains(boot._content_body, "HP: sempre cheio no inicio de cada duelo."))
	assert_true(_label_tree_contains(boot._content_body, "Ao vencer: escolha 1 buff temporario antes do proximo duelo."))
...
func test_arena_buff_choice_renders_comparable_cards_with_existing_actions() -> void:
	...
	assert_true(_label_tree_contains(boot._content_body, "Aplica no proximo duelo; nao entra no inventario nem no save permanente."))
...
func test_arena_active_pending_buff_opens_choice_without_autoselecting_first_buff() -> void:
	...
	assert_true(_label_tree_contains(boot._content_body, "Agora: escolha 1 buff temporario antes do duelo 2/3."))
	assert_true(boot._action_buttons.has(AppShellActionContractScript.ACTION_ARENA_RESUME_ATTEMPT))
	assert_false(boot._action_buttons.has(AppShellActionContractScript.ACTION_ARENA_RESOLVE_DUEL))
...
func test_arena_active_after_selected_buff_returns_to_resolve_duel() -> void:
	...
	assert_true(_label_tree_contains(boot._content_body, "Buff aplicado ao proximo duelo: Vitalidade Menor (+4% HP maximo)"))
	assert_true(boot._action_buttons.has(AppShellActionContractScript.ACTION_ARENA_RESOLVE_DUEL))
	assert_false(boot._action_buttons.has(AppShellActionContractScript.arena_choose_buff_action("arena_buff_vitalidade_menor")))
...
func test_arena_summary_continues_to_arena_instead_of_reward_claim_copy() -> void:
	...
	assert_true(_label_tree_contains(boot._content_body, "Recompensa: recompensa de conclusao ja aplicada no ultimo duelo"))
	assert_true(_label_tree_contains(boot._content_body, "Progresso: a Temporada 1 sera atualizada ao continuar."))
