...
static func render(host: Control) -> void:
	...
	background.color = UiTokens.color("bg_void")
	background.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	host.add_child(background)

	_render_premium_background(host)
	_render_refuge_first_screen_layer(host)

	var root := VBoxContainer.new()
	root.name = "AppShellChromeRoot"
	root.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	var shell_margin := MobileUiContractScript.shell_margin(compact)
	root.offset_left = shell_margin
	root.offset_top = shell_margin
	root.offset_right = -shell_margin
	root.offset_bottom = -shell_margin
	root.add_theme_constant_override("separation", MobileUiContractScript.panel_gap(compact))
	host.add_child(root)
...
static func _render_premium_background(host: Control) -> void:
	var blood_wash := ColorRect.new()
	blood_wash.color = UiTokens.color("bg_blood_wash")
	blood_wash.color.a = 0.46
	blood_wash.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	blood_wash.mouse_filter = Control.MOUSE_FILTER_IGNORE
	host.add_child(blood_wash)

	var top_rule := ColorRect.new()
	top_rule.color = UiTokens.color("border_blood")
	top_rule.color.a = 0.9
	top_rule.set_anchors_and_offsets_preset(Control.PRESET_TOP_WIDE)
	top_rule.custom_minimum_size.y = 3
	top_rule.offset_bottom = 3
...
static func _render_header(host: Control, root: VBoxContainer, compact: bool) -> void:
	...
	header.add_theme_stylebox_override("panel", UiTokens.panel_style("shell_header", compact))
	header.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	root.add_child(header)

	var header_box := VBoxContainer.new()
	header_box.add_theme_constant_override("separation", 6 if compact else 8)
	header.add_child(header_box)

	var title_row := HBoxContainer.new()
	title_row.add_theme_constant_override("separation", MobileUiContractScript.panel_gap(compact))
	...
	status_label.text = "%s - refugio ativo" % ProjectInfoScript.PROJECT_NAME
	status_label.autowrap_mode = TextServer.AUTOWRAP_OFF if compact else TextServer.AUTOWRAP_WORD_SMART
	if compact:
		status_label.clip_text = true
		status_label.add_theme_font_size_override("font_size", 12)
	status_label.add_theme_color_override("font_color", UiTokens.color("accent_bone"))
	...
	back_button.custom_minimum_size = Vector2(70, MobileUiContractScript.MIN_TOUCH_TARGET) if compact else Vector2(118, MobileUiContractScript.MIN_TOUCH_TARGET)
	host.call("_prepare_touch_button", back_button)
	_apply_button_style(back_button, "secondary")
...
static func _render_content_shell(host: Control, root: VBoxContainer, compact: bool) -> void:
	...
	content_panel.add_theme_stylebox_override("panel", UiTokens.panel_style("shell_content", compact))
	content_panel.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	content_panel.size_flags_vertical = Control.SIZE_EXPAND_FILL
	root.add_child(content_panel)

	var content_stack := VBoxContainer.new()
	content_stack.add_theme_constant_override("separation", MobileUiContractScript.panel_gap(compact))
	content_panel.add_child(content_stack)

	var content_title := Label.new()
	content_title.add_theme_font_size_override("font_size", 20 if compact else 24)
	...
	content_body.add_theme_constant_override("separation", MobileUiContractScript.panel_gap(compact))
	scroll.add_child(content_body)
	var sync_content_width := func() -> void:
		content_body.custom_minimum_size.x = maxf(0.0, scroll.size.x - MobileUiContractScript.TOUCH_SCROLLBAR_WIDTH)
...
static func _render_confirmation_dialog(host: Control) -> void:
	...
	MobileUiContractScript.apply_cta_button(confirm_dialog.get_ok_button())
	MobileUiContractScript.apply_touch_button(confirm_dialog.get_cancel_button())
	_apply_button_style(confirm_dialog.get_ok_button(), "cta")
	_apply_button_style(confirm_dialog.get_cancel_button(), "secondary")
...
static func _apply_button_style(button: Button, style_id: String) -> void:
	if button == null:
		return
	button.add_theme_color_override("font_color", UiTokens.color("text_on_accent" if style_id == "cta" else "text_primary"))
	button.add_theme_stylebox_override("normal", UiTokens.button_style(style_id, "normal"))
	button.add_theme_stylebox_override("hover", UiTokens.button_style(style_id, "hover"))
	button.add_theme_stylebox_override("pressed", UiTokens.button_style(style_id, "pressed"))
	button.add_theme_stylebox_override("focus", UiTokens.button_style(style_id, "hover"))
