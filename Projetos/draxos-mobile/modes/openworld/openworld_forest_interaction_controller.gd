...
var runtime: OpenworldForestRuntimeState = null
...
func configure(
	next_model: Variant,
	next_runtime: OpenworldForestRuntimeState,
...
func _advance_nearby_collection(delta: float) -> void:
	var bridge = _bridge()
	var nearest := runtime.nearest_resource(bridge)
	if nearest.is_empty():
		if not model.active_collection.is_empty():
	...
	var distance := runtime.player_position.distance_to(Vector2(nearest.get("position", Vector2.ZERO)))
