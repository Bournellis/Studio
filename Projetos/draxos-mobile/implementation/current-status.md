...
| Internal Alpha v0 | T03-P12 release plan e portal base completos | Track 03 documentada com escopo, plano, runbook, checklist, design lock, plano de release e tutorial remoto. `DMOB-D048` a `DMOB-D055` resolvidas; valores numericos da loja ficam como dados calibraveis do catalogo/seed. Configuracao segura do backend remoto preparada no cliente/docs/testes; save `normal`/`progression_lab` existe no cliente e no Supabase local, com reset separado, aplicacao server-backed do Progression Lab, Base Manager jogavel, Social basico jogavel, Competicao/leaderboard alpha, Loja proof-of-concept, Batalha visual polida e QA local automatizado verde; portal base criado em `portal/internal-alpha/`; proximo passo e T03-P13 com valores publicos do Supabase remoto |
...
1. Fabio seguir `docs/supabase-remote-tutorial.md` e criar `.env.internal-alpha.local` ignorado com os valores publicos do projeto remoto.
2. Executar `T03-P13 - Supabase Remote Bootstrap`: link da CLI, migrations, Edge Functions, healthcheck e smoke remoto.
3. Depois do remoto verde, seguir `T03-P14` email/senha, `T03-P15` manifest de updates e `T03-P16` exports Android/PC/Web.
