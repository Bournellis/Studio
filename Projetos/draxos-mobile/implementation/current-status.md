...
| Track 10 | Integrada; batalha portrait com palco limpo pronta para QA visual manual | Track 10 reformulou `battle_running` como fullscreen sem chrome com `BattleStage2D` focado no duelo e `Pular batalha` no canto inferior direito, simplificou `battle_summary` para resultado minimo e adicionou `battle_logs` para eventos textuais da batalha atual. Sem backend/schema/economia/ranking/recompensas/simulador/publicacao remota. |
...
| Validacao | Track 10 com matriz client verde para o patch | No patch Track 10 de 2026-05-28, `tools/validate.gd` passou com GUT `98/98` testes e `1208` asserts; GUT client completo passou com `98/98` testes e `1208` asserts; `smoke_mobile_presentation.gd`, `smoke_foundation_hardening.gd` e `git diff --check` passaram. `smoke_battle_replay.gd` chegou ate a batalha/replay e falhou em `/battle/history` com `NOT_FOUND` pelo mesmo Edge Runtime local antigo ja registrado; validar esse smoke com a funcao `battle` atual servida via `BATTLE_FUNCTION_URL` ou reiniciar/redeployar a runtime local. |
