...
| Internal Alpha v0 | T03-P07 completo; proximo T03-P08 | Track 03 documentada com escopo, plano, runbook, checklist e design lock. `DMOB-D048` a `DMOB-D055` resolvidas; valores numericos da loja ficam como dados calibraveis do catalogo/seed. Configuracao segura do backend remoto preparada no cliente/docs/testes; save `normal`/`progression_lab` existe no cliente e no Supabase local, com reset separado, aplicacao server-backed do Progression Lab, Base Manager jogavel, Social basico jogavel e Competicao/leaderboard alpha validados; Supabase remoto real, builds e update manifest ficam adiados ate o gameplay local estar pronto |
...
| Godot project | Alpha PC local pronto + save context/reset server-backed + Base/Social/Competicao jogaveis + Battle Lab/Progression Lab dev | Hub alpha hardenizado, autoloads, `.gutconfig.json`, content generator, catalogo gerado, `SessionStore` com `session_id`, `active_save_type`, `apply_snapshot_cache`, `apply_save_reset` e metadados `progression_lab`, `SupabaseClient` com telemetria, header ativo `x-draxos-save-type`, reset do save ativo e acoes sociais, Base com mapa de predios clicaveis/painel por estrutura/upgrade por predio/atalho alpha de Energia, Social com campos de username/guilda/chat, criar/entrar em guilda, painel de identidade/amigos/membros/estruturas/chat e tooltips, Competicao com preview, ultima batalha competitiva, top 10, posicao do jogador e tooltips, `BattleLogPresenter`, `BattleVisualMockup`, `BattleStage2D` responsivo com tooltips imediatos, nos estaveis durante efeitos, cooldown restante por relogio continuo de replay e feedback textual com nomes completos, telas dev-only do Battle Lab e Progression Lab, cache local-only read-only sem token valido, runner Deno sanitizado/Windows-safe, smoke real de labs, smoke visual/comportamental e GUT |
| Supabase project | Conta guest + dois saves locais + reset separado + battle MVP + first-slice sim + base/social/competicao/monetizacao/telemetria v0 prontos | Layout `supabase/`, migrations MVP/base/social/ranking/monetizacao/`save_type`/reset, Auth anonimo, healthcheck, `account/*`, `battle/*`, `base/*`, `social/*`, `competition/*`, `monetization/*`, `telemetry/*`, `progression-lab/*`, Base com payload de apresentacao server-side para custo/tempo/producao/bloqueio, Social com identidade de conta, amigos por username, guilda create/join, chat por polling e rate limit, Competicao com pontos de arena por batalha normal, top 10, self rank, bots fora do ranking e Lab excluido, seeds `FIRST_SLICE`, JWT config de funcoes, simulador compartilhado e helper `_shared/save_context.ts` configurados |
| Validacao | Verde tecnico; tuning manual pendente | Godot validate + GUT `48/48`, `307` asserts passam; smoke session shell passa; smoke alpha loop passa; `supabase db reset` passa com migrations `202605260001_two_save_context.sql`, `202605260002_reset_save_context.sql` e `202605260003_progression_lab_apply.sql`; Deno checks/lint das funcoes Supabase/server passam; smoke `first_slice_battle_smoke.ts` valida arena idempotente e `ranking/current`; smoke `two_save_context_smoke.ts` cria saves distintos para a mesma sessao Auth; smoke `reset_save_context_smoke.ts` valida reset isolado/idempotente; smoke `progression_lab_apply_smoke.ts` valida aplicacao server-backed do Lab; smoke `base_manager_smoke.ts` valida payload jogavel da Base, compra de Energia, upgrade por predio e fila cheia; smoke `social_competition_smoke.ts` valida dois testadores, amizade por username, guilda create/join, chat idempotente, rate limit, polling, top 10, self rank, bots fora do ranking e RLS; smokes de monetizacao/telemetria passam; smoke de labs executa Battle Lab/Progression Lab via `OS.execute`; smoke visual/comportamental passa e valida cache local-only sem token online; smoke de exports passa; Supabase remoto real ainda precisa projeto/credenciais e smoke contra URL real |
...
- Smokes Supabase existentes de batalha, first-slice, monetizacao e telemetria: passaram apos reset de save; `first_slice_battle_smoke.ts` passou validando arena/ranking idempotente e `social_competition_smoke.ts` passou com top 10/self rank no fluxo de dois testadores.
...
- GUT integrado: `48/48` testes, `307` asserts.
...
1. Executar `T03-P08 - Loja Proof-Of-Concept`.
