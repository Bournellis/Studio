...
- Active Surface: `Portrait Entry and Refugio visual loop integrated`
- Active Track: `Track 09 - Portrait Entry, Refugio Scene And Visual Loop Rework`
- Active Track Status: `INTEGRATED_PORTRAIT_LOOP_READY`
...
| Track 09 | Integrada; loop visual portrait pronto para walkthrough manual | Track 09 separou `entry` operacional de `refuge` jogavel, tornou o app portrait-first, converteu a batalha para portrait fullscreen e reposicionou Base como `base_management` interna. Sem backend/schema/economia/ranking/recompensas/simulador/publicacao remota. |
...
| Validacao | Track 09 integrada com matriz verde local | Na integracao Track 09 de 2026-05-28, `tools/validate.gd` e GUT passaram com `95/95` testes e `1144` asserts; `smoke_mobile_presentation.gd`, `smoke_foundation_hardening.gd`, `smoke_foundation_surfaces.gd`, `smoke_session_shell.gd`, `smoke_runtime_config.gd`, `smoke_exports.gd` e `git diff --check` passaram. `smoke_battle_replay.gd` no Edge Runtime local padrao ainda retorna `NOT_FOUND` em `/battle/history` porque a runtime em `127.0.0.1:54321` serve funcao antiga; validar esse smoke com a funcao `battle` atual servida via `BATTLE_FUNCTION_URL`. |
...
1. Fazer walkthrough visual/manual do loop Track 09 no Godot e Android: Entry, save/lab, Refugio, hotspots, Base management e battle summary portrait.
