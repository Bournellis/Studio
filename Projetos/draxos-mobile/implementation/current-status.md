...
| Internal Alpha v0 | T03-P09 completo; proximo T03-P11 local QA/playtest | Track 03 documentada com escopo, plano, runbook, checklist e design lock. `DMOB-D048` a `DMOB-D055` resolvidas; valores numericos da loja ficam como dados calibraveis do catalogo/seed. Configuracao segura do backend remoto preparada no cliente/docs/testes; save `normal`/`progression_lab` existe no cliente e no Supabase local, com reset separado, aplicacao server-backed do Progression Lab, Base Manager jogavel, Social basico jogavel, Competicao/leaderboard alpha, Loja proof-of-concept e Batalha visual polida para leitura de playtest; Supabase remoto real, builds e update manifest ficam adiados ate o gameplay local estar pronto |
...
| Validacao | Verde tecnico; tuning manual pendente | Godot validate + GUT `49/49`, `320` asserts passam; smoke session shell passa; smoke alpha loop passa com redeem de Loja; `supabase db reset` passa com migrations `202605260001_two_save_context.sql`, `202605260002_reset_save_context.sql` e `202605260003_progression_lab_apply.sql`; Deno checks/lint das funcoes Supabase/server passam; smoke `first_slice_battle_smoke.ts` valida arena idempotente e `ranking/current`; smoke `two_save_context_smoke.ts` cria saves distintos para a mesma sessao Auth; smoke `reset_save_context_smoke.ts` valida reset isolado/idempotente; smoke `progression_lab_apply_smoke.ts` valida aplicacao server-backed do Lab; smoke `base_manager_smoke.ts` valida payload jogavel da Base, compra de Energia, upgrade por predio, fila cheia e fila dupla; smoke `monetization_rewards_smoke.ts` valida redeems diarios, premium por Diamante, fila dupla e RLS; smoke `social_competition_smoke.ts` valida dois testadores, amizade por username, guilda create/join, chat idempotente, rate limit, polling, top 10, self rank, bots fora do ranking e RLS; smoke de telemetria passa; smoke de labs executa Battle Lab/Progression Lab via `OS.execute`; smoke visual/comportamental passa e valida cache local-only sem token online; smoke de exports passa; Supabase remoto real ainda precisa projeto/credenciais e smoke contra URL real |
...
1. Preparar `T03-P11 - QA, Smokes E Playtest Fechado` em modo local/Godot.
2. Manter `T03-P10 - Releases E Updates` adiado ate o gameplay local estar pronto para remoto/builds.
