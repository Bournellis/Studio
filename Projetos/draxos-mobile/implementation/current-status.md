...
| Internal Alpha v0 | T03-P08 completo; proximo T03-P09 | Track 03 documentada com escopo, plano, runbook, checklist e design lock. `DMOB-D048` a `DMOB-D055` resolvidas; valores numericos da loja ficam como dados calibraveis do catalogo/seed. Configuracao segura do backend remoto preparada no cliente/docs/testes; save `normal`/`progression_lab` existe no cliente e no Supabase local, com reset separado, aplicacao server-backed do Progression Lab, Base Manager jogavel, Social basico jogavel, Competicao/leaderboard alpha e Loja proof-of-concept validados; Supabase remoto real, builds e update manifest ficam adiados ate o gameplay local estar pronto |
...
1. Executar `T03-P09 - Batalha Visual Polish Pequeno`.
2. Fazer apenas ajustes de clareza visual/HUD/tooltips que aumentem leitura do playtest.
