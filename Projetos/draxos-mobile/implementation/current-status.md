...
- Active Track Status: `T03-P06_COMPLETE - SOCIAL BASIC PLAYABLE READY`
- Current Operational Baseline: Track 00 completa com primeiro slice server-authoritative, Track 01 completa para hardening do alpha PC local e Track 02 com Progression Lab/Battle Lab v1 implementados. Godot 4.6.2 possui hub alpha, Base/Social/Competicao/Monetizacao v0, batalha `battle_log_v1` server-authoritative, mockup visual 2D procedural compartilhado por Batalha/Battle Lab, exports Android/PC/Web, validate/GUT/smokes verdes no ultimo baseline e Supabase local em layout oficial. Track 03 prepara Internal Alpha v0: email/senha, dois saves por conta (`normal` e `progression_lab`), Supabase remoto Free, Progression Lab isolado, loja com redeems alpha, leaderboards basicas, social/base jogaveis e manifest de updates para Android/PC/Web. Ordem local-first aprovada em 2026-05-26: implementar e validar o jogo rodando no Godot/local primeiro; Supabase remoto, builds Android/PC/Web e manifest de updates ficam adiados ate o gameplay local estar pronto. T03-P02 esta preparado no repo com `BackendConfig`, ambiente `internal_alpha_v0`, env vars seguras, runbook/smoke remoto e bloqueio contra service role no cliente. T03-P03A adicionou contexto local de save ativo no Godot, cache persistente e HUD de save. T03-P03B adicionou `players.save_type` no Supabase local, isolamento `normal`/`progression_lab` por `auth_user_id + save_type`, resolucao do header `x-draxos-save-type` em todos os endpoints alpha e smoke de dois saves server-backed. T03-P03C adicionou reset server-authoritative por save, preservando o outro save da mesma sessao Auth e limpando snapshots client-side do save resetado. T03-P04 adicionou `progression-lab/apply` server-backed: o servidor valida healthy saves versionados, aplica perfis/milestones apenas no save `progression_lab`, preserva o save `normal`, mantem o Lab fora do ranking e permite jogar batalhas/base/loja/social isolados apos aplicar o perfil. T03-P05 transformou a Base em fluxo jogavel: mapa de predios clicaveis, painel detalhado por estrutura, custo/tempo/producao/bloqueio calculados pelo servidor, compra alpha de Energia para destravar upgrades e upgrade server-authoritative por predio. T03-P06 transformou Social em fluxo basico jogavel: amigos por username, criar/entrar em guilda, membros/estruturas, chat por polling, rate limit, tooltips/painéis no Hub e identidade social de conta com marcador `lab`. Estrategia backend registrada: Supabase para alpha, Backend Proprio + Postgres como plano de saida preferido e Nakama como alternativa futura apenas se realtime/social competitivo virar pilar.
...
| Internal Alpha v0 | T03-P06 completo; proximo T03-P07 | Track 03 documentada com escopo, plano, runbook, checklist e design lock. `DMOB-D048` a `DMOB-D055` resolvidas; valores numericos da loja ficam como dados calibraveis do catalogo/seed. Configuracao segura do backend remoto preparada no cliente/docs/testes; save `normal`/`progression_lab` existe no cliente e no Supabase local, com reset separado, aplicacao server-backed do Progression Lab, Base Manager jogavel e Social basico jogavel validados; Supabase remoto real, builds e update manifest ficam adiados ate o gameplay local estar pronto |
...
| Godot project | Alpha PC local pronto + save context/reset server-backed + Base/Social jogaveis + Battle Lab/Progression Lab dev | Hub alpha hardenizado, autoloads, `.gutconfig.json`, content generator, catalogo gerado, `SessionStore` com `session_id`, `active_save_type`, `apply_snapshot_cache`, `apply_save_reset` e metadados `progression_lab`, `SupabaseClient` com telemetria, header ativo `x-draxos-save-type`, reset do save ativo e acoes sociais, Base com mapa de predios clicaveis/painel por estrutura/upgrade por predio/atalho alpha de Energia, Social com campos de username/guilda/chat, criar/entrar em guilda, painel de identidade/amigos/membros/estruturas/chat e tooltips, `BattleLogPresenter`, `BattleVisualMockup`, `BattleStage2D` responsivo com tooltips imediatos, nos estaveis durante efeitos, cooldown restante por relogio continuo de replay e feedback textual com nomes completos, telas dev-only do Battle Lab e Progression Lab, cache local-only read-only sem token valido, runner Deno sanitizado/Windows-safe, smoke real de labs, smoke visual/comportamental e GUT |
| Supabase project | Conta guest + dois saves locais + reset separado + battle MVP + first-slice sim + base/social/competicao/monetizacao/telemetria v0 prontos | Layout `supabase/`, migrations MVP/base/social/ranking/monetizacao/`save_type`/reset, Auth anonimo, healthcheck, `account/*`, `battle/*`, `base/*`, `social/*`, `competition/*`, `monetization/*`, `telemetry/*`, `progression-lab/*`, Base com payload de apresentacao server-side para custo/tempo/producao/bloqueio, Social com identidade de conta, amigos por username, guilda create/join, chat por polling e rate limit, seeds `FIRST_SLICE`, JWT config de funcoes, simulador compartilhado e helper `_shared/save_context.ts` configurados |
| Validacao | Verde tecnico; tuning manual pendente | Godot validate + GUT `48/48`, `306` asserts passam; smoke session shell passa; smoke alpha loop passa; `supabase db reset` passa com migrations `202605260001_two_save_context.sql`, `202605260002_reset_save_context.sql` e `202605260003_progression_lab_apply.sql`; Deno checks das funcoes Supabase/server passam; smoke `two_save_context_smoke.ts` cria saves distintos para a mesma sessao Auth; smoke `reset_save_context_smoke.ts` valida reset isolado/idempotente; smoke `progression_lab_apply_smoke.ts` valida aplicacao server-backed do Lab; smoke `base_manager_smoke.ts` valida payload jogavel da Base, compra de Energia, upgrade por predio e fila cheia; smoke `social_competition_smoke.ts` valida dois testadores, amizade por username, guilda create/join, chat idempotente, rate limit, polling e RLS; smokes existentes de batalha, monetizacao e telemetria passam; smoke de labs executa Battle Lab/Progression Lab via `OS.execute`; smoke visual/comportamental passa e valida cache local-only sem token online; smoke de exports passa; Supabase remoto real ainda precisa projeto/credenciais e smoke contra URL real |
...
- `supabase/functions/base/` implementa `GET /base/state`, `POST /base/collect` e `POST /base/upgrade` com estruturas permanentes, fila de construcao, conclusao de jobs vencidos, coleta offline, ledger, idempotencia por `request_id`, filtro por `x-draxos-save-type` e payload de UI com descricao, beneficio, custo, tempo, status, bloqueio e remaining time por predio.
...
- `supabase/functions/competition/` implementa `GET /competition/matchmaking/preview` e `GET /competition/ranking/current` com fallback de bot, ranking de season sem bots, filtro por `x-draxos-save-type` e exclusao explicita do `progression_lab` no ranking.
- `supabase/functions/monetization/` implementa `GET /monetization/state`, `POST /monetization/rewards/claim` e `POST /monetization/alpha-purchase` com Battle Pass, Diamante, recompensas diarias/semanais, premium alpha, ledger, idempotencia e filtro por `x-draxos-save-type`.
...
- `npx -y deno run --allow-net --allow-env server/tests/base_manager_smoke.ts`: passou, validando Base jogavel com payload de custo/tempo/bloqueio, compra alpha de Energia, upgrade por predio e fila cheia.
- Smokes Supabase existentes de batalha, first-slice, monetizacao e telemetria: passaram apos reset de save; `social_competition_smoke.ts` passou com o fluxo novo de dois testadores.
...
- GUT integrado: `48/48` testes, `306` asserts.
...
- Smoke P12 via Supabase runtime: `social/state` exige auth e retorna identidade de conta, `friends/add` adiciona por username, `guild/create` e `guild/join` permitem dois testadores na mesma guilda, `chat/send` e idempotente e rate-limited, `competition/matchmaking/preview` retorna bot nao ranqueado, `competition/ranking/current` cria linha propria e insert direto em `guilds` com JWT anonimo e bloqueado.
...
1. Executar `T03-P07 - Competicao e Leaderboards`.
2. Expandir Loja com o isolamento de save ja validado.
3. Retomar email/senha e remoto apenas quando o gameplay local estiver pronto para compartilhar.
