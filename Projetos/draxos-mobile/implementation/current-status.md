...
- Track 06 Feature Installation Update 2026-05-27: Track 06 foi aberta como fase ativa sobre a fundacao Track 05. Objetivo: criar feature rails e instalar primeiros slices visiveis de baixo risco: runtime config, perfil/conta, historico de batalhas, rotina da Base, Social QoL e Asset Pack 01. Guardrails: sem tuning numerico, sem migration `account_profiles` + `game_saves`, sem pagamento real, sem iOS/mobile browser, sem social realtime e sem publicacao remota.
- Track 06 Runtime Config Update 2026-05-27: T06-C esta `READY_FOR_MERGE` em `codex/draxos-mobile/t06-runtime-config`. Entregou `GET /release/config` com `runtime_config_v1` release-scoped/read-only, flags T06 allowlisted, cliente Godot com fallback conservador e smokes/GUT focados. Sem schema, tuning, secrets, service role, estado de jogador, publicacao remota ou mutacao de gameplay.
...
| Track 06 | Ativa; feature rails e primeiros slices | Track 06 abre a instalacao de features sobre a fundacao Track 05: feature registry, runtime config, perfil/conta, battle history/replay, rotina da Base, Social QoL e Asset Pack 01. Sem tuning numerico, sem migration conta/save, sem pagamento real, sem realtime social e sem publicacao remota. |
...
1. Integrar `T06-A` e usar essa base para os agentes paralelos.
2. Executar `T06-B` Feature Rails e `T06-C` Runtime Config.
3. Executar `T06-D` a `T06-H` em worktrees paralelas, com `T06-I` para integracao final.
