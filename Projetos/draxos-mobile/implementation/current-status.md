...
| Internal Alpha v0 | T03-P03A completo; proximo T03-P03B | Track 03 documentada com escopo, plano, runbook, checklist e design lock. `DMOB-D048` a `DMOB-D055` resolvidas; valores numericos da loja ficam como dados calibraveis do catalogo/seed. Configuracao segura do backend remoto preparada no cliente/docs/testes; Supabase remoto real, builds e update manifest ficam adiados ate o gameplay local estar pronto |
...
| Godot project | Alpha PC local pronto + save context local + Battle Lab/Progression Lab dev | Hub alpha hardenizado, autoloads, `.gutconfig.json`, content generator, catalogo gerado, `SessionStore` com `session_id`, `active_save_type`, `apply_snapshot_cache` e metadados `progression_lab`, `SupabaseClient` com telemetria e header futuro `x-draxos-save-type`, `BattleLogPresenter`, `BattleVisualMockup`, `BattleStage2D` responsivo com tooltips imediatos, nos estaveis durante efeitos, cooldown restante por relogio continuo de replay e feedback textual com nomes completos, telas dev-only do Battle Lab e Progression Lab, cache local-only read-only sem token valido, runner Deno sanitizado/Windows-safe, smoke real de labs, smoke visual/comportamental e GUT |
...
| Validacao | Verde tecnico; tuning manual pendente | Godot validate + GUT `46/46`, `280` asserts passam; smoke session shell passa; smoke de labs executa Battle Lab/Progression Lab via `OS.execute`; smoke visual/comportamental passa e valida cache local-only sem token online; smoke de exports passa; Deno check do smoke remoto passa; Deno tests passam; dry-run do seeder seleciona `25/25` saves; Battle Lab Source Identity Balance v2 gera `PASS`; Progression Lab gera `REVIEW`; Supabase remoto real ainda precisa projeto/credenciais e smoke contra URL real |
...
- `online/supabase_client.gd` implementa HTTPRequest para Auth, `account/*`, `battle/*`, `base/*`, `social/*`, `competition/*`, `monetization/*` e `telemetry/*` em Supabase local/remoto via `BackendConfig`, preparando `x-draxos-save-type` para a proxima etapa server-side.
...
1. Executar `T03-P03B - save_type server-side no Supabase local`.
2. Implementar reset separado por save.
3. Implementar isolamento do Progression Lab antes de expandir Base/Social/Loja.
