...
- Active Surface: `Foundation stabilization and asset/service readiness`
- Active Track: `Track 05 - Foundation Stabilization And Asset/Service Readiness`
- Active Track Status: `ACTIVE_FOUNDATION_STABILIZATION`
...
- Track 05 Foundation Update 2026-05-27: Track 05 entrou em `ACTIVE_FOUNDATION_STABILIZATION` para consolidar validacao, Hub foundation, contratos de servico, readiness de assets, pacote humano do Progression Lab e release ops antes de assets reais e novos servicos. Guardrails: sem migration `account_profiles` + `game_saves`, sem tuning numerico, sem assets finais, sem servicos novos de gameplay e sem publicacao remota.
...
| Track 05 | Ativa; fundacao antes de assets/servicos | Estabilizar validacao, Hub foundation, contratos de servico, asset pipeline, pacote humano do Progression Lab e release ops antes de assets reais ou servicos novos. Sem migration, sem tuning numerico, sem assets finais e sem publicacao remota nesta track. |
...
| Validacao | Verde tecnico local + downloads/manifest/Cloudflare remotos verdes | Na integracao Track 04 de 2026-05-27, `tools/validate.gd` passou com `60/60` testes e `417` asserts, `tools/smoke_session_shell.gd` passou e `tools/smoke_battle_replay.gd` passou. T03-P11 resetou cache Godot, scratch Progression Lab e Supabase local. Em 2026-05-27, baseline anterior Godot validate + GUT passou com `54/54`, `367` asserts apos o teste de UI compacta; checks/lints Deno das funcoes Supabase/server passaram incluindo `release`; smokes locais e remotos cobriram batalha, dois saves, reset isolado, Progression Lab apply, Base, Social/Competicao, Monetizacao, Telemetria, email/senha alpha e manifest; T03-P16 passou em `smoke_exports.gd` e `tools/export_internal_alpha.ps1`; T03-P17 passou em publicacao/QA remoto; T03-P17A passou em manifest/downloads remotos, `release_manifest_smoke.ts`, `internal_alpha_remote_smoke.ts`, deploy Wrangler e comparacao HTML local/remoto de Portal/Web. T03-P18 passou em Deno check/lint de `release`, `publish_internal_alpha.ps1`, `build_cloudflare_pages_package.ps1`, deploy Wrangler `https://3a994d39.draxos-mobile-internal-alpha.pages.dev`, comparacao HTML preview/stable, `release_manifest_smoke.ts` remoto e `internal_alpha_remote_smoke.ts` com release. |
...
- Track 05 estabiliza a fundacao antes de assets reais e novos servicos: matriz de validacao, Hub foundation, contratos de servico, asset pipeline, Progression Human Pack e release ops sem expandir produto.
...
1. `../AGENTS.md`
2. `tracks/track-05-foundation-stabilization-and-asset-service-readiness/current-status.md`
3. `tracks/track-05-foundation-stabilization-and-asset-service-readiness/scope.md`
4. `tracks/track-05-foundation-stabilization-and-asset-service-readiness/implementation-plan.md`
...
1. Revisar/publicar a branch `codex/draxos-mobile/t04-integration`.
2. Fazer rodada humana no Godot/Supabase local com os perfis destacados por `T04-G` antes de tuning numerico.
3. Escolher o proximo pacote Track 04: UX/onboarding Android, tuning pequeno documentado ou modularizacao fina remanescente.
