...
| Track 08 | Ativa; hardening de fundacao em andamento | T08-C entregue em worktree: session/save/cache/runtime config agora tem contexto interno por save, snapshots por surface, protecao contra resposta stale, cache local-only sem token valido e diagnostics sem secrets. Demais trilhas seguem paralelas; sem gameplay novo, tuning, schema, endpoints publicos, assets finais ou publicacao remota. |
