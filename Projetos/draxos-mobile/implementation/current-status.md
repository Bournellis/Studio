...
| Internal Alpha v0 | T03-P03C completo; proximo T03-P04 | Track 03 documentada com escopo, plano, runbook, checklist e design lock. `DMOB-D048` a `DMOB-D055` resolvidas; valores numericos da loja ficam como dados calibraveis do catalogo/seed. Configuracao segura do backend remoto preparada no cliente/docs/testes; save `normal`/`progression_lab` existe no cliente e no Supabase local, com reset separado validado; Supabase remoto real, builds e update manifest ficam adiados ate o gameplay local estar pronto |
...
| Validacao | Verde tecnico; tuning manual pendente | Godot validate + GUT `47/47`, `291` asserts passam; smoke session shell passa; `supabase db reset` passa com migrations `202605260001_two_save_context.sql` e `202605260002_reset_save_context.sql`; Deno checks das funcoes Supabase/server passam; smoke `two_save_context_smoke.ts` cria saves distintos para a mesma sessao Auth; smoke `reset_save_context_smoke.ts` valida reset isolado/idempotente; smokes existentes de batalha, base, social/competicao, monetizacao e telemetria passam; smoke de labs executa Battle Lab/Progression Lab via `OS.execute`; smoke visual/comportamental passa e valida cache local-only sem token online; smoke de exports passa; Supabase remoto real ainda precisa projeto/credenciais e smoke contra URL real |
...
- `npx -y supabase db reset`: passou aplicando `202605260001_two_save_context.sql` e `202605260002_reset_save_context.sql`.
...
- GUT integrado: `47/47` testes, `291` asserts.
...
1. Executar `T03-P04 - Progression Lab aplicado ao save progression_lab`.
2. Expandir Base/Social/Competicao/Loja com o isolamento de save ja validado.
