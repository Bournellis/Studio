...
- Active Surface: `Post-alpha evolution and Track 04 integrated Hub surfaces`
- Active Track: `Track 04 - Post-Handoff Hardening And Hub Modularization`
- Active Track Status: `ACTIVE_POST_ALPHA_EVOLUTION`
...
| Track 04 | Ativa; integracao paralela consolidada | `T04-P00` concluido por confirmacao do usuario. `T04-A` a `T04-H` integradas em branch propria: presenters render-only, plano de modularizacao, relatorio Progression/Economia e decisao Account/Save Gate. Nao misturar extracao UI com backend/schema; `account_profiles` + `game_saves` segue fora do pacote atual porque `T04-H` decidiu manter `players.save_type` no curto prazo. |
