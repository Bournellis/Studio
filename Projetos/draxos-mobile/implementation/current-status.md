...
- Active Track Status: `T03-P11_LOCAL_QA_COMPLETE - AUTOMATED LOCAL QA GREEN`
...
| Internal Alpha v0 | T03-P11 QA local automatizado completo | Track 03 documentada com escopo, plano, runbook, checklist e design lock. `DMOB-D048` a `DMOB-D055` resolvidas; valores numericos da loja ficam como dados calibraveis do catalogo/seed. Configuracao segura do backend remoto preparada no cliente/docs/testes; save `normal`/`progression_lab` existe no cliente e no Supabase local, com reset separado, aplicacao server-backed do Progression Lab, Base Manager jogavel, Social basico jogavel, Competicao/leaderboard alpha, Loja proof-of-concept, Batalha visual polida e QA local automatizado verde; Supabase remoto real, builds e update manifest ficam adiados ate o gameplay local estar aprovado para compartilhar |
...
| Validacao | Verde tecnico; tuning manual pendente | T03-P11 resetou cache Godot, scratch Progression Lab e Supabase local. Godot validate + GUT `49/49`, `320` asserts passam; smoke session shell/replay/alpha loop/dev labs/dev lab UI/export presets passam; `supabase db reset` passa com migrations `202605260001_two_save_context.sql`, `202605260002_reset_save_context.sql` e `202605260003_progression_lab_apply.sql`; Deno checks/lint das funcoes Supabase/server passam; smokes locais cobrem batalha MVP/FIRST_SLICE, dois saves, reset isolado, Progression Lab apply, Base, Social/Competicao, Monetizacao e Telemetria; `deno test` de Battle Lab/Progression Lab/simulador passa `20/20`; Supabase remoto real ainda precisa projeto/credenciais e smoke contra URL real |
...
1. Fazer uma passada manual no Godot/editor para UX/legibilidade real usando o roteiro de `docs/playtest-internal-alpha-v0.md`.
2. Depois da aprovacao manual, decidir entre ajustes locais finais ou retomada de `T03-P02`/`T03-P10` para remoto/builds.
