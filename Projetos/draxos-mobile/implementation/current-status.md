...
| Validacao | Verde tecnico local + remoto email/manifest + exports locais | T03-P11 resetou cache Godot, scratch Progression Lab e Supabase local. Em 2026-05-27, Godot validate + GUT passou com `53/53`, `359` asserts; `supabase db reset` passou com `202605270001_alpha_email_account.sql`; checks/lints Deno das funcoes Supabase/server passaram incluindo `release`; smokes locais cobrem batalha MVP/FIRST_SLICE, dois saves, reset isolado, Progression Lab apply, Base, Social/Competicao, Monetizacao, Telemetria, email/senha alpha e manifest; remoto T03-P13/T03-P15 passou com `supabase db push`, deploy das Edge Functions, `supabase config push --yes`, `internal_alpha_remote_smoke.ts` healthcheck, Auth anonimo dev, email/senha com saves normal/Lab e `DRAXOS_REMOTE_RELEASE_SMOKE`; `supabase migration list` alinhado; T03-P16 passou em `smoke_exports.gd` e `tools/export_internal_alpha.ps1` gerando APK/PC ZIP/Web com hashes |
...
1. Executar `T03-P17` publicacao unlisted e QA remoto fechado com Fabio + 1 amigo.
2. Depois do QA, seguir `T03-P18` handoff da Internal Alpha v0.
