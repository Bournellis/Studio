...
| Internal Alpha v0 | Design lock completo + T03-P02 repo-ready | Track 03 documentada com escopo, plano, runbook, checklist e design lock. `DMOB-D048` a `DMOB-D055` resolvidas; valores numericos da loja ficam como dados calibraveis do catalogo/seed. Configuracao segura do backend remoto preparada no cliente/docs/testes; Supabase remoto real ainda precisa ser criado/configurado |
...
- `online/supabase_client.gd` implementa HTTPRequest para Auth, `account/*`, `battle/*`, `base/*`, `social/*`, `competition/*`, `monetization/*` e `telemetry/*` em Supabase local/remoto via `BackendConfig`.
...
- GUT integrado: `44/44` testes, `264` asserts.
...
1. Criar/configurar o projeto Supabase remoto real e rodar `server/tests/internal_alpha_remote_smoke.ts`.
2. Executar `T03-P03 - Conta Email/Senha E Dois Saves`.
