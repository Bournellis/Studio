...
| Internal Alpha v0 | T03-P03B completo; proximo T03-P03C | Track 03 documentada com escopo, plano, runbook, checklist e design lock. `DMOB-D048` a `DMOB-D055` resolvidas; valores numericos da loja ficam como dados calibraveis do catalogo/seed. Configuracao segura do backend remoto preparada no cliente/docs/testes; save `normal`/`progression_lab` ja existe no cliente e no Supabase local; Supabase remoto real, builds e update manifest ficam adiados ate o gameplay local estar pronto |
...
| Godot project | Alpha PC local pronto + save context server-backed + Battle Lab/Progression Lab dev | Hub alpha hardenizado, autoloads, `.gutconfig.json`, content generator, catalogo gerado, `SessionStore` com `session_id`, `active_save_type`, `apply_snapshot_cache` e metadados `progression_lab`, `SupabaseClient` com telemetria e header ativo `x-draxos-save-type`, `BattleLogPresenter`, `BattleVisualMockup`, `BattleStage2D` responsivo com tooltips imediatos, nos estaveis durante efeitos, cooldown restante por relogio continuo de replay e feedback textual com nomes completos, telas dev-only do Battle Lab e Progression Lab, cache local-only read-only sem token valido, runner Deno sanitizado/Windows-safe, smoke real de labs, smoke visual/comportamental e GUT |
| Supabase project | Conta guest + dois saves locais + battle MVP + first-slice sim + base/social/competicao/monetizacao/telemetria v0 prontos | Layout `supabase/`, migrations MVP/base/social/ranking/monetizacao/`save_type`, Auth anonimo, healthcheck, `account/*`, `battle/*`, `base/*`, `social/*`, `competition/*`, `monetization/*`, `telemetry/*`, seeds `FIRST_SLICE`, JWT config de funcoes, simulador compartilhado e helper `_shared/save_context.ts` configurados |
| Validacao | Verde tecnico; tuning manual pendente | Godot validate + GUT `46/46`, `280` asserts passam; smoke session shell passa; `supabase db reset` passa com migration `202605260001_two_save_context.sql`; Deno checks das funcoes Supabase/server passam; smoke `two_save_context_smoke.ts` cria saves distintos para a mesma sessao Auth; smokes existentes de batalha, base, social/competicao, monetizacao e telemetria passam; smoke de labs executa Battle Lab/Progression Lab via `OS.execute`; smoke visual/comportamental passa e valida cache local-only sem token online; smoke de exports passa; Supabase remoto real ainda precisa projeto/credenciais e smoke contra URL real |
...
- `online/supabase_client.gd` implementa HTTPRequest para Auth, `account/*`, `battle/*`, `base/*`, `social/*`, `competition/*`, `monetization/*` e `telemetry/*` em Supabase local/remoto via `BackendConfig`, enviando `x-draxos-save-type` para selecionar `normal` ou `progression_lab`.
...
- `modes/boot/boot.gd` apresenta um hub alpha com abas para Batalha, Base, Social, Competicao e Loja, areas rolaveis, Voltar/Esc, confirmacoes simples para mutacoes, feedback visivel para sucesso/erro/pre-condicao, busy states, refresh, reset seguro de cache local, alternancia entre save normal e save Progression Lab, bloqueio de acoes online apenas quando o Lab esta em cache local-only, mockup visual compartilhado na aba Batalha e telemetria de telas/acoes/replay/offline.
...
- `supabase/functions/account/` implementa `POST /account/guest` e `GET /account/state` com JWT anonimo, service role interno, idempotencia por `request_id` e filtro por `x-draxos-save-type`.
...
- `npx -y supabase db reset`: passou aplicando `202605260001_two_save_context.sql`.
...
- Smokes Supabase existentes de batalha, base, social/competicao, monetizacao e telemetria: passaram apos `save_type`.
...
- GUT integrado: `46/46` testes, `280` asserts.
...
1. Executar `T03-P03C - reset separado por save`.
2. Implementar aplicacao do Progression Lab no save `progression_lab`.
3. Expandir Base/Social/Competicao/Loja com o isolamento de save ja validado.
