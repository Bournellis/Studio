...
- Active Surface: `Battle presentation rework integrated`
- Active Track: `Track 10 - Battle Presentation Rework`
- Active Track Status: `INTEGRATED_BATTLE_PRESENTATION_READY`
...
- Track 09C Refugio Game Menu Rework 2026-05-28: Fabio pediu um Refugio com menus de jogo reais em vez de lista/painel. O Refugio agora renderiza `RefugeSceneBoard` fullscreen portrait com cena procedural do altar, HUD compacto, icones grandes para Batalha/Refugio/Social/Competicao/Loja/Perfil, quick actions de Coletar/Energia e `RefugeMenuPopup` para cada menu. A experiencia normal nao usa mais `Caminhos do Refugio`/`RefugePathGrid`; o conteudo existente abre em drawers/popups preservando actions e rotas. Guardrails preservados: sem backend, schema, economia, contratos HTTP, assets finais ou publicacao remota.
- Track 10 Battle Presentation Rework 2026-05-28: Fabio pediu reformulacao completa da apresentacao da batalha no novo app portrait. `battle_running` agora usa fullscreen sem chrome, palco limpo com `BattleStage2D` via `BattleVisualMockup` em modo `stage_only`, sem header externo, timeline ou cards fora do palco, e `Pular batalha` como acao grande no canto inferior direito do layout. `battle_summary` virou resultado minimo com `Ver logs` e `Voltar ao Refugio`; `battle_logs` mostra os eventos textuais da batalha atual em tela propria com `Voltar ao Resultado` e `Voltar ao Refugio`. Guardrails preservados: sem backend, schema, simulador, `battle_log_v1`, recompensa, ranking, economia, assets finais ou publicacao remota.
- Release Update 2026-05-28: builds publicadas atualizadas para site/Web/APK/Windows a partir de worktree limpa `codex/draxos-mobile/release-update-builds`. Artefatos: Android APK `27,965,106` bytes SHA-256 `ad6d2579ce003769cfce2536b788c1330abb283d0ae90cc785d1d016ae514ca6`, PC Windows ZIP `36,466,312` bytes SHA-256 `ad5fb8351bb001604479d95737fc702bb9b0ff6779afb9e3e31692b7bc189031`, Web index `5,442` bytes SHA-256 `75fdd260b889582cb723256e87ca9867ae35b7cdd3411cbb2ca21ace5585366a`. Supabase Storage/manifest e Edge Functions `battle`/`release` foram atualizados; Cloudflare Pages deploy concluiu em `https://36b1d46c.draxos-mobile-internal-alpha.pages.dev`. Observacao: o dominio principal `https://draxos-mobile-internal-alpha.pages.dev` esta protegido por Cloudflare Access e responde com tela de login para checagem publica anonima.
...
| Track 09 | Integrada; Refugio como menu de jogo portrait pronto para walkthrough manual | Track 09 separou `entry` operacional de `refuge` jogavel, tornou o app portrait-first e converteu a batalha para portrait fullscreen. Track 09B enxugou Entry e corrigiu o fluxo slim. Track 09C substituiu a lista `Caminhos do Refugio` por uma cena fullscreen com altar procedural, icones de menu e popups/drawers por area, mantendo Refugio/Base, Social, Competicao, Loja, Perfil, Batalha, Coletar e Energia acessiveis por icones. Sem backend/schema/economia/ranking/recompensas/simulador/publicacao remota. |
| Track 10 | Integrada; batalha portrait com palco limpo e builds publicadas para QA manual | Track 10 reformulou `battle_running` como fullscreen sem chrome com `BattleStage2D` focado no duelo e `Pular batalha` no canto inferior direito, simplificou `battle_summary` para resultado minimo e adicionou `battle_logs` para eventos textuais da batalha atual. Builds site/Web/APK/Windows republicadas em 2026-05-28; Cloudflare Pages esta atras de Cloudflare Access para acesso anonimo. |
...
| Validacao | Track 10 + release update com matriz verde | No patch Track 10 de 2026-05-28, `tools/validate.gd` passou com GUT `98/98` testes e `1208` asserts; `smoke_mobile_presentation.gd`, `smoke_foundation_hardening.gd`, `smoke_exports.gd`, checks Deno de `supabase/functions` e `server/functions`, `smoke_battle_replay.gd` remoto, `release_manifest_smoke.ts`, `internal_alpha_remote_smoke.ts` com release manifest e `git diff --check` passaram. Downloads APK/ZIP e Web asset em Supabase Storage responderam HTTP 200. |
