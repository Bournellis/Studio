...
| Validacao | Track 06 integrada com matriz verde | Na integracao Track 06 de 2026-05-27, `tools/validate.gd` e GUT passaram com `73/73` testes e `843` asserts; `smoke_session_shell.gd`, `smoke_runtime_config.gd`, `smoke_battle_replay.gd`, `smoke_foundation_surfaces.gd`, `smoke_dev_labs.gd`, `smoke_dev_lab_ui.gd` e `smoke_exports.gd` passaram; checks Deno de `supabase/functions` e `server/functions` passaram; `server/tests/battle_history_replay_smoke.ts` passou em `deno check`; `git diff --check` passou. Baselines anteriores de Track 03/04/05 e smokes remotos permanecem documentados nos relatorios/handoffs correspondentes. |
