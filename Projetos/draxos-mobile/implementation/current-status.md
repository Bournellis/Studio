...
- Track 06 Feature Installation Update 2026-05-27: Track 06 foi aberta como fase ativa sobre a fundacao Track 05. T06-B consolidou o contrato padrao de instalacao de feature em `tracks/track-06-feature-installation-rails-and-first-slices/feature-registry.md`: toda feature declara owner, surface, endpoints afetados, escopo de servico, smoke/GUT obrigatorio, fallback e rollback antes de runtime. T06-E Battle History esta pronta para integracao: endpoints read-only save-scoped `GET /battle/history` e `GET /battle/replay?battle_id=...`, UI de historico/replay salvo na aba Batalha e smokes/GUT focados, sem rerodar simulador nem reaplicar recompensa. Objetivo da track: instalar primeiros slices visiveis de baixo risco: runtime config, perfil/conta, historico de batalhas, rotina da Base, Social QoL e Asset Pack 01. Guardrails: sem tuning numerico, sem migration `account_profiles` + `game_saves`, sem pagamento real, sem iOS/mobile browser, sem social realtime e sem publicacao remota.
...
| Track 06 | Ativa; T06-E Battle History pronto para integracao | Track 06 instala features sobre a fundacao Track 05. T06-B define registry, template, checklist por feature, validacao por surface, fallback e rollback. T06-E entrega historico/replay read-only save-scoped na Batalha, com endpoints e UI sem mudar simulador/recompensa/schema. Proximos slices paralelos: runtime config, perfil/conta, rotina da Base, Social QoL e Asset Pack 01. Sem tuning numerico, sem migration conta/save, sem pagamento real, sem realtime social e sem publicacao remota. |
