...
- Active Track Status: `T03-P17A_SIGNOFF_APPROVED - T03-P18_READY`
- Release Prep Update: T03-P12 registrou a ordem `T03-P12` a `T03-P18`, criou a base do portal estatico em `portal/internal-alpha/`, criou manifest exemplo e tutorial detalhado para o Supabase remoto observado (`armxgipvnbbshzqawklw`, `https://armxgipvnbbshzqawklw.supabase.co`). T03-P13 concluiu bootstrap remoto: CLI linkado, migrations aplicadas, Edge Functions publicadas, migrations locais/remotas alinhadas e smoke minimo de healthcheck verde. T03-P14 concluiu auth email/senha: cliente Godot, `/account/bootstrap`, migration `create_alpha_account`, config Auth remoto sem confirmacao obrigatoria e smokes local/remoto verdes. T03-P15 concluiu manifest remoto/version gate: `GET /release/manifest` publicado local/remoto, Hub checa update no boot e bloqueia acoes online quando a versao minima exigir. T03-P16 concluiu exports locais Android/PC/Web com hashes registrados. T03-P17 publicou APK/PC ZIP em Supabase Storage unlisted, publicou Portal/Web no Cloudflare Pages, atualizou o manifest remoto com links finais e passou no QA remoto automatizado. T03-P17A aplicou uma passada curta de ergonomia Android no Hub/abas, gerou novos artefatos locais, foi aprovada por Fabio, republicou APK/PC ZIP/manifest/Cloudflare Pages e passou na validacao remota final. Em 2026-05-27, Fabio aprovou o signoff para avancar; proximo passo e `T03-P18`.
- Site protection/upgrade 2026-05-27: worktree `codex/draxos-mobile/site-protection` preparou portal com login Supabase para downloads, endpoint `GET /release/download`, migration do bucket privado `draxos-internal-alpha-private`, tutorial operacional e script de publicacao com downloads protegidos por padrao. No remoto, `202605270003_internal_alpha_private_downloads.sql` foi aplicada, `release` foi redeployada, APK/PC ZIP foram publicados no bucket privado, manifest remoto agora aponta APK/PC para `/release/download` com `auth_required`, `/release/download` rejeita chamadas sem JWT e Cloudflare Access foi configurado/validado no dominio estavel para tornar Portal/Web privados. O upgrade do site publicou o hub alpha privado em abas com arte local, Downloads, Web, Guia, Status e Grimorio; `GET /content/grimoire` foi gerado a partir de `data/definitions/*.json`, espelhado em `supabase/functions` e `server/functions`, deployado no Supabase remoto e validado por smoke local/remoto. Hotfix posterior corrigiu a normalizacao da URL assinada de Storage para APK/PC e ativou `html/experimental_virtual_keyboard=true` no export Web. Ajuste final deixou o site sem login/acesso de jogo: Cloudflare Access por email e a unica barreira do hub, downloads usam links diretos do pacote publicado, a Web build fica visivel para usuarios autorizados, o Grimorio usa `portal/assets/grimoire-catalog.json` e registro/login ficam apenas in-game; o fluxo Supabase do site permanece escondido para uso futuro. Hotfix de tela preta reuploadou `web/index.js` ausente e `web/index.pck` stale no Supabase Storage, e o script de publicacao passou a validar tamanhos exatos dos assets Web principais. Cloudflare Pages redeployado em `https://4b709bb0.draxos-mobile-internal-alpha.pages.dev`.
- Site visual upgrade 2026-05-28: portal Internal Alpha recebeu assets otimizados derivados de `assets/referenciaimagens/`, hero full-width com sala ritual Draxos, cards tematicos para Inicio/Downloads/Web, Grimorio com background proprio, moldura/sigilos por categoria e validacao visual local desktop/mobile no pacote Cloudflare Pages.
...
| Internal Alpha v0 | T03-P17A aprovada; T03-P18 pronto | Track 03 documentada com escopo, plano, runbook, checklist, design lock, plano de release e tutorial remoto. `DMOB-D048` a `DMOB-D055` resolvidas; valores numericos da loja ficam como dados calibraveis do catalogo/seed. Configuracao segura do backend remoto preparada no cliente/docs/testes; save `normal`/`progression_lab` existe no cliente/Supabase, com reset separado, aplicacao server-backed do Progression Lab, Base Manager jogavel, Social basico jogavel, Competicao/leaderboard alpha, Loja proof-of-concept, Batalha visual polida e QA local automatizado verde; Supabase remoto `armxgipvnbbshzqawklw` linkado, migrado, com functions publicadas, Auth email/senha configurado, `release/manifest`, `release/download` e `content/grimoire` publicados, Storage publico `draxos-internal-alpha` ativo para os downloads do portal atual e Storage privado `draxos-internal-alpha-private` preservado para downloads autenticados futuros. Portal/Web publicados no Cloudflare Pages em `https://draxos-mobile-internal-alpha.pages.dev` com Cloudflare Access validado; site sem login/acesso de jogo visivel, downloads diretos para usuarios da allowlist, Web build visivel no hub e Grimorio via JSON estatico do portal; assets Web do Storage reconciliados apos hotfix de tela preta; upgrade visual local de 2026-05-28 aproxima o hub do tema Draxos com imagens reais e assets otimizados. Fabio aprovou seguir para `T03-P18` em 2026-05-27. |
...
| Validacao | Verde tecnico local + downloads/manifest/Cloudflare/Grimorio/Web remotos verdes | T03-P11 resetou cache Godot, scratch Progression Lab e Supabase local. Em 2026-05-27, Godot validate + GUT passou com `54/54`, `367` asserts apos o teste de UI compacta; `supabase db reset` passou com `202605270001_alpha_email_account.sql`; checks/lints Deno das funcoes Supabase/server passaram incluindo `release` e `content`; smokes locais cobrem batalha MVP/FIRST_SLICE, dois saves, reset isolado, Progression Lab apply, Base, Social/Competicao, Monetizacao, Telemetria, email/senha alpha, manifest e Grimorio; remoto T03-P13/T03-P15 passou com `supabase db push`, deploy das Edge Functions, `supabase config push --yes`, `internal_alpha_remote_smoke.ts` healthcheck, Auth anonimo dev, email/senha com saves normal/Lab e `DRAXOS_REMOTE_RELEASE_SMOKE`; `supabase migration list` alinhado; T03-P16 passou em `smoke_exports.gd` e `tools/export_internal_alpha.ps1`; T03-P17 passou em `tools/publish_internal_alpha.ps1`, `release_manifest_smoke.ts`, `internal_alpha_remote_smoke.ts`, `first_slice_battle_smoke.ts`, `base_manager_smoke.ts`, `monetization_rewards_smoke.ts`, `social_competition_smoke.ts`, `battle_request_smoke.ts` e `client_telemetry_smoke.ts` contra remoto; T03-P17A passou em `validate.gd`, `smoke_exports.gd`, rebuild local Android/PC/Web, manifest/downloads remotos, `release_manifest_smoke.ts` remoto, `internal_alpha_remote_smoke.ts` com release, deploy Wrangler para `https://a2383707.draxos-mobile-internal-alpha.pages.dev` e comparacao HTML local/remoto de Portal/Web no preview e dominio estavel. O upgrade do site passou em `deno task check/lint` para `supabase/functions` e `server/functions`, `grimoire_catalog_smoke.ts` local/remoto, `release_manifest_smoke.ts` local/remoto, `release_download_smoke.ts` remoto com HEAD da URL assinada, download sem JWT `401`, Web export com `experimentalVK:true`, pacote Cloudflare sem placeholders, asset local copiado, `grimoire-catalog.json` empacotado e verificacao visual desktop/mobile por Playwright. Hotfix Web validou HEAD remoto com `index.js` 315759 bytes, `index.pck` 415576 bytes e `index.wasm` 37695054 bytes, parser do publish, `git diff --check`, `publish_internal_alpha.ps1 -SkipUpload -SkipDeploy -PublicDownloads` com env local e render do `/web/` no navegador sem erros de console. Upgrade visual 2026-05-28 passou em `publish_internal_alpha.ps1 -SkipUpload -SkipDeploy -PublicDownloads`, `build_cloudflare_pages_package.ps1`, checagem de placeholders/assets, e verificacao visual desktop/mobile do portal e Grimorio no navegador local sem erros de console. |
...
1. Executar `T03-P18` handoff da Internal Alpha v0.
