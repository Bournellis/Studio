...
- Active Surface: `Foundation review and hardening`
- Active Track: `Track 08 - Foundation Review And Hardening`
- Active Track Status: `ACTIVE_FOUNDATION_HARDENING`
...
- Track 08 Foundation Update 2026-05-27: Track 08 foi aberta como `ACTIVE_FOUNDATION_HARDENING` para endurecer a fundacao pos-Track 07 antes de novas features/assets/servicos maiores. Escopo: contratos internos de app shell, session/save boundary, mobile UI contract, battle mode contract, checks leves de service/assets e smoke final de hardening. Guardrails: sem gameplay novo, tuning, backend endpoint novo, schema, migration `account_profiles` + `game_saves`, assets finais ou publicacao remota.
- Track 08 T08-D Mobile UI Contract Update 2026-05-27: T08-D esta pronta em `codex/draxos-mobile/t08-mobile-ui-contract`. Entregou helper comum para alvo minimo de toque, drag threshold, politica de scrollbar/touch e layout portrait/landscape, com `DraxosTouchScrollContainer`, `boot.gd` e botoes existentes da shell/base reutilizando o contrato. Validacao local passou com GUT client `85/85` testes e `985` asserts, `smoke_mobile_presentation.gd` e `tools/validate.gd`. Sem redesign, assets, backend/schema, tuning ou gameplay novo.
- Track 08 T08-C Session/Save Boundary Update 2026-05-27: T08-C endureceu `SessionStore`/`SupabaseClient` sem mudar Auth, contratos HTTP, schema, `players.save_type` ou payload publico. Entregas: contexto interno `_client.save_type` em respostas save-scoped, rejeicao de resposta stale apos troca de save, `surface_save_types` por snapshot de account/base/social/competition/monetization/battle, cache local-only do Progression Lab sem token valido, reset do Lab limpando metadados antigos, fallback runtime config preservado e `diagnostics_snapshot()` sem secrets. Validacao final desta worktree fica registrada no Doing T08-C.
- Track 08 T08-F Update 2026-05-27: T08-F esta pronta para integracao com checagem Deno sem rede para matriz de endpoints e feature registry, alem de GUT focado para estabilidade de `AssetIds` e fallback de arte opcional. Sem endpoint, schema, migration, servico novo, asset final ou mudanca de runtime de produto.
...
| Track 08 | Ativa; hardening de fundacao em andamento | Track 08 revisa e endurece a fundacao pos-Track 07: contratos de rota/back/orientacao, session/save/cache/runtime config, touch/scroll/layout, battle mode, service/asset checks e validation harness. T08-B, T08-C, T08-D e T08-F ja foram integradas na worktree T08-H; sem gameplay novo, tuning, schema, endpoints publicos, assets finais ou publicacao remota. |
...
| Validacao | Track 07 integrada com matriz verde | Na integracao Track 07 de 2026-05-27, `tools/validate.gd` e GUT passaram com `85/85` testes e `968` asserts; `smoke_session_shell.gd`, `smoke_foundation_surfaces.gd`, `smoke_mobile_presentation.gd`, `smoke_exports.gd` e `smoke_battle_replay.gd` passaram; `smoke_battle_replay.gd` foi validado apontando `BATTLE_FUNCTION_URL` para a funcao battle atual servida a partir do worktree de integracao porque a Edge Runtime local padrao ainda estava servindo uma funcao antiga. `git diff --check` passou. Baselines anteriores de Track 03/04/05/06 e smokes remotos permanecem documentados nos relatorios/handoffs correspondentes. |
...
- `modes/boot/boot.gd` apresenta um app shell mobile-first com Refugio full screen como home, rotas/back stack, telas internas para Conta, Base, Social, Competicao e Loja, scroll/touch foundation com alvo maior, Voltar/Esc, confirmacoes simples para mutacoes, feedback visivel para sucesso/erro/pre-condicao, busy states, refresh, reset seguro de cache local, reset perigoso apenas do save ativo, alternancia entre save normal e save Progression Lab, bloqueio de acoes online apenas quando o Lab esta em cache local-only, mockup visual compartilhado, autobattler full-screen landscape com skip/summary, Base jogavel, Social jogavel, Competicao jogavel e Loja proof-of-concept com redeems diarios, compras por Diamante, paineis de catalogo/recompensas/status e tooltips, alem de telemetria de telas/acoes/replay/offline. Track 08 deve endurecer os contratos internos desse shell sem grande reescrita.
...
- Track 07 final: `tools/validate.gd` e GUT completo passaram com `85/85` testes e `968` asserts; `tools/smoke_session_shell.gd`, `tools/smoke_foundation_surfaces.gd`, `tools/smoke_mobile_presentation.gd`, `tools/smoke_exports.gd` e `tools/smoke_battle_replay.gd` passaram. Para `smoke_battle_replay.gd`, a funcao `battle` atual foi servida localmente a partir do worktree de integracao via `BATTLE_FUNCTION_URL` porque a Edge Runtime padrao ainda apontava para uma funcao antiga sem `/battle/history`.
...
1. Integrar `T08-A` no `master` como base comum de documentacao/coordenação.
2. Executar `T08-B`, `T08-C`, `T08-D` e `T08-F` em worktrees paralelas.
3. Executar `T08-E` depois de `T08-B`, `T08-G` depois de `T08-B` a `T08-F`, e fechar `T08-H` com validacao final.
