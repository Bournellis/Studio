...
| Internal Alpha v0 | T03-P13 remoto bootstrap completo | Track 03 documentada com escopo, plano, runbook, checklist, design lock, plano de release e tutorial remoto. `DMOB-D048` a `DMOB-D055` resolvidas; valores numericos da loja ficam como dados calibraveis do catalogo/seed. Configuracao segura do backend remoto preparada no cliente/docs/testes; save `normal`/`progression_lab` existe no cliente e no Supabase local, com reset separado, aplicacao server-backed do Progression Lab, Base Manager jogavel, Social basico jogavel, Competicao/leaderboard alpha, Loja proof-of-concept, Batalha visual polida e QA local automatizado verde; portal base criado em `portal/internal-alpha/`; Supabase remoto `armxgipvnbbshzqawklw` linkado, migrado, com functions publicadas e healthcheck verde; proximo passo e T03-P14 auth email/senha |
...
| Validacao | Verde tecnico local + remoto minimo | T03-P11 resetou cache Godot, scratch Progression Lab e Supabase local. Godot validate + GUT `49/49`, `320` asserts passam; smoke session shell/replay/alpha loop/dev labs/dev lab UI/export presets passam; `supabase db reset` passa com migrations `202605260001_two_save_context.sql`, `202605260002_reset_save_context.sql` e `202605260003_progression_lab_apply.sql`; Deno checks/lint das funcoes Supabase/server passam; smokes locais cobrem batalha MVP/FIRST_SLICE, dois saves, reset isolado, Progression Lab apply, Base, Social/Competicao, Monetizacao e Telemetria; `deno test` de Battle Lab/Progression Lab/simulador passa `20/20`; remoto T03-P13 passou com `supabase db push`, deploy das 9 Edge Functions, `internal_alpha_remote_smoke.ts` healthcheck e `supabase migration list` alinhado |
...
1. Executar `T03-P14 - Auth Email/Senha E Alpha Gate` no Godot/backend remoto.
2. Depois do login real, seguir `T03-P15` manifest de updates.
3. Depois do manifest, seguir `T03-P16` exports Android/PC/Web.
