...
| Internal Alpha v0 | T03-P04 completo; proximo T03-P05 | Track 03 documentada com escopo, plano, runbook, checklist e design lock. `DMOB-D048` a `DMOB-D055` resolvidas; valores numericos da loja ficam como dados calibraveis do catalogo/seed. Configuracao segura do backend remoto preparada no cliente/docs/testes; save `normal`/`progression_lab` existe no cliente e no Supabase local, com reset separado e aplicacao server-backed do Progression Lab validados; Supabase remoto real, builds e update manifest ficam adiados ate o gameplay local estar pronto |
...
- `supabase/functions/base/` implementa `GET /base/state`, `POST /base/collect` e `POST /base/upgrade` com estruturas permanentes, fila de construcao, conclusao de jobs vencidos, coleta offline, ledger, idempotencia por `request_id` e filtro por `x-draxos-save-type`.
...
- Smokes Supabase existentes de batalha, first-slice, base, social/competicao, monetizacao e telemetria: passaram apos reset de save.
...
- GUT integrado: `48/48` testes, `303` asserts.
...
1. Executar `T03-P05 - Base Manager jogavel`.
2. Expandir Social/Competicao/Loja com o isolamento de save ja validado.
