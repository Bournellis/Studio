...
- Active Track Status: `ACTIVE_FEATURE_INSTALLATION`
...
- Track 06 Feature Installation Update 2026-05-27: Track 06 foi aberta como fase ativa sobre a fundacao Track 05. T06-B consolidou o contrato padrao de instalacao de feature em `tracks/track-06-feature-installation-rails-and-first-slices/feature-registry.md`: toda feature declara owner, surface, endpoints afetados, escopo de servico, smoke/GUT obrigatorio, fallback e rollback antes de runtime. Objetivo da track: instalar primeiros slices visiveis de baixo risco: runtime config, perfil/conta, historico de batalhas, rotina da Base, Social QoL e Asset Pack 01. Guardrails: sem tuning numerico, sem migration `account_profiles` + `game_saves`, sem pagamento real, sem iOS/mobile browser, sem social realtime e sem publicacao remota.
- Track 06 Runtime Config Update 2026-05-27: T06-C esta `READY_FOR_INTEGRATION` em `codex/draxos-mobile/t06-runtime-config`. Entregou `GET /release/config` com `runtime_config_v1` release-scoped/read-only, flags T06 allowlisted, cliente Godot com fallback conservador e smokes/GUT focados. Sem schema, tuning, secrets, service role, estado de jogador, publicacao remota ou mutacao de gameplay.
- Track 06 Profile/Account Update 2026-05-27: T06-D esta `READY_FOR_INTEGRATION` em `codex/draxos-mobile/t06-profile-account`. Entregou painel Perfil/Conta read-only usando `SessionStore`, snapshot `account/state`, save ativo, username, level, poder, auth method, update state e status alpha, sem endpoint novo e sem mudar Auth/schema/economia.
- Track 06 T06-G Social QoL Update 2026-05-27: branch `codex/draxos-mobile/t06-social-qol` pronta para integracao. Social segue client-only e render-only: o presenter agora destaca refresh/polling, identidade da conta, amigos, guilda, estados vazios e mensagens atuais com formatting legivel. `smoke_foundation_surfaces.gd` valida guilda/membros/estruturas e mensagem atual retornada por polling. Sem endpoint novo, schema, realtime, moderacao, ranking behavior, backend ou vazamento do Progression Lab para leaderboard.
- Track 06 Asset Pack 01 Update 2026-05-27: T06-H esta `READY_FOR_INTEGRATION` em `codex/draxos-mobile/t06-asset-pack-01`. Entregou PNGs leves para UI, portraits e `battle_icon_*`, com hook opcional em `BattleSymbolIcon`/`BattleStage2D`; missing art e fallback nativo continuam permitidos. Sem backend, schema, economia, tuning, gameplay ou publicacao remota.
- Track 06 Battle History Update 2026-05-27: T06-E esta `READY_FOR_INTEGRATION` em `codex/draxos-mobile/t06-battle-history`. Entregou endpoints read-only save-scoped `GET /battle/history` e `GET /battle/replay?battle_id=...`, UI de historico/replay salvo na aba Batalha e smokes/GUT focados, sem rerodar simulador nem reaplicar recompensa.
...
| Track 06 | Ativa; todos os slices C-H em integracao T06-I | Track 06 instala features sobre a fundacao Track 05. T06-B define registry, template, checklist por feature, validacao por surface, fallback e rollback. T06-C runtime config, T06-D Perfil/Conta, T06-E Battle History, T06-F Base Routine, T06-G Social QoL e T06-H Asset Pack 01 estao integrados na branch T06-I para validacao final. Sem tuning numerico, sem migration conta/save, sem pagamento real, sem realtime social e sem publicacao remota. |
...
| Validacao | Track 05 integrada com matriz verde | Na integracao Track 05 de 2026-05-27, `tools/validate.gd` e GUT passaram com `63/63` testes e `696` asserts; `smoke_session_shell.gd`, `smoke_battle_replay.gd`, `smoke_foundation_surfaces.gd`, `smoke_dev_labs.gd`, `smoke_dev_lab_ui.gd` e `smoke_exports.gd` passaram; checks Deno de `supabase/functions` e `server/functions` passaram; `release_artifacts_remote_smoke.ts` passou em `deno check` e `deno lint`; `git diff --check` passou. Baselines anteriores de Track 03/04 e smokes remotos permanecem documentados nos relatorios/handoffs correspondentes. |
