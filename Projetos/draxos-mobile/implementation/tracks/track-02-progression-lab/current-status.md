...
- Status: `IN_PROGRESS - TOOLING IMPLEMENTED`
- Baseline: Track 00 completa, Track 01 alpha PC local pronta, Battle Lab e simulador economico existentes. Progression Lab v1 agora gera saves saudaveis, relatorios, bot pool, seeder local e fluxo manual dev-only no Godot.
...
- `npx -y deno run --allow-read --allow-write tools/progression_lab/generate.ts`: `25` saves, `75` bots, status `REVIEW`, `10` itens de review.
...
Rodar o seeder contra Supabase local com `SUPABASE_SERVICE_ROLE_KEY`, carregar manualmente pelo Godot os saves `2h`, `5h`, `10h`, `15h` e `20h`, registrar feedback e abrir rodada before/after para custo, recompensa, poder e bots.
