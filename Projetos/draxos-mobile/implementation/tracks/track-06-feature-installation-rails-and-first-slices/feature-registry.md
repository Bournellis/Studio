...
- Service contract notes: no JWT required; no `x-draxos-save-type`; no idempotency; read-only; response must not include secrets, service-role data, mutable gameplay state or release publication controls.
- Client files: Godot read path and fallback state to be declared by T06-C.
- Backend files: `supabase/functions/release`, `server/functions/release`, endpoint smoke to be declared by T06-C.
- Smoke required: runtime config smoke.
- GUT required: runtime config read path/fallback GUT if Godot client is touched.
- Other validation: Deno checks, `validate.gd`, GUT client, `git diff --check`.
- Fallback: client defaults to conservative disabled flags when endpoint fails or payload version is unknown.
- Rollback: disable client read path/flag use and remove `GET /release/config` handler without changing `GET /release/manifest`.
- Guardrail notes: no remote publication, no manifest mutation, no gameplay tuning, no secrets.
- Handoff notes: T06-I must verify config defaults before integrating feature slices behind flags.
