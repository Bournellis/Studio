...
- Track 03 com design lock, T03-P02 repo-ready e T03-P03B completo: Internal Alpha v0 documentada para email/senha, dois saves por conta, Supabase remoto Free, plano de saida para Backend Proprio + Postgres, Progression Lab isolado, Base/Social/Competicao/Loja jogaveis, leaderboard sem bots, redeems diarios em Diamante, manifest de updates e playtest fechado Fabio + 1 amigo. A ordem atual e local-first: implementar no Godot/Supabase local antes de remoto/builds. O cliente ja possui `BackendConfig`, ambiente `internal_alpha_v0`, env vars seguras, smoke remoto, save ativo `normal`/`progression_lab`, cache/HUD de save e o Supabase local ja resolve `save_type` server-side em todos os endpoints alpha.
...
| Objetivo | Status |
|---|---|
| Build fechada Internal Alpha v0 com conta email/senha, dois saves, backend remoto, updates e features principais funcionais | T03-P03B completo; proximo T03-P03C reset separado |
