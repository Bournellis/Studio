...
func test_catalog_contains_starter_deck_and_reward() -> void:
	...
	assert_eq(catalog.starter_deck_ids.size(), 10)
	assert_eq(catalog.reward_card_id, "improvised_ballista")
...
func test_npc_reward_unlocks_one_extra_card_once() -> void:
	...
	assert_eq(GameSession.unlocked_card_ids.size(), 10)

	var reward_id: String = GameSession.claim_npc_reward()
	assert_eq(reward_id, "improvised_ballista")
	assert_true(GameSession.has_npc_reward_card)
	assert_eq(GameSession.unlocked_card_ids.size(), 11)

	GameSession.claim_npc_reward()
	assert_eq(GameSession.unlocked_card_ids.size(), 11)

func test_deck_rules_require_exactly_ten_unlocked_cards() -> void:
	...
	var short_deck: Array = GameSession.selected_deck_ids.slice(0, 9)
	...
	locked_deck[0] = "improvised_ballista"
