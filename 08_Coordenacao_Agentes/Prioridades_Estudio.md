...
- Foco P2 de implementacao (`BOSQUE_MECANICO_BASICO_V2_PUBLISHED_INTERNAL_ALPHA`, release root `internal-alpha/v0-bosque-v2-guidance-20260604-7c2d981`, official Portal URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`, deployment evidence `https://ae049df9.draxos-mobile-internal-alpha.pages.dev`; com integracao local ainda nao publicada `Openworld Collection Sync Local Fix` + `Main Menu Refactor` ja incorporada ao `master` em `1c72399`; preservando First Access Runtime Fix `4608977`, Integrated Runtime Fix `ab5834c`, Integrated App/Arena/Bosque `99304ed`, Web Launch Resilience, Refugio Visual Cleanup, Openworld QoL regression fix e Foundation Hardening V2 (`FOUNDATION_HARDENING_V2_PUBLISHED_INTERNAL_ALPHA`, release root `internal-alpha/v0-foundation-hardening-v2-hotfix2-20260601-58671a4`, preview `https://ca946749.draxos-mobile-internal-alpha.pages.dev`) como baselines anteriores, Track 21 Arena Loop Unlock/Friction como contexto do Autobattler, Track 20 Season 1 Arena Calibration, Remote Lab Runner, Track 13 release safety e Track 14 agent ops): `Projetos/draxos-mobile/`
...
| P2 | DraxosMobile | `Projetos/draxos-mobile/` | Implementacao - `BOSQUE_MECANICO_BASICO_V2_PUBLISHED_INTERNAL_ALPHA` | `P2_IMPLEMENTACAO` | Codigo, design, documentacao local, configuracao de infraestrutura | Revisar a integracao Openworld Collection Sync + Main Menu Refactor ja incorporada ao `master` e decidir publicacao/hotfix antes de ampliar playtest humano do Bosque v2 publicado | Bosque Mecanico Basico v2 publicado em 2026-06-04 no release root `internal-alpha/v0-bosque-v2-guidance-20260604-7c2d981`, official Portal URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`, deployment evidence `https://ae049df9.draxos-mobile-internal-alpha.pages.dev`; guidance opcional persistida no save normal, recursos fixos com folga, `Encerrar visita` sem objetivo obrigatorio e `fogueira_estavel_1` visual/bloqueante. Integracao local posterior `Openworld Collection Sync Local Fix` + `Main Menu Refactor` incorporada ao `master` em `1c72399` corrige rollback apos coleta e simplifica o menu principal, mas ainda nao foi publicada e nao altera o baseline remoto. `supabase db push`, `supabase functions deploy modes`, Storage upload, Cloudflare Pages, manifest remoto, RemoteReadOnly e smoke Web no preview passaram na publicacao Bosque v2; smoke Web carregou o jogo em 3699 ms com screenshot `build/diagnostics/web-launch-remote-20260604-194152/web-launch-remote.png`; production fixo anonimo retorna Cloudflare Access como esperado. Android APK usa `debug_fallback`, aceito para playtest funcional, com release signing adiado para distribuicao Android mais ampla. First Access Runtime Fix `4608977`, Integrated Runtime Fix `ab5834c`, Integrated App/Arena/Bosque `99304ed`, Web Launch Resilience, Refugio Visual Cleanup, Openworld QoL regression fix, Foundation Hardening V2 (`FOUNDATION_HARDENING_V2_PUBLISHED_INTERNAL_ALPHA`, release root `internal-alpha/v0-foundation-hardening-v2-hotfix2-20260601-58671a4`, preview `https://ca946749.draxos-mobile-internal-alpha.pages.dev`) e Hardening Platform V1 ficam preservados como baselines anteriores. Track 21 Arena Loop Unlock/Friction e Track 20 Season 1 Arena Calibration seguem preservados como contexto do modo Autobattler/Arena PVE; Remote Lab Runner segue preservado para Battle Lab/Progression Lab no Web export; iOS sem pedido explicito; mobile browser fora do escopo primario; secrets e service role nunca entram no cliente/export; publicacao remota exige `-ConfirmRemoteMutation`; mudancas visuais em Entry/Refugio/Batalha exigem `foundation-responsive-layout-contract.md` + `smoke_responsive_layout.gd`; novos dados devem usar `account_profiles/game_saves`; regras devem passar por registry/ruleset versionado; mutations economicas/social novas devem passar por RPC transacional v1 com `request_id`/`request_hash`; direct chat, ajudas, contribuicoes, moderacao, PVP, novas armas, novas spells, economia ampla, previsao de vitoria, contra-escolha por oponente, thresholds customizados, comportamento por inimigo e controles avancados de replay bloqueados ate decisao propria |
...
Nota local DraxosMobile: integracao `Openworld Collection Sync Local Fix` + `Main Menu Refactor` incorporada ao `master` em `1c72399`, sem publicacao remota; corrige rollback apos coleta, remove Mode Hub player-facing, `Coletar` geral, `Energia` direta e atalho dev Openworld, mantem `Bosque` como entrada direta, move Preparacao para Arena PVE e preserva os gates `ClientQuick`/`ModePlatform`/`ServerQuick`.
