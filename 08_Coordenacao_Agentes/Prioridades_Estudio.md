...
- Foco P2 de implementacao (`FOUNDATION_FINAL_POLISH_DELIVERED`, Track 13/Track 14 como baselines ativos, com Rpgsuave Bosque + Minigame Platform v0 publicados no Internal Alpha para playtest humano): `Projetos/draxos-mobile/`
...
| P2 | DraxosMobile | `Projetos/draxos-mobile/` | Implementacao - `FOUNDATION_FINAL_POLISH_DELIVERED` + `RPGSUAVE_INTEGRATED_ALPHA_PUBLISHED` | `P2_IMPLEMENTACAO` | Codigo, design, documentacao local, configuracao de infraestrutura | Playtest humano do Rpgsuave Bosque publicado no Internal Alpha | Branch canonica para novos agentes segue `codex/draxos-mobile/foundation-final-polish` ate decisao explicita de merge/push; Track 13 e Track 14 continuam como baselines ativos de release safety e operacao de agentes; branch de integracao `codex/draxos-mobile/rpgsuave-integrated-alpha` publicou Rpgsuave Labs Dev, Edge Function `minigames`, schema `mode_*` e Reward Bridge v0; release root atual `internal-alpha/v0-rpgsuave-integrated-alpha-20260531-0aa3969`; Portal/Web preview `https://d1e73b74.draxos-mobile-internal-alpha.pages.dev`; iOS sem pedido explicito; mobile browser fora do escopo primario; secrets e service role nunca entram no cliente/export; publicacao remota exige `-ConfirmRemoteMutation`; mudancas visuais em Entry/Refugio/Batalha exigem `foundation-responsive-layout-contract.md` + `smoke_responsive_layout.gd`; novos dados devem usar `account_profiles/game_saves`; regras devem passar por `foundation_ruleset_v0`/registry; mutations economicas/social/minigame novas devem passar por RPC transacional v1 com `request_id`/`request_hash`; Full gate local deve incluir Supabase RPC, Edge RPC e smoke RLS/admin; Battle Lab/Progression Lab agora cobrem Track 16 potions/behavior/crafting como evidencia lab-only; direct chat, ajudas, contribuicoes, moderacao, tuning numerico, novas armas, novas spells, economia, previsao de vitoria, contra-escolha por oponente, thresholds customizados, comportamento por inimigo e controles avancados de replay bloqueados ate decisao explicita |
