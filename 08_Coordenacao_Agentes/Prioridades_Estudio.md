...
- Foco P2 de implementacao (`S1_ARENA_CALIBRATION_LOCAL_PACKAGE_READY`, pacote local Track 20 pronto sobre o Internal Alpha remoto `internal-alpha/v0-remote-lab-runner-20260531-e659d7e5`, preservando Track 19 Arena Consistency Pass, Track 13 release safety e Track 14 agent ops): `Projetos/draxos-mobile/`
...
| P2 | DraxosMobile | `Projetos/draxos-mobile/` | Implementacao - `S1_ARENA_CALIBRATION_LOCAL_PACKAGE_READY` | `P2_IMPLEMENTACAO` | Codigo, design, documentacao local, configuracao de infraestrutura | Playtest humano do pacote local Track 20 da Arena PVE tutorial/3 duelos/multidificuldade, Web Labs e repeat rewards; publicacao remota so com aprovacao explicita | Track 20 Season 1 Arena Calibration e o pacote local atual em `build/internal-alpha/publish`: matriz S1 com 27 tiers, catalogo gerado, labs/backend/client usando `arena_id + difficulty_id`, rewards first-clear/repeat por tier e power de Arena apenas como metrica PVE; publicacao remota atual ainda inclui Remote Lab Runner (`internal-alpha/v0-remote-lab-runner-20260531-e659d7e5`, preview `https://9ae1e953.draxos-mobile-internal-alpha.pages.dev/web/index.html`) para Battle Lab/Progression Lab no Web export via Edge Function protegida pela mesma conta alpha Supabase por email/senha e save `normal`; direcao viva em `Projetos/draxos-mobile/docs/pve-arena-initial-direction.md`; PVP entra depois como modo competitivo com bots apenas como fallback/simulacao; iOS sem pedido explicito; mobile browser fora do escopo primario; secrets e service role nunca entram no cliente/export; publicacao remota exige `-ConfirmRemoteMutation`; mudancas visuais em Entry/Refugio/Batalha exigem `foundation-responsive-layout-contract.md` + `smoke_responsive_layout.gd`; novos dados devem usar `account_profiles/game_saves`; regras devem passar por `foundation_ruleset_v0`/registry; mutations economicas/social novas devem passar por RPC transacional v1 com `request_id`/`request_hash`; Battle Lab/Progression Lab precisam cobrir Arena PVE antes de tuning numerico; direct chat, ajudas, contribuicoes, moderacao, PVP, novas armas, novas spells, economia ampla, previsao de vitoria, contra-escolha por oponente, thresholds customizados, comportamento por inimigo e controles avancados de replay bloqueados ate decisao propria |
