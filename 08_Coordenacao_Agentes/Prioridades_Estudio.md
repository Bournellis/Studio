...
| P2 | DraxosMobile | `Projetos/draxos-mobile/` | Implementacao - `FOUNDATION_AUDIT_ACTIVE` | `P2_IMPLEMENTACAO` | Codigo, design, documentacao local, configuracao de infraestrutura | Revisar manualmente o hotfix publicado em Android/Windows/Web; confirmar Labs Dev, Refugio/Batalha contidos, download APK sem Bearer token, splash estatico ao solicitar batalha e loop pos-login antes de social basico | iOS sem pedido explicito; mobile browser fora do escopo primario; secrets e service role nunca entram no cliente/export; publicacao remota exige `-ConfirmRemoteMutation`; mudancas visuais em Entry/Refugio/Batalha exigem `foundation-responsive-layout-contract.md` + `smoke_responsive_layout.gd`; migration conta/save estrutural fora do pacote atual; tuning numerico, armas, spells, economia, visual final e apresentacao de batalha bloqueados ate decisao explicita |
