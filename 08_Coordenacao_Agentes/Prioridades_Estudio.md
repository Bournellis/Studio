...
- Foco P2 de implementacao (`BOSQUE_SESSION_LIFECYCLE_STRUCTURES_HOTFIX_V1_PUBLISHED_INTERNAL_ALPHA`, release root `internal-alpha/v0-bosque-session-lifecycle-structures-hotfix-v1-20260607-c953b51`, official Portal URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`, deployment evidence `https://8ecac093.draxos-mobile-internal-alpha.pages.dev`; publicado em Web/APK, APK/manifest `0.0.9-alpha.0`/version code `9`, impedindo retomada de sessoes expiradas do Bosque, restaurando full spawn em nova visita e preservando `fogueira_estavel_1` em `upgrades` + `structures`; Bosque World Hub Domain Separation v1 preservado como pacote anterior de separacao local/global, seguido por Bosque Fogueira Potion Crafting v1, Bosque Durable Bau Mochila v1, Arena PVE Menu Flow Simplification v1 e Bosque Offline-First Checkpoint v1): `Projetos/draxos-mobile/`
...
| P2 | DraxosMobile | `Projetos/draxos-mobile/` | Implementacao - `BOSQUE_SESSION_LIFECYCLE_STRUCTURES_HOTFIX_V1_PUBLISHED_INTERNAL_ALPHA` | `P2_IMPLEMENTACAO` | Codigo, design, documentacao local, configuracao de infraestrutura | Playtest humano do pacote Bosque Session Lifecycle & Durable Structures Hotfix v1; validar entrada apos mais de 2h/24h fora com full spawn, progresso duravel preservado, Fogueira persistente apos checkpoint/relog, station craft com `Bau do Bosque` + `Conta/Ossario`, e Arena Preparacao/pocoes sem regressao | Bosque Session Lifecycle & Durable Structures Hotfix v1 publicado em 2026-06-07 no release root `internal-alpha/v0-bosque-session-lifecycle-structures-hotfix-v1-20260607-c953b51`, official Portal URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`, deployment evidence `https://8ecac093.draxos-mobile-internal-alpha.pages.dev`; APK/manifest `0.0.9-alpha.0`/version code `9`, aplica migration `202606070001_openworld_bosque_session_lifecycle_structures_v1.sql`, filtra sessoes expiradas e persiste `structures` duraveis. Stable Portal/Web ficam Cloudflare Access protected. Android APK usa `debug_fallback`, aceito para playtest funcional; release signing adiado. iOS sem pedido explicito; mobile browser fora do escopo primario; secrets e service role nunca entram no cliente/export; novas mutacoes economicas/social devem seguir RPC transacional v1; tuning amplo, PVP, novas armas/spells/economia e visual final bloqueados ate decisao propria |
...
Nota DraxosMobile: `Bosque Session Lifecycle & Durable Structures Hotfix v1` foi publicado como Internal Alpha em `internal-alpha/v0-bosque-session-lifecycle-structures-hotfix-v1-20260607-c953b51`, mantendo a URL principal como contrato player-facing. O pacote corrige a retomada indevida de sessoes expiradas do Bosque, garante nova visita com full spawn apos expiracao, persiste `structures` duraveis e canonicaliza `fogueira_estavel_1` em `upgrades` e `structures`; `Bosque World Hub Domain Separation v1` segue preservado como pacote anterior de separacao local/global e `Bosque Fogueira Potion Crafting v1` segue preservado como pacote anterior de station craft. Track 13 release safety e Track 14 agent ops seguem como guardrails operacionais.
