...
- Foco P2 de implementacao (`BOSQUE_RESUME_EXIT_LIFECYCLE_V1_PUBLISHED_INTERNAL_ALPHA`, release root `internal-alpha/v0-bosque-resume-exit-lifecycle-v1-20260608-9a0f7c0`, official Portal URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`, deployment evidence `https://39128c59.draxos-mobile-internal-alpha.pages.dev`; publicado em Web/APK, APK/manifest `0.0.12-alpha.0`/version code `12`, operations-v2 e feel local preservados, reentrada `Voltar -> reabrir -> sair` retomando sessao viva em vez de criar Bosque novo fora do save, conflito de start devolvendo sessao ativa e saida com checkpoint falho sem prender o jogador; Bosque Feel & Spawn Authority v1 preservado como pacote anterior, seguido por Bosque Persistence Rebase v1, Bosque Session Lifecycle & Durable Structures Hotfix v1, Bosque World Hub Domain Separation v1, Bosque Fogueira Potion Crafting v1, Bosque Durable Bau Mochila v1 e Arena PVE Menu Flow Simplification v1): `Projetos/draxos-mobile/`
...
| P2 | DraxosMobile | `Projetos/draxos-mobile/` | Implementacao - `BOSQUE_RESUME_EXIT_LIFECYCLE_V1_PUBLISHED_INTERNAL_ALPHA` | `P2_IMPLEMENTACAO` | Codigo, design, documentacao local, configuracao de infraestrutura | Playtest humano do pacote Bosque Resume Exit Lifecycle v1; validar `Voltar -> reabrir -> sair` sem Bosque novo fora do save ou trap, cooldowns de nodes apos relog, Bau/Fogueira persistentes apos ACK, feel de coleta sem restart por movimento leve, ausencia de flicker/rollback visual, station craft com `Bau do Bosque` + `Conta/Ossario`, menu/deposito/craft responsivos e Arena Preparacao/pocoes sem regressao | Bosque Resume Exit Lifecycle v1 publicado em 2026-06-08 no release root `internal-alpha/v0-bosque-resume-exit-lifecycle-v1-20260608-9a0f7c0`, official Portal URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`, deployment evidence `https://39128c59.draxos-mobile-internal-alpha.pages.dev`; APK/manifest `0.0.12-alpha.0`/version code `12`, `modes` e `release` redeployadas, remote manifest/artifact/read-only/Web launch smokes passaram. Bosque Feel & Spawn Authority v1 segue preservado como pacote anterior de feel/spawn; Bosque Persistence Rebase v1 segue preservado como pacote anterior de persistencia/operacoes com migrations remotas `202606080001_openworld_bosque_persistence_rebase_v1.sql` e `202606080002_openworld_bosque_jsonb_object_length_hotfix_v1.sql`. Stable Portal/Web ficam Cloudflare Access protected. Android APK usa `debug_fallback`, aceito para playtest funcional; release signing adiado. iOS sem pedido explicito; mobile browser fora do escopo primario; secrets e service role nunca entram no cliente/export; novas mutacoes economicas/social devem seguir RPC transacional v1; tuning amplo, PVP, novas armas/spells/economia e visual final bloqueados ate decisao propria |
...
Nota DraxosMobile: `Bosque Resume Exit Lifecycle v1` foi publicado como Internal Alpha em `internal-alpha/v0-bosque-resume-exit-lifecycle-v1-20260608-9a0f7c0`, mantendo a URL principal como contrato player-facing. O pacote preserva as operacoes duraveis ACK-backed de `Bosque Persistence Rebase v1` e o feel local de `Bosque Feel & Spawn Authority v1`, mas corrige o lifecycle de retorno: sessoes vivas preservadas sao retomadas, conflitos de start devolvem a sessao ativa existente, e falhas de checkpoint na saida preservam pendencias sem prender o jogador no Bosque. Bosque Feel & Spawn Authority v1 preservado como pacote anterior de feel/spawn; Bosque Persistence Rebase v1 preservado como pacote anterior de persistencia/operacoes; Bosque Session Lifecycle & Durable Structures Hotfix v1 preservado como pacote anterior de lifecycle/structures; e Bosque World Hub Domain Separation v1 segue preservado como pacote anterior de separacao local/global. Track 13 release safety e Track 14 agent ops seguem como guardrails operacionais.
