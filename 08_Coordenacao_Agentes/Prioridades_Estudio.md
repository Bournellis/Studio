...
| P2 | DraxosMobile | `Projetos/draxos-mobile/` | Implementacao - Internal Alpha v0 local-first save context ready | `P2_IMPLEMENTACAO` | Codigo, design, documentacao local, configuracao de infraestrutura | Executar T03-P03B: resolver `save_type` no Supabase local/server-side antes de remoto/builds | iOS sem pedido explicito; mobile browser fora do escopo; remoto/builds adiados ate gameplay local pronto; secrets e service role nunca entram no cliente/export |
