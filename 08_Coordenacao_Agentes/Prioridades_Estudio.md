...
- Foco operacional temporario unico: `Projetos/JogoDaCopa/` (`JOGO_DA_COPA_TRACK_02_QUALITY_UPGRADE_V1_COMPLETE`)
...
| P0 TEMP | JogoDaCopa | `Projetos/JogoDaCopa/` | Implementacao - foco operacional temporario unico | `P2_IMPLEMENTACAO` | Codigo, design, validacao, playtest no editor e documentacao local | Playtest humano de `Copa Arena Futebol` no editor e no debug export Windows, com tuning de menu, gols, bola/chute/boost, bot e resultado/rematch | Enquanto o foco temporario estiver ativo, agentes devem escolher este projeto por padrao para implementacao/design/validacao/playtest |
...
Nota JogoDaCopa: Track 02 Quality Upgrade V1 completa em 2026-06-10 com marcador `JOGO_DA_COPA_TRACK_02_QUALITY_UPGRADE_V1_COMPLETE`. O produto jogavel agora se chama `Copa Arena Futebol`: 1x1 contra bot em terceira pessoa, arena noturna de vidro com glow/SSAO/fog, pitch em shader, redes grid, placares de estadio vivos, bola com shader de paineis/trail/squash, avatares humanoides com rig/AnimationTree preservando contratos, countdown de kickoff, slow-mo de gol, VFX de chute/gol/boost/freada, HUD broadcast, indicador da bola, menu 3D com preview, resultado/rematch, bot com predicao/defesa/boost/dificuldades e kickoff alternado. Nome/icone/splash/preset Windows criados; `tools/validate.gd` PASS 28 tests / 279 asserts; debug export Windows smoke PASS para `builds/windows/CopaArenaFutebol.exe`.
