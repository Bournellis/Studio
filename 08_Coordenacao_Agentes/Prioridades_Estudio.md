...
- Foco P2 de implementacao (`BOSQUE_NODE_COOLDOWN_ACK_V1_PUBLISHED_INTERNAL_ALPHA`, release root `internal-alpha/v0-bosque-node-cooldown-ack-v1-20260608-626b4ad`, official Portal URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`, deployment evidence `https://5cce952e.draxos-mobile-internal-alpha.pages.dev`; publicado em Web/APK, APK/manifest `0.0.13-alpha.0`/version code `13`, operations-v2, feel local e reentrada preservados, cooldown local ancorado no momento da coleta, resync idle aplicando progresso duravel do servidor e rejeicao terminal `OPENWORLD_NODE_ON_COOLDOWN` sem retry infinito; Bosque Resume Exit Lifecycle v1 preservado como pacote anterior, seguido por Bosque Feel & Spawn Authority v1, Bosque Persistence Rebase v1, Bosque Session Lifecycle & Durable Structures Hotfix v1, Bosque World Hub Domain Separation v1, Bosque Fogueira Potion Crafting v1, Bosque Durable Bau Mochila v1 e Arena PVE Menu Flow Simplification v1): `Projetos/draxos-mobile/`
...
| P2 | DraxosMobile | `Projetos/draxos-mobile/` | Implementacao - `BOSQUE_NODE_COOLDOWN_ACK_V1_PUBLISHED_INTERNAL_ALPHA` | `P2_IMPLEMENTACAO` | Codigo, design, documentacao local, configuracao de infraestrutura | Playtest humano do pacote Bosque Node Cooldown ACK v1; validar respawn/cooldown apos sair/voltar/relog, segunda coleta sem "aguardando servidor" permanente, `Voltar -> reabrir -> sair` sem Bosque novo fora do save ou trap, Bau/Fogueira persistentes apos ACK, station craft com `Bau do Bosque` + `Conta/Ossario`, menu/deposito/craft responsivos e Arena Preparacao/pocoes sem regressao | Bosque Node Cooldown ACK v1 publicado em 2026-06-08 no release root `internal-alpha/v0-bosque-node-cooldown-ack-v1-20260608-626b4ad`, official Portal URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`, deployment evidence `https://5cce952e.draxos-mobile-internal-alpha.pages.dev`; APK/manifest `0.0.13-alpha.0`/version code `13`, `release` redeployada, remote manifest/artifact/release/Web launch smokes passaram. Bosque Resume Exit Lifecycle v1 segue preservado como pacote anterior de reentrada/saida; Bosque Feel & Spawn Authority v1 segue preservado como pacote anterior de feel/spawn; Bosque Persistence Rebase v1 segue preservado como pacote anterior de persistencia/operacoes com migrations remotas `202606080001_openworld_bosque_persistence_rebase_v1.sql` e `202606080002_openworld_bosque_jsonb_object_length_hotfix_v1.sql`. Stable Portal/Web ficam Cloudflare Access protected. Android APK usa `debug_fallback`, aceito para playtest funcional; release signing adiado. iOS sem pedido explicito; mobile browser fora do escopo primario; secrets e service role nunca entram no cliente/export; novas mutacoes economicas/social devem seguir RPC transacional v1; tuning amplo, PVP, novas armas/spells/economia e visual final bloqueados ate decisao propria |
...
Nota DraxosMobile: `Bosque Node Cooldown ACK v1` foi publicado como Internal Alpha em `internal-alpha/v0-bosque-node-cooldown-ack-v1-20260608-626b4ad`, mantendo a URL principal como contrato player-facing. O pacote preserva as operacoes duraveis ACK-backed de `Bosque Persistence Rebase v1`, o feel local de `Bosque Feel & Spawn Authority v1` e a reentrada de `Bosque Resume Exit Lifecycle v1`, mas corrige o cooldown/ACK de nodes: respawn local parte da coleta, reentrada idle aceita progresso duravel do servidor, e rejeicao terminal de cooldown nao fica em retry infinito. Bosque Resume Exit Lifecycle v1 preservado como pacote anterior de resume/exit; Bosque Feel & Spawn Authority v1 preservado como pacote anterior de feel/spawn; Bosque Persistence Rebase v1 preservado como pacote anterior de persistencia/operacoes; Bosque Session Lifecycle & Durable Structures Hotfix v1 preservado como pacote anterior de lifecycle/structures; e Bosque World Hub Domain Separation v1 segue preservado como pacote anterior de separacao local/global. Track 13 release safety e Track 14 agent ops seguem como guardrails operacionais.
