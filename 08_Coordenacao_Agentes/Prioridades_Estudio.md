...
- Foco P2 de implementacao (`FOUNDATION_FINAL_POLISH_DELIVERED`, com Foundation Closeout, labs alinhados ao Track 16, shell/source guards finais, smoke local RLS/admin no Full gate sobre Track 13 release safety e Track 14 agent ops, Internal Alpha republicado na release root `internal-alpha/v0-foundation-final-polish-20260530-8c658f6`, Edge CORS remoto sincronizado pelo Debug Clean Web Config e Web Auth Foundation Context Hotfix com DB remoto alinhado e pacote Storage aguardando reauth do Wrangler para Pages): `Projetos/draxos-mobile/`
...
| P2 | DraxosMobile | `Projetos/draxos-mobile/` | Implementacao - `FOUNDATION_FINAL_POLISH_DELIVERED` | `P2_IMPLEMENTACAO` | Codigo, design, documentacao local, configuracao de infraestrutura | Reautenticar Wrangler e completar deploy Cloudflare do Web Auth Foundation Context Hotfix antes de abrir tuning; depois escolher explicitamente base builder tuning, autobattler tuning, social expansion ou minigame contract usando os labs alinhados como evidencia | Base canonica local para novos agentes: branch `codex/draxos-mobile/foundation-final-polish` no HEAD final validado e publicado ate decisao de merge/push; release root Cloudflare atual `internal-alpha/v0-foundation-final-polish-20260530-8c658f6`; hotfix Storage aguardando Pages em `internal-alpha/v0-web-auth-foundation-context-20260530`; Debug Clean Web Config corrigiu CORS remoto de Edge Functions para o header `x-draxos-api-version`; Web Auth Foundation Context Hotfix alinhou migrations remotas `202605300001`-`202605300004` e corrige guest com sessao email cacheada no cliente; iOS sem pedido explicito; mobile browser fora do escopo primario; secrets e service role nunca entram no cliente/export; publicacao remota exige `-ConfirmRemoteMutation`; mudancas visuais em Entry/Refugio/Batalha exigem `foundation-responsive-layout-contract.md` + `smoke_responsive_layout.gd`; novos dados devem usar `account_profiles/game_saves`; regras devem passar por `foundation_ruleset_v0`/registry; mutations economicas/social novas devem passar por RPC transacional v1 com `request_id`/`request_hash`; Full gate local deve incluir Supabase RPC, Edge RPC e smoke RLS/admin; Battle Lab/Progression Lab agora cobrem Track 16 potions/behavior/crafting como evidencia lab-only; direct chat, ajudas, contribuicoes, moderacao, tuning numerico, novas armas, novas spells, economia, previsao de vitoria, contra-escolha por oponente, thresholds customizados, comportamento por inimigo e controles avancados de replay bloqueados ate decisao explicita |
