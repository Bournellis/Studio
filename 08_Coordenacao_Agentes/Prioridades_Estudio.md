...
- Foco P2 de implementacao (`FOUNDATION_CLOSEOUT_DELIVERED`, com labs alinhados ao Track 16 sobre Foundation Expansion Readiness, Track 13 release safety, Track 14 agent ops e First Session Clarity v1 aprovado): `Projetos/draxos-mobile/`
...
| P2 | DraxosMobile | `Projetos/draxos-mobile/` | Implementacao - `FOUNDATION_CLOSEOUT_DELIVERED` | `P2_IMPLEMENTACAO` | Codigo, design, documentacao local, configuracao de infraestrutura | Escolher explicitamente base builder tuning, autobattler tuning, social expansion ou minigame contract usando os labs alinhados como evidencia | iOS sem pedido explicito; mobile browser fora do escopo primario; secrets e service role nunca entram no cliente/export; publicacao remota exige `-ConfirmRemoteMutation`; mudancas visuais em Entry/Refugio/Batalha exigem `foundation-responsive-layout-contract.md` + `smoke_responsive_layout.gd`; novos dados devem usar `account_profiles/game_saves`; regras devem passar por `foundation_ruleset_v0`/registry; mutations economicas/social novas devem passar por RPC transacional v1 com `request_id`/`request_hash`; Full gate local passou com Supabase/Edge em 2026-05-30; Battle Lab/Progression Lab agora cobrem Track 16 potions/behavior/crafting como evidencia lab-only; direct chat, ajudas, contribuicoes, moderacao, tuning numerico, novas armas, novas spells, economia, previsao de vitoria, contra-escolha por oponente, thresholds customizados, comportamento por inimigo e controles avancados de replay bloqueados ate decisao explicita |
