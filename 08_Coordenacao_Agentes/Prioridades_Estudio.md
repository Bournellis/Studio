...
- Foco P2 de implementacao (`FOUNDATION_FINAL_POLISH_DELIVERED`, Track 13/Track 14 como baselines ativos, com Rpgsuave Bosque Visual Upgrade v1 publicado no Internal Alpha para playtest humano): `Projetos/draxos-mobile/`
...
| P2 | DraxosMobile | `Projetos/draxos-mobile/` | Implementacao - `FOUNDATION_FINAL_POLISH_DELIVERED` + `RPGSUAVE_VISUAL_UPGRADE_V1_PUBLISHED` | `P2_IMPLEMENTACAO` | Codigo, design, documentacao local, configuracao de infraestrutura | Playtest humano do Rpgsuave Bosque Visual Upgrade v1 em mobile portrait | Branch canonica para novos agentes segue `codex/draxos-mobile/foundation-final-polish` ate decisao explicita de merge/push; Track 13 e Track 14 continuam como baselines ativos de release safety e operacao de agentes; branch `codex/draxos-mobile/rpgsuave-visual-upgrade-v1` publicou Rpgsuave Labs Dev fullscreen mobile portrait, camera no personagem, joystick-only, HUD in-game, Mochila funcional e assets procedurais Godot; release root atual `internal-alpha/v0-rpgsuave-visual-upgrade-v1-20260531-809c4dc`; Portal/Web preview `https://b5b7a32a.draxos-mobile-internal-alpha.pages.dev`; Rpgsuave continua Labs Dev-only e sem CTA publico no Refugio; iOS sem pedido explicito; mobile browser fora do escopo primario; secrets e service role nunca entram no cliente/export; publicacao remota exige `-ConfirmRemoteMutation`; mudancas visuais em Entry/Refugio/Batalha exigem `foundation-responsive-layout-contract.md` + `smoke_responsive_layout.gd`; novos dados devem usar `account_profiles/game_saves`; regras devem passar por `foundation_ruleset_v0`/registry; mutations economicas/social/minigame novas devem passar por RPC transacional v1 com `request_id`/`request_hash`; Full gate local deve incluir Supabase RPC, Edge RPC e smoke RLS/admin; Battle Lab/Progression Lab agora cobrem Track 16 potions/behavior/crafting como evidencia lab-only; direct chat, ajudas, contribuicoes, moderacao, tuning numerico, novas armas, novas spells, economia, previsao de vitoria, contra-escolha por oponente, thresholds customizados, comportamento por inimigo e controles avancados de replay bloqueados ate decisao explicita |
