...
| P2 | DraxosMobile | `Projetos/draxos-mobile/` | Implementacao - `FOUNDATION_AUDIT_ACTIVE` | `P2_IMPLEMENTACAO` | Codigo, design, documentacao local, configuracao de infraestrutura | Executar Foundation Audit: alinhar docs vivos e auditar o loop pos-login Base -> coletar recursos -> evoluir base -> batalhar -> receber recompensas -> verificar base novamente | iOS sem pedido explicito; mobile browser fora do escopo primario; secrets e service role nunca entram no cliente/export; publicacao remota exige `-ConfirmRemoteMutation`; migration conta/save estrutural fora do pacote atual; tuning numerico, armas, spells, economia, visual final e apresentacao de batalha bloqueados ate Foundation Audit e decisao explicita |
