...
Nota local DraxosMobile: integracao `Openworld Collection Sync Local Fix` + `Main Menu Refactor` em `codex/draxos-mobile/merge-current-work`, sem publicacao remota; corrige rollback apos coleta, remove Mode Hub player-facing, `Coletar` geral, `Energia` direta e atalho dev Openworld, mantem `Bosque` como entrada direta, move Preparacao para Arena PVE e preserva os gates `ClientQuick`/`ModePlatform`.
