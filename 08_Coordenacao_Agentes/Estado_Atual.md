...
- Foco P2 de implementacao: `Projetos/draxos-mobile/` (`BOSQUE_WORLD_HUB_DOMAIN_SEPARATION_V1_PUBLISHED_INTERNAL_ALPHA`, release root `internal-alpha/v0-bosque-world-hub-domain-separation-v1-20260606-81ecf05`, official Portal URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`, deployment evidence `https://d1872010.draxos-mobile-internal-alpha.pages.dev`, publicado em `main`, APK/manifest `0.0.8-alpha.0`/version code `8`; separa materiais locais do Bosque de recursos globais da conta e persiste Fogueira como estrutura/estacao duravel)
...
- Baseline atual: Bosque World Hub Domain Separation v1 publicado como Internal Alpha no release root `internal-alpha/v0-bosque-world-hub-domain-separation-v1-20260606-81ecf05`, official Portal URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html` e deployment evidence `https://d1872010.draxos-mobile-internal-alpha.pages.dev`. O pacote forca APK/manifest `0.0.8-alpha.0`/version code `8`, publica Web/APK novos, aplica migration remota `202606060004_bosque_world_hub_domain_separation_v1.sql`, normaliza inventarios legados `ossos_preview`/`po_osso_preview` para `resto_ritual`/`po_cinzento`, impede que coleta do Bosque altere `ossos`/`po_osso` globais, persiste `fogueira_estavel_1` em `upgrades` e `structures`, adiciona `BosqueWorldContext` e preserva station craft server-authoritative com materiais do `Bau do Bosque` + `Conta/Ossario`. Bosque Fogueira Potion Crafting v1 (`internal-alpha/v0-bosque-fogueira-potion-crafting-v1-20260606-cad6d2c`, preview `https://08d00f24.draxos-mobile-internal-alpha.pages.dev`) segue preservado como pacote anterior de station craft; Bosque Durable Bau Mochila v1, Arena PVE Menu Flow Simplification v1 e Bosque Offline-First Checkpoint v1 seguem preservados como baselines anteriores.
...
- Proximo passo: playtest humano do pacote Bosque World Hub Domain Separation v1 a partir de `main`; validar separacao `Baú do Bosque`/`Mochila do Bosque`/`Conta-Ossario`, Fogueira persistente apos sair/relogar, station craft com `po_osso` global correto, cache legado normalizado e regressao da Arena Preparacao/buff flow. Manter microeventos revisionados fora do loop normal do Bosque sem nova decisao.
