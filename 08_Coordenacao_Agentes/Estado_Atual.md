...
- Tech probe P2 de implementacao: `Projetos/FpsShooter/` (`FPS_PLAYGROUND_TRACK_06B_THIRD_PERSON_FOOTBALL_CAMERA_COMPLETE`; PC Windows editor-first FPS Playground independente com tema Draxos leve; proximo passo: playtest humano no editor focado no Futebol em terceira pessoa, framing da camera, avatar local visivel e feel do chute body-forward)
...
- Proximo passo: playtest humano no editor focado no `Futebol` em terceira pessoa: distancia/altura/foco da camera, leitura do avatar local, feel do chute body-forward, selecao de pele/camisa no intro e decisao da proxima track de gameplay/personagem.
