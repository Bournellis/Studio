...
- Baseline atual: Track 00 em bootstrap com T00-P01 a T00-P09 concluidos; T00-P10 em andamento com v0 executavel. O cliente Godot 4.6.2 tem boot scene, `tools/validate.gd`, `tools/smoke_session_shell.gd`, `tools/smoke_battle_replay.gd`, GUT 9.6.0, autoloads `UiTokens`/`AssetIds`/`ContentLibrary`/`SessionStore`/`SupabaseClient`, `BattleLogPresenter`, pipeline `data/definitions/*.json` -> `data/generated/draxos_mobile_catalog.tres`, conteudo real inicial, cliente HTTPRequest para Auth anonimo + `account/guest` + `account/state` + `battle/request` + `battle/latest`, cache local nao autoritativo, replay placeholder de `battle_log_v1` e testes client verdes (`14/14`, `52` asserts). Supabase runtime local esta configurado no layout oficial `supabase/` com Docker Desktop, Deno via `npx`, Auth anonimo, migrations MVP, healthcheck Edge Function, `account/guest`, `account/state`, `battle/request`, `battle/latest`, seeds de bots `FIRST_SLICE`, modo `FIRST_SLICE_SIM` server-authoritative v0, idempotencia por `request_id`, recompensas XP/Almas/Energia/Sangue/Ossos, teste deterministico do simulador verde, escrita direta do cliente bloqueada e smokes P06-P08 preservados como baseline. Design do primeiro slice ja definiu cap 40, levels permanentes, unlocks de spell/passiva/pet, base v0 implementavel, missoes/onboarding v0, monetizacao/recompensas v0, social/ranking/chat v0, combate real/simulador v0, matchmaking por poder, bots iniciais, telemetria minima, schema de build, UX alpha com Refugio e baseline calibravel de economia/simulador de seasons.
...
- Proximo passo: continuar `T00-P10 - Conteudo Real E Simulador Completo` com DoTs/status/resistencias, smoke `FIRST_SLICE_SIM` no runtime Supabase e replay rico no cliente.
