...
- Foco P2 de implementacao: `Projetos/draxos-mobile/` (`BOSQUE_PERSISTENCE_REBASE_V1_PUBLISHED_INTERNAL_ALPHA`, release root `internal-alpha/v0-bosque-persistence-rebase-v1-20260608-bc23f74`, official Portal URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`, deployment evidence `https://0c0a8dcf.draxos-mobile-internal-alpha.pages.dev`, publicado Web/APK, APK/manifest `0.0.10-alpha.0`/version code `10`; Bosque usa checkpoint operations ACK-backed, `openworld_forest_progress_v2`, cooldowns por node e Fogueira liberada apenas por ACK de `structures`)
...
- Baseline atual: Bosque Persistence Rebase v1 publicado como Internal Alpha no release root `internal-alpha/v0-bosque-persistence-rebase-v1-20260608-bc23f74`, official Portal URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html` e deployment evidence `https://0c0a8dcf.draxos-mobile-internal-alpha.pages.dev`. O pacote forca APK/manifest `0.0.10-alpha.0`/version code `10`, publica Web/APK novos, aplica migrations remotas `202606080001_openworld_bosque_persistence_rebase_v1.sql` e `202606080002_openworld_bosque_jsonb_object_length_hotfix_v1.sql`, troca snapshot local em background por checkpoint `operations` com ACK, persiste `openworld_forest_progress_v2`, usa `node_state.next_spawn_at` para cooldowns por node, trata cache local apenas como preview/retry e libera Fogueira station somente apos ACK de `structures.fogueira_estavel_1`. Bosque Session Lifecycle & Durable Structures Hotfix v1 segue preservado como pacote anterior; Bosque World Hub Domain Separation v1, Bosque Fogueira Potion Crafting v1, Bosque Durable Bau Mochila v1, Arena PVE Menu Flow Simplification v1 e Bosque Offline-First Checkpoint v1 seguem preservados como baselines anteriores.
