...
- Foco P2 de implementacao: `Projetos/draxos-mobile/` (`BOSQUE_FEEL_SPAWN_AUTHORITY_V1_PUBLISHED_INTERNAL_ALPHA`, release root `internal-alpha/v0-bosque-feel-spawn-authority-v1-20260608-70b79c3`, official Portal URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`, deployment evidence `https://16ac3cb7.draxos-mobile-internal-alpha.pages.dev`, publicado Web/APK, APK/manifest `0.0.11-alpha.0`/version code `11`; Bosque preserva checkpoint operations ACK-backed e corrige feel com server-time cooldowns, coleta sticky, sem flicker/rollback visual e menu/deposito/craft sem busy global indevido)
...
- Baseline atual: Bosque Feel & Spawn Authority v1 publicado como Internal Alpha no release root `internal-alpha/v0-bosque-feel-spawn-authority-v1-20260608-70b79c3`, official Portal URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html` e deployment evidence `https://16ac3cb7.draxos-mobile-internal-alpha.pages.dev`. O pacote forca APK/manifest `0.0.11-alpha.0`/version code `11`, publica Web/APK novos, preserva checkpoint `operations` com ACK e corrige o feel do Bosque com server-time para cooldowns por node, grace visual de respawn, coleta sticky, reconciliacao sem rollback de preview e menu/deposito/craft sem busy global indevido. Bosque Persistence Rebase v1 segue preservado como pacote anterior de persistencia/operacoes com migrations remotas `202606080001_openworld_bosque_persistence_rebase_v1.sql` e `202606080002_openworld_bosque_jsonb_object_length_hotfix_v1.sql`; Bosque Session Lifecycle & Durable Structures Hotfix v1 segue preservado como pacote anterior; Bosque World Hub Domain Separation v1, Bosque Fogueira Potion Crafting v1, Bosque Durable Bau Mochila v1, Arena PVE Menu Flow Simplification v1 e Bosque Offline-First Checkpoint v1 seguem preservados como baselines anteriores.
...
- Proximo passo: playtest humano do pacote Bosque Feel & Spawn Authority v1; validar feel de coleta sem restart por movimento leve, ausencia de flicker/rollback visual de nodes, cooldowns de nodes apos relog, Bau/Fogueira persistentes somente apos ACK, aviso/bloqueio ao sair durante `Salvando...`, descarte claro de operacoes nao confirmadas quando sessao expira, station craft com `po_osso` global correto e regressao da Arena Preparacao/buff flow. Manter microeventos revisionados fora do loop normal do Bosque sem nova decisao.
