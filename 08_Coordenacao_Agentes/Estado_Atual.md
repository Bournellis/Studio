...
- Foco P2 de implementacao: `Projetos/draxos-mobile/` (`BOSQUE_MECANICO_BASICO_V2_PUBLISHED_INTERNAL_ALPHA`, release root `internal-alpha/v0-bosque-v2-guidance-20260604-7c2d981`, official Portal URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`, deployment evidence `https://ae049df9.draxos-mobile-internal-alpha.pages.dev`, com correcoes locais ainda nao publicadas `Openworld Collection Sync Local Fix` e `Main Menu Refactor` integradas em `codex/draxos-mobile/merge-current-work`, preservando First Access Runtime Fix `4608977`, Integrated Runtime Fix `ab5834c`, Integrated App/Arena/Bosque `99304ed`, Web Launch Resilience, Refugio Visual Cleanup, Openworld QoL regression fix, Foundation Hardening V2 (`FOUNDATION_HARDENING_V2_PUBLISHED_INTERNAL_ALPHA`, release root `internal-alpha/v0-foundation-hardening-v2-hotfix2-20260601-58671a4`, preview `https://ca946749.draxos-mobile-internal-alpha.pages.dev`) e Hardening Platform V1 como baselines anteriores, Track 21 Arena Loop Unlock/Friction como contexto do Autobattler, Track 20 Season 1 Arena Calibration, Remote Lab Runner, Track 13 release safety e Track 14 agent ops)
...
- Baseline atual: Bosque Mecanico Basico v2 publicado como Internal Alpha no release root `internal-alpha/v0-bosque-v2-guidance-20260604-7c2d981`, official Portal URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html` e deployment evidence `https://ae049df9.draxos-mobile-internal-alpha.pages.dev`, mantendo o dominio fixo protegido por Cloudflare Access e usando o preview hash como evidencia tecnica publica. O pacote traz Bosque livre/relaxante com guidance opcional persistida no save normal, recursos fixos com folga, `Encerrar visita` sem objetivo obrigatorio e `fogueira_estavel_1` visual/bloqueante. Correcao local posterior `Openworld Collection Sync Local Fix` em `codex/draxos-mobile/openworld-local-validation` corrige rollback apos coleta, mas ainda nao foi publicada e nao altera o baseline remoto. `supabase db push`, `supabase functions deploy modes`, Storage upload, Cloudflare Pages, manifest remoto e RemoteReadOnly passaram na publicacao Bosque v2. Smoke Chrome/CDP no preview carregou o jogo em 3699 ms, com screenshot em `build/diagnostics/web-launch-remote-20260604-194152/web-launch-remote.png`; GET anonimo no production fixo retorna Cloudflare Access como esperado. Android APK usa `debug_fallback`, aceito para playtest funcional; release signing fica adiado para distribuicao Android mais ampla. First Access Runtime Fix `4608977`, Integrated Runtime Fix `ab5834c`, Integrated App/Arena/Bosque `99304ed`, Web Launch Resilience, Refugio Visual Cleanup, Openworld QoL regression fix, Foundation Hardening V2 (`FOUNDATION_HARDENING_V2_PUBLISHED_INTERNAL_ALPHA`, release root `internal-alpha/v0-foundation-hardening-v2-hotfix2-20260601-58671a4`, preview `https://ca946749.draxos-mobile-internal-alpha.pages.dev`) e Hardening Platform V1 seguem preservados como baselines anteriores. Track 13 release safety e Track 14 agent ops seguem preservados. Track 21 Arena Loop Unlock/Friction e Track 20 Season 1 Arena Calibration seguem como contexto preservado do Autobattler/Arena PVE; Remote Lab Runner segue como contexto preservado de Labs no Web export.
- Pacote local integrado atual: `Openworld Collection Sync Local Fix` + `Main Menu Refactor` em `codex/draxos-mobile/merge-current-work`, validado localmente ate agora e ainda nao publicado; corrige rollback apos coleta, remove `Modos`/Mode Hub player-facing, `Coletar` geral, `Energia` direta e atalho dev Openworld, mantem `Bosque` como entrada direta, move Preparacao para Arena PVE e deixa compra de Energia na Loja.
...
- Proximo passo: revisar a integracao local de Openworld Collection Sync + Main Menu Refactor e decidir publicacao/hotfix antes de ampliar o playtest humano do Bosque Mecanico Basico v2 publicado.
