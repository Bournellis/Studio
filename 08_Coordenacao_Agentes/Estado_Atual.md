...
- Baseline atual: Track 02 completa para playtest de usuario em Godot 4.6.2 com rota fixa de 29 mapas, save/snapshot v5, recompensas/reliquias/Souls shop expandida, keywords completas, AI/intent, modos e formatos de encontro, field effects, boss hooks, UI polida, descarte marcado na fase principal, testes modulares, diretores/servicos internos de fundacao incluindo enemy AI/intent, combate/dano, reward/shop e BattleRoot presenters, geracao idempotente do catalogo, simulador compartilhado de pacing, checklist de playtest, AutoRun Gate Pack V1 com presets/matriz/politicas macro/golden comparison/baselines oficiais smoke e quick/gate explicito/scorecards JSON-Markdown, Scenario Fixtures V1 com pacote `track02_core_v1`, 12 cenarios deterministicos nomeados, expectativas PASS/WARN/FAIL, gate explicito e relatorios JSON/CSV/Markdown, Gameplay Lab V1 com pacote `track02_battle_core_v1`, 12 batalhas isoladas via BattleEngine real, policies legais deterministicas, expectativas PASS/WARN/FAIL, gate explicito e relatorios JSON/CSV/Markdown, Lab Diff Reporter V1 para comparar outputs before/after de AutoRun/Scenario/Battle, Card Impact Pack V1/V2/V3/V4/V4.1 com V4/V4.1 cobrindo 108 cartas ativas de jogador, 30 cartas inimigas report-only e 15 `elemental_*` legado auditadas, assinaturas de efeito exigidas para cartas de jogador, familias non-damage, utility e card-flow derivadas de logs/snapshots incluindo `temporary_ability_power`, `cards_drawn`, `deck_delta`, `hand_delta`, `discard_delta` e qualidade `card_flow_expected/observed`, metadados de contaminacao por suporte, policy `card_focus_isolated`, before/after/compare gate explicito e relatorios JSON/CSV/Markdown, Card Impact Smoke Tuning V1, Card Redesign Batch 01, Player Card Redesign Batch 02, Reward Card Redesign Batch 01 Using V4 e Reward Card Redesign Batch 02 Utility Using V4 aplicados em lotes pequenos, V4.1 detectando card-flow em 2 cartas sem regressao estrutural, arquitetura/closeout de fundacao documentados, validacao verde 185/185 e telemetria de rota completa 29/29.
- Baseline addendum: Card Impact V4.1 Card-Flow Harness Pass passou em 2026-06-06 com `track02_card_impact_v4_1` before/after/compare em `user://card_impact/track02_card_impact_v4_1_card_flow_harness`, 108 cartas de jogador, 30 inimigas report-only, 15 legadas inativas, 2 cartas card-flow esperadas, zero structural errors, zero new failures e zero removed records. `necro_colheita_das_almas` e `necro_colheita_das_almas_lvl3` observaram `cards_drawn=1`, `deck_delta=-1` e `card_flow_observed=true`; o lvl3 usou prestate de lab `initial_dead_unit_count=2` e registrou `ashes_gained=6`. V4 historico em `user://card_impact/reward_card_redesign_batch_02_utility_v4` segue verde; Battle Lab 9 PASS / 3 WARN / 0 FAIL, Scenario Fixtures 9 PASS / 3 WARN / 0 FAIL, AutoRun smoke/quick verdes e `validate.gd` 185/185.
...
- Proximo passo: executar um pequeno redesign real de card-flow usando Card Impact V4.1 before/change/after/compare, entao decidir se algum threshold/relatorio deve virar expectativa promovida; assinaturas causais de cartas inimigas seguem como follow-up tecnico. Track 02 segue pronta para playtest de usuario da rota completa.

## DraxosMobile

- Status: **P2_IMPLEMENTACAO - ARENA_BOSQUE_VISIBLE_V2_PUBLISHED_INTERNAL_ALPHA**
- Fase: `Implementacao`
...
- Proximo passo: playtest humano do pacote Arena/Bosque Visible V2 focando instalacao da build `0.0.2`/version code `2`, Preparacao antes de iniciar, tentativa ativa/travada e buff choice, tutorial -> primeira arena real de 3 duelos, buff pendente apos update/reopen, buff escolhido -> Resolver duelo, retomar/abandonar/encerrar tentativa antiga e Bosque deposit/craft/persistencia antes de abrir tuning amplo ou novas expansoes.
