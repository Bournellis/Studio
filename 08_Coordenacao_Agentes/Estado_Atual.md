...
- Foco P2 de implementacao: `Projetos/draxos-mobile/` (`BOSQUE_NODE_COOLDOWN_ACK_V1_PUBLISHED_INTERNAL_ALPHA`, release root `internal-alpha/v0-bosque-node-cooldown-ack-v1-20260608-626b4ad`, official Portal URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`, deployment evidence `https://5cce952e.draxos-mobile-internal-alpha.pages.dev`, publicado Web/APK, APK/manifest `0.0.13-alpha.0`/version code `13`; Bosque preserva checkpoint operations ACK-backed, feel local e reentrada, e corrige respawn/cooldown de node e retry infinito em `OPENWORLD_NODE_ON_COOLDOWN`)
...
- Baseline atual: Bosque Node Cooldown ACK v1 publicado como Internal Alpha no release root `internal-alpha/v0-bosque-node-cooldown-ack-v1-20260608-626b4ad`, official Portal URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html` e deployment evidence `https://5cce952e.draxos-mobile-internal-alpha.pages.dev`. O pacote forca APK/manifest `0.0.13-alpha.0`/version code `13`, publica Web/APK novos, preserva checkpoint `operations` com ACK, o feel local e o lifecycle de reentrada, e corrige cooldown/ACK de nodes: respawn local parte da coleta, reentrada idle aceita progresso duravel do servidor, e rejeicao terminal de cooldown nao fica em retry infinito. Bosque Resume Exit Lifecycle v1 segue preservado como pacote anterior de reentrada/saida; Bosque Feel & Spawn Authority v1 segue preservado como pacote anterior de feel/spawn; Bosque Persistence Rebase v1 segue preservado como pacote anterior de persistencia/operacoes com migrations remotas `202606080001_openworld_bosque_persistence_rebase_v1.sql` e `202606080002_openworld_bosque_jsonb_object_length_hotfix_v1.sql`; Bosque Session Lifecycle & Durable Structures Hotfix v1 segue preservado como pacote anterior; Bosque World Hub Domain Separation v1, Bosque Fogueira Potion Crafting v1, Bosque Durable Bau Mochila v1, Arena PVE Menu Flow Simplification v1 e Bosque Offline-First Checkpoint v1 seguem preservados como baselines anteriores.
...
- Proximo passo: playtest humano do pacote Bosque Node Cooldown ACK v1; validar respawn/cooldown apos sair/voltar/relog, segunda coleta sem "aguardando servidor" permanente, `Voltar -> reabrir -> sair` sem Bosque novo fora do save ou trap, Bau/Fogueira persistentes somente apos ACK, aviso/bloqueio ao sair durante `Salvando...`, descarte claro de operacoes nao confirmadas quando sessao expira, station craft com `po_osso` global correto e regressao da Arena Preparacao/buff flow. Manter microeventos revisionados fora do loop normal do Bosque sem nova decisao.
