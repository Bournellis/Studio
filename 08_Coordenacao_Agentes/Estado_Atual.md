...
- Tech probe P2 de implementacao: `Projetos/FpsShooter/` (`FPS_PLAYGROUND_TRACK_05_FOUNDATION_HARDENING_REFACTOR_COMPLETE`; PC Windows editor-first FPS Playground independente com tema Draxos leve; proximo passo: escolher a proxima track focada de crescimento de gameplay, com candidatos recomendados em futebol feel/posse, arena conteudo/mapa ou outro modo pequeno em primeira pessoa)
...
- Status: **P2_IMPLEMENTACAO - FPS_PLAYGROUND_TRACK_05_FOUNDATION_HARDENING_REFACTOR_COMPLETE**
- Fase: `Implementacao - FPS Playground Tech Probe`
- Local: `Projetos/FpsShooter/`
- Baseline atual: projeto oficial implementavel criado para testar Godot 4.6.2 em gameplay 3D primeira pessoa, PC Windows editor-first. Track 05 Foundation Hardening & Refactor V1 completa sobre Track 04A e baselines anteriores: `FPS Playground` preserva menu principal centralizado por container/painel com `Arena Shooter` e `Futebol`, preserva o comportamento aceito dos dois modos e adiciona documentacao local, perfis de validacao, builders runtime de layout, helpers puros de regras, helpers de mira/visibilidade do bot e testes focados de helpers alem da regressao integrada. `Arena Shooter` preserva `Duel Pit V2` com spawns protegidos, cover, plataformas, jump pads, pickups elevados com micro-commit, rifle hitscan, RMB Plasma Bolt, feedback/HUD de combate, bot vertical-aware, rotas por jump pads, dodge de plasma, salto simples e knockback aceito, sem void/fall zones no mapa atual. `Futebol` preserva modo em primeira pessoa 1x1 contra bot, sem armas, inicia pausado em `Como Jogar`/`Comecar`, LMB chute, RMB chute forte, bola `RigidBody3D` arcade solta, campo festivo por primitivas, gols fechados com piso e paredes laterais internas, gols ate 3, bot que ataca/defende, score HUD, feedback/audio sintetico de chute/gol e retorno ao menu pelo pause.
...
- Proximo passo: escolher a proxima track focada de crescimento de gameplay do `FPS Playground`; candidatos recomendados: tuning de feel/posse do Futebol, expansao de conteudo/mapa da Arena ou outro modo pequeno em primeira pessoa.
