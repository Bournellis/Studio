...
- Foco P2 de implementacao: `Projetos/draxos-mobile/` (`BOSQUE_SESSION_LIFECYCLE_STRUCTURES_HOTFIX_V1_PUBLISHED_INTERNAL_ALPHA`, release root `internal-alpha/v0-bosque-session-lifecycle-structures-hotfix-v1-20260607-c953b51`, official Portal URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`, deployment evidence `https://8ecac093.draxos-mobile-internal-alpha.pages.dev`, publicado Web/APK, APK/manifest `0.0.9-alpha.0`/version code `9`; nova visita do Bosque apos sessao expirada volta com full spawn e Fogueira/structures duraveis persistem por save)
...
- Baseline atual: Bosque Session Lifecycle & Durable Structures Hotfix v1 publicado como Internal Alpha no release root `internal-alpha/v0-bosque-session-lifecycle-structures-hotfix-v1-20260607-c953b51`, official Portal URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html` e deployment evidence `https://8ecac093.draxos-mobile-internal-alpha.pages.dev`. O pacote forca APK/manifest `0.0.9-alpha.0`/version code `9`, publica Web/APK novos, aplica migration remota `202606070001_openworld_bosque_session_lifecycle_structures_v1.sql`, impede que sessoes expiradas sejam projetadas como `active_session`, descarta cache ativo local expirado, inicia nova visita com `collected_nodes = {}` e full spawn, preserva `Bau`, `Mochila`, `upgrades` e `structures`, e canonicaliza `fogueira_estavel_1` em `upgrades` + `structures`. Bosque World Hub Domain Separation v1 segue preservado como pacote anterior de separacao local/global; Bosque Fogueira Potion Crafting v1, Bosque Durable Bau Mochila v1, Arena PVE Menu Flow Simplification v1 e Bosque Offline-First Checkpoint v1 seguem preservados como baselines anteriores.
...
- Proximo passo: playtest humano do pacote Bosque Session Lifecycle & Durable Structures Hotfix v1; validar entrada apos sessao expirada/24h com nodes full spawn, progresso duravel preservado, Fogueira visivel apos checkpoint e relog, station craft com `po_osso` global correto, cache legado normalizado e regressao da Arena Preparacao/buff flow. Manter microeventos revisionados fora do loop normal do Bosque sem nova decisao.
