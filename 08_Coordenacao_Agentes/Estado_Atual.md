...
- Foco P2 de implementacao: `Projetos/draxos-mobile/` (`BOSQUE_MECANICO_BASICO_V2_PUBLISHED_INTERNAL_ALPHA`, release root `internal-alpha/v0-bosque-v2-guidance-20260604-7c2d981`, official Portal URL `https://draxos-mobile-internal-alpha.pages.dev/`, direct Web URL `https://draxos-mobile-internal-alpha.pages.dev/web/index.html`, deployment evidence `https://ae049df9.draxos-mobile-internal-alpha.pages.dev`, com correcoes locais ainda nao publicadas `Openworld Collection Sync Local Fix` e `Main Menu Refactor` incorporadas ao `master` em `1c72399`, preservando First Access Runtime Fix `4608977`, Integrated Runtime Fix `ab5834c`, Integrated App/Arena/Bosque `99304ed`, Web Launch Resilience, Refugio Visual Cleanup, Openworld QoL regression fix, Foundation Hardening V2 (`FOUNDATION_HARDENING_V2_PUBLISHED_INTERNAL_ALPHA`, release root `internal-alpha/v0-foundation-hardening-v2-hotfix2-20260601-58671a4`, preview `https://ca946749.draxos-mobile-internal-alpha.pages.dev`) e Hardening Platform V1 como baselines anteriores, Track 21 Arena Loop Unlock/Friction como contexto do Autobattler, Track 20 Season 1 Arena Calibration, Remote Lab Runner, Track 13 release safety e Track 14 agent ops)
...
- Pacote local integrado atual: `Openworld Collection Sync Local Fix` + `Main Menu Refactor` incorporado ao `master` em `1c72399`, validado localmente e ainda nao publicado; corrige rollback apos coleta, remove `Modos`/Mode Hub player-facing, `Coletar` geral, `Energia` direta e atalho dev Openworld, mantem `Bosque` como entrada direta, move Preparacao para Arena PVE e deixa compra de Energia na Loja.
...
- Proximo passo: revisar a integracao ja mergeada de Openworld Collection Sync + Main Menu Refactor e decidir publicacao/hotfix antes de ampliar o playtest humano do Bosque Mecanico Basico v2 publicado.
